...
POSTS = [
    ...
    "I think a useful AI engineering layer is terminal-native agent infra:\n\n- shared sessions\n- live human intervention\n- prompt recognizers\n- permission handling\n- subscription-auth CLIs\n- isolated workspaces\n\n`termbridge` is my first cut at that stack.\n\nLocal testing in progress.",
    "How `termbridge` works:\n\n1. start a named tmux session\n2. stream the pane to a browser terminal\n3. expose MCP tools for send/capture/wait\n4. persist Claude Code auth in a Docker volume\n5. let the agent drive the TUI like a human\n\nInteractive CLIs become agent-compatible.",
    "If you stopped using Claude Code for agent workflows because API-based usage changed the economics, this is the direction I think matters:\n\ndo not replace Claude Code.\n\nRun it as the terminal-native coding engine, then let your preferred agent orchestrator pilot it safely.",
    "The hard part is not “call Claude.”\n\nThe hard part is making a real interactive terminal usable by both an agent and a human:\n\nscreen sync, idle detection, OAuth URL recognition, permission prompts, write locks, Docker isolation, browser takeover.\n\nAgent infra gets real here.",
]
