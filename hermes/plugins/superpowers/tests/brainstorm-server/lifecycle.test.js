...
async function runTests() {
  ...
  await test('server-started URL brackets IPv6 URL hosts', async () => {
    ...
    const srv = spawn('node', [SERVER], {
      env: {
        ...
        BRAINSTORM_TOKEN: REDACTED_SET_LOCALLY
        BRAINSTORM_DIR: dir,
  ...
  await test('persists the bound port AND key, and restores both on restart', async () => {
    ...
    const tokenFile=REDACTED_SET_LOCALLY
  ...
  await test('hardens existing persisted token file permissions', async ()=REDACTED_SET_LOCALLY
    ...
    try {
      ...
      srv = spawn('node', [SERVER], {
        env: {
          ...
          BRAINSTORM_TOKEN_FILE: REDACTED_SET_LOCALLY
          BRAINSTORM_LIFECYCLE_CHECK_MS: 100000
        }
      });
      let out = ''; srv.stdout.on('data', d => out += d.toString());
  ...
  await test('stored key can authenticate WebSocket after same-port restart', async ()=REDACTED_SET_LOCALLY
  ...
  await test('fallback with persisted token generates a fresh unpersisted key', async ()=REDACTED_SET_LOCALLY
    const dir = fs.mkdtempSync('/tmp/bs-port-');
    const portFile = path.join(dir, '.last-port');
    const tokenFile=REDACTED_SET_LOCALLY
    const preferredToken=REDACTED_SET_LOCALLY
    let a = null, b = null;
  ...
  await test('fallback with explicit BRAINSTORM_TOKEN fails closed', async ()=REDACTED_SET_LOCALLY
    const dir = fs.mkdtempSync('/tmp/bs-port-');
    const portFile = path.join(dir, '.last-port');
    const explicitToken=REDACTED_SET_LOCALLY
    let a = null, b = null;
  ...
  await test('unauthenticated requests do not defeat the idle timeout', async ()=REDACTED_SET_LOCALLY
    const dir = fs.mkdtempSync('/tmp/bs-life-');
