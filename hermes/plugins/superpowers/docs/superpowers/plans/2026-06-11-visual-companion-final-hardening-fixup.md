...
In `skills/brainstorming/scripts/server.cjs`, replace the current `const TOKEN=REDACTED_SET_LOCALLY
...
let TOKEN=REDACTED_SET_LOCALLY
let tokenSource=REDACTED_SET_LOCALLY
```
...
```js
    if (err.code === 'EADDRINUSE' && !triedFallback) {
      if (tokenSource=REDACTED_SET_LOCALLY
      ...
      if (tokenSource=REDACTED_SET_LOCALLY
        ...
        tokenSource=REDACTED_SET_LOCALLY
      }
