...
## Hermes integration with Claude Pro/Max subscription OAuth

When the user specifically wants Hermes to use their Claude Pro/Max subscription rather than Anthropic API billing, do not configure `provider: anthropic` unless they also have an `ANTHROPIC_API_KEY`. The subscription route is through Claude Code / Agent SDK OAuth:

1. Have the user run `claude setup-token` and place the resulting token in their local Hermes env as `CLAUDE_CODE_OAUTH_TOKEN`. Never ask them to paste the token into chat.
2. Ensure `ANTHROPIC_API_KEY` is not shadowing the OAuth token if the goal is subscription quota.
3. Use the Claude Agent SDK ACP adapter when Hermes needs a direct model/subagent backend rather than one-off `claude -p` calls:
   - `HERMES_COPILOT_ACP_COMMAND=npx`
   - `HERMES_COPILOT_ACP_ARGS=-y @agentclientprotocol/claude-agent-acp`
   - `model.provider=copilot-acp`, `model.base_url=acp://copilot`, `model.api_mode=chat_completions`, `model.default=sonnet`
4. For Hermes child agents, also set `delegation.provider=copilot-acp`, `delegation.model=sonnet`, and `delegation.base_url=acp://copilot`.

See `references/hermes-claude-subscription-acp.md` for detailed setup, verification, and pitfalls.
...
- **Output tokens:** `export MAX_MCP_OUTPUT_TOKENS=REDACTED_SET_LOCALLY
...
| Variable | Effect |
|----------|--------|
| `ANTHROPIC_API_KEY` | API key for authentication (alternative to OAuth); in non-interactive / Agent SDK flows this may shadow subscription OAuth, so unset it when the goal is to use Claude Pro/Max quota. |
| `CLAUDE_CODE_OAUTH_TOKEN` | Long-lived OAuth token from `claude setup-token`; authenticates Claude Code / Agent SDK usage against a Claude Pro, Max, Team, or Enterprise subscription. Do not handle or echo the user's token. |
