...
**Restart required.** `security.redact_secrets` is snapshotted at import time — toggling it mid-session (e.g. via `export HERMES_REDACT_SECRETS=REDACTED_SET_LOCALLY
...
- **Key fact for prompt authoring:** when `TERMINAL_ENV !=REDACTED_SET_LOCALLY
