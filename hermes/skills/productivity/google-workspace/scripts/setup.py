...
Commands:
  setup.py --check                          # Is auth valid? Exit 0=REDACTED_SET_LOCALLY
...
CLIENT_SECRET_PATH=REDACTED_SET_LOCALLY
PENDING_AUTH_PATH=REDACTED_SET_LOCALLY
...
def _load_token_payload(path: Path=REDACTED_SET_LOCALLY
    try:
...
def check_auth_live():
    ...
    if not check_auth(quiet=REDACTED_SET_LOCALLY
        return False
    try:
...
def check_auth(quiet: bool=REDACTED_SET_LOCALLY
...
def store_client_secret(path: str):
    ...
    CLIENT_SECRET_PATH.write_text(json.dumps(data, indent=REDACTED_SET_LOCALLY
...
def get_auth_url():
    ...
    auth_url, state=REDACTED_SET_LOCALLY
        access_type="offline",
        prompt="consent",
    )
    _save_pending_auth(state=REDACTED_SET_LOCALLY
...
def exchange_auth_code(code: str):
    ...
    pending_auth=REDACTED_SET_LOCALLY
    raw_callback = code
    ...
    try:
        ...
        os.environ["OAUTHLIB_RELAX_TOKEN_SCOPE"]=REDACTED_SET_LOCALLY
        flow.fetch_token(code=REDACTED_SET_LOCALLY
    except Exception as e:
    ...
    token_payload=REDACTED_SET_LOCALLY
    ...
    if actually_granted:
        token_payload["scopes"]=REDACTED_SET_LOCALLY
    ...
    TOKEN_PATH.write_text(json.dumps(token_payload, indent=REDACTED_SET_LOCALLY
    PENDING_AUTH_PATH.unlink(missing_ok=REDACTED_SET_LOCALLY
...
def revoke():
    ...
    try:
        ...
        urllib.request.urlopen(
            urllib.request.Request(
                f"https://oauth2.googleapis.com/revoke?token=REDACTED_SET_LOCALLY
                method="POST",
    ...
    TOKEN_PATH.unlink(missing_ok=REDACTED_SET_LOCALLY
    PENDING_AUTH_PATH.unlink(missing_ok=REDACTED_SET_LOCALLY
...
def main():
    ...
    group.add_argument("--client-secret", metavar=REDACTED_SET_LOCALLY
    group.add_argument("--auth-url", action=REDACTED_SET_LOCALLY
    group.add_argument("--auth-code", metavar=REDACTED_SET_LOCALLY
