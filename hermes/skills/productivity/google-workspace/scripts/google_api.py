...
def _ensure_authenticated():
    if not TOKEN_PATH.exists():
        print("Not authenticated. Run the setup script first:", file=REDACTED_SET_LOCALLY
...
def _gws_env() -> dict[str, str]:
    env = os.environ.copy()
    env["GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE"]=REDACTED_SET_LOCALLY
    return env
...
def get_credentials():
    ...
    if not creds.valid:
        print("Token is invalid. Re-run setup.", file=REDACTED_SET_LOCALLY
        sys.exit(1)
    return creds
...
def build_service(api, version):
    ...
    return build(api, version, credentials=REDACTED_SET_LOCALLY
