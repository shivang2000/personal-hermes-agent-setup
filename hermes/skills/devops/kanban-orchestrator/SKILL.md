...
metadata:
  hermes:
    ...
    related_skills: [kanban-worker, bot-taREDACTED_SECRET, bot-task-worker-completion]
---
...
- **Treat worker completion as terminal.** When a worker/card reports done with usable artifacts and no question/blocker, do not send a bot-to-bot acknowledgement. Silence is the expected acknowledgement. Reply only to unblock, correct, or assign real follow-up work. If that reply goes through a shared chat/channel, start it with the target bot's real mention token / numeric user ID (for Discord, `<@BOT_USER_ID>`). In Shivang's Discord setup: office Hermes / `hermes-work-agent` is `<@1516899802913308733>`, and personal Hermes / this bot is `<@1516782563757133964>`. Use `bot-taREDACTED_SECRET` for the full protocol.
