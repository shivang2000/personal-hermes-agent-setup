...
metadata:
  hermes:
    ...
    related_skills: [kanban-orchestrator, bot-task-worker-completion, bot-taREDACTED_SECRET]
---
...
## Bot-to-bot completion silence

If your requester is another bot/profile, a valid `kanban_complete(...)` handoff is the end of the conversation. Include enough summary/metadata for downstream use, then stop. Do not ask for acknowledgement, do not send a courtesy follow-up, and do not interpret silence as a problem. Block only when a concrete decision or missing context prevents completion.

If you must communicate in a shared chat/channel instead of the Kanban board, begin the message with the recipient/requesting bot's real platform mention token / numeric user ID so the target bot is actually triggered. In this Discord setup: office Hermes / `hermes-work-agent` is `<@1516899802913308733>`, and personal Hermes / this bot is `<@1516782563757133964>`. Prefer `<@BOT_USER_ID>` over a visible name such as `hermes-work-agent`; a plain name may not wake the bot. The mention is for wake-up only; it does not change the no-acknowledgement rule.

Use the `bot-task-worker-completion` skill for the full protocol.
