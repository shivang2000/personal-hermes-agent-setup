...
describe('pair-agent flow end-to-end (HTTP only, no ngrok)', () => {
  ...
  test('GET /health returns daemon status and includes token for chrome-extension origin', async ()=REDACTED_SET_LOCALLY
  ...
  test('GET /health without chrome-extension origin does NOT include token', async ()=REDACTED_SET_LOCALLY
    const resp = await fetch(`${daemon.baseUrl}/health`);
    expect(resp.status).toBe(200);
    const body = await resp.json() as any;
  ...
  test('GET /connect alive probe returns {alive: true} unauth', async ()=REDACTED_SET_LOCALLY
  ...
  test('POST /pair with root Bearer returns a setup_key', async ()=REDACTED_SET_LOCALLY
  ...
  test('POST /pair without root Bearer returns 403', async ()=REDACTED_SET_LOCALLY
    const resp = await fetch(`${daemon.baseUrl}/pair`, {
      method: 'POST',
  ...
  test('POST /connect with setup_key exchanges for a scoped token', async ()=REDACTED_SET_LOCALLY
    // 1) Get a setup key
    ...
    const { token, scopes }=REDACTED_SET_LOCALLY
  ...
  test('POST /command with no auth returns 401', async ()=REDACTED_SET_LOCALLY
  ...
  test('POST /sse-session with root Bearer returns a Set-Cookie for gstack_sse', async ()=REDACTED_SET_LOCALLY
  ...
  test('POST /sse-session without root Bearer returns 401', async ()=REDACTED_SET_LOCALLY
    const resp = await fetch(`${daemon.baseUrl}/sse-session`, { method: 'POST' });
    expect(resp.status).toBe(401);
  });

  test('GET /activity/stream without auth returns 401', async ()=REDACTED_SET_LOCALLY
    const resp = await fetch(`${daemon.baseUrl}/activity/stream`);
    expect(resp.status).toBe(401);
  });

  test('GET /activity/stream with ?token=REDACTED_SET_LOCALLY
    // The old ?token=REDACTED_SET_LOCALLY
