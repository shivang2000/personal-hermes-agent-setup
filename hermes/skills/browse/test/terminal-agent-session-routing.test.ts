...
describe('terminal-agent session routing (v1.44+ Commit 2)', () => {
  test('1. validTokens is a Map binding token → sessionId', ()=REDACTED_SET_LOCALLY
    ...
    expect(src).toMatch(/const validTokens=REDACTED_SET_LOCALLY
    expect(src).not.toMatch(/const validTokens=REDACTED_SET_LOCALLY
  });
  ...
  test('3. /internal/grant binds an optional sessionId to the token', ()=REDACTED_SET_LOCALLY
    const src = fs.readFileSync(AGENT_TS, 'utf-8');
