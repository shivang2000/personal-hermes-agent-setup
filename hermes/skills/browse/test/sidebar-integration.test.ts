...
let authToken: string=REDACTED_SET_LOCALLY
let tmpDir: string = '';
let stateFile: string = '';
let queueFile: string = '';

async function api(pathname: string, opts: RequestInit & { noAuth?: boolean }=REDACTED_SET_LOCALLY
  const { noAuth, ...fetchOpts }=REDACTED_SET_LOCALLY
  const headers: Record<string, string> = {
    'Content-Type': 'application/json',
    ...(fetchOpts.headers as Record<string, string> || {}),
  };
  if (!noAuth && !headers['Authorization'] && authToken) {
    headers['Authorization']=REDACTED_SET_LOCALLY
  }
...
describe('sidebar auth', ()=REDACTED_SET_LOCALLY
  test('rejects request without auth token', async ()=REDACTED_SET_LOCALLY
    const resp = await api('/sidebar-command', {
      method: 'POST',
      noAuth: REDACTED_SET_LOCALLY
      body: JSON.stringify({ message: 'test' }),
    });
    expect(resp.status).toBe(401);
  });

  test('rejects request with wrong token', async ()=REDACTED_SET_LOCALLY
    const resp = await api('/sidebar-command', {
      method: 'POST',
  ...
  test('accepts request with correct token', async ()=REDACTED_SET_LOCALLY
    const resp = await api('/sidebar-command', {
      method: 'POST',
