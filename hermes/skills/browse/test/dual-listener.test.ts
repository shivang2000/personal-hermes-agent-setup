...
describe('Tunnel surface filter', () => {
  ...
  test('tunnel surface 403s root token bearers with clear hint', ()=REDACTED_SET_LOCALLY
    const filterBlock = sliceBetween(
      SERVER_SRC,
      "surface === 'tunnel'",
      "if (url.pathname === '/connect' && req.method === 'GET')"
  ...
  test('tunnel surface 401s when non-/connect request lacks scoped token', ()=REDACTED_SET_LOCALLY
    const filterBlock = sliceBetween(
      SERVER_SRC,
      "surface === 'tunnel'",
      "if (url.pathname === '/connect' && req.method === 'GET')"
...
describe('GET /connect alive probe', () => {
  test('GET /connect returns {alive: true} unauth on both surfaces', ()=REDACTED_SET_LOCALLY
