...
describe('browser tab bar (server.ts)', () => {
  ...
  test('/sidebar-tabs requires auth', ()=REDACTED_SET_LOCALLY
...
describe('sidebar auth race prevention', ()=REDACTED_SET_LOCALLY
  const bgSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'background.js'), 'utf-8');
  const spSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'sidepanel.js'), 'utf-8');

  test('getPort response includes authToken (not just port + connected)', ()=REDACTED_SET_LOCALLY
  ...
  test('tryConnect uses token from getPort response', ()=REDACTED_SET_LOCALLY
