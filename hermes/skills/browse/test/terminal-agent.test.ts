...
describe('pty-session-cookie: mint/validate/revoke', () => {
  ...
  test('a freshly minted token validates', ()=REDACTED_SET_LOCALLY
    const { token }=REDACTED_SET_LOCALLY
    expect(validatePtySessionToken(token)).toBe(true);
  });

  test('null and unknown tokens fail validation', ()=REDACTED_SET_LOCALLY
  ...
  test('revoke makes a token invalid', ()=REDACTED_SET_LOCALLY
    const { token }=REDACTED_SET_LOCALLY
    expect(validatePtySessionToken(token)).toBe(true);
...
describe('Source-level guard: /health does NOT surface ptyToken', ()=REDACTED_SET_LOCALLY
  test('/health response body does not include ptyToken', ()=REDACTED_SET_LOCALLY
...
describe('Source-level guard: terminal-agent', () => {
  ...
  test('validates the session token against an in-memory token set', ()=REDACTED_SET_LOCALLY
    const wsHandler = AGENT_SRC.slice(AGENT_SRC.indexOf("if (url.pathname === '/ws')"));
  ...
  test('Sec-WebSocket-Protocol auth: strips gstack-pty. prefix and echoes back', ()=REDACTED_SET_LOCALLY
    const wsHandler = AGENT_SRC.slice(AGENT_SRC.indexOf("if (url.pathname === '/ws')"));
...
describe('Source-level guard: server.ts /pty-session route', () => {
  test('validates AUTH_TOKEN, grants over loopback, returns token + Set-Cookie', ()=REDACTED_SET_LOCALLY
