...
describe('parseProxyConfig', () => {
  ...
  test('parses URL-only env-credentials', ()=REDACTED_SET_LOCALLY
    const cfg = parseProxyConfig({
  ...
  test('parses URL-only no-auth', ()=REDACTED_SET_LOCALLY
  ...
  test('D9: refuses when env has only password and URL has user', ()=REDACTED_SET_LOCALLY
