...
describe('token-registry', ()=REDACTED_SET_LOCALLY
  beforeEach(() => {
  ...
  describe('root token', ()=REDACTED_SET_LOCALLY
    it('identifies root token correctly', ()=REDACTED_SET_LOCALLY
      expect(isRootToken('root-token-for-tests')).toBe(true);
      expect(isRootToken('not-root')).toBe(false);
    });

    it('validates root token with full scopes', ()=REDACTED_SET_LOCALLY
    ...
    // false, not error.
    it('returns false for a multibyte token whose string length matches but UTF-8 byte length differs', ()=REDACTED_SET_LOCALLY
    ...
    it('returns false for a token that differs only in length (same prefix)', ()=REDACTED_SET_LOCALLY
      expect(isRootToken('root-token-for-tests-extra')).toBe(false);
      expect(isRootToken('root-token-for-test')).toBe(false);
    });

    it('returns false for a same-length token that differs only in the last byte', ()=REDACTED_SET_LOCALLY
  ...
  describe('createToken', ()=REDACTED_SET_LOCALLY
    it('creates a session token with defaults', ()=REDACTED_SET_LOCALLY
    ...
    it('creates token with custom scopes', ()=REDACTED_SET_LOCALLY
    ...
    it('creates token with indefinite expiry', ()=REDACTED_SET_LOCALLY
      const info = createToken({
        clientId: 'forever',
        expiresSeconds: null,
      });
    ...
    it('overwrites existing token for same clientId', ()=REDACTED_SET_LOCALLY
  ...
  describe('setup key exchange', () => {
    ...
    it('exchanges setup key for session token', ()=REDACTED_SET_LOCALLY
    ...
    it('rejects session token as setup key', ()=REDACTED_SET_LOCALLY
      const session = createToken({ clientId: 'test' });
      expect(exchangeSetupKey(session.token)).toBeNull();
    });
  });

  describe('validateToken', ()=REDACTED_SET_LOCALLY
    it('validates active session token', ()=REDACTED_SET_LOCALLY
    ...
    it('rejects unknown token', ()=REDACTED_SET_LOCALLY
      expect(validateToken('gsk_sess_unknown')).toBeNull();
    });

    it('rejects expired token', async ()=REDACTED_SET_LOCALLY
  ...
  describe('checkScope', () => {
    ...
    it('root token allows everything', ()=REDACTED_SET_LOCALLY
  ...
  describe('revokeToken', ()=REDACTED_SET_LOCALLY
    it('revokes existing token', ()=REDACTED_SET_LOCALLY
  ...
  describe('rotateRoot', () => {
    it('generates new root and invalidates all tokens', ()=REDACTED_SET_LOCALLY
  ...
  describe('listTokens', ()=REDACTED_SET_LOCALLY
    it('lists active session tokens', ()=REDACTED_SET_LOCALLY
