...
export interface ServerConfig {
   ...
   */
  beforeRoute?: (req: Request, surface: Surface, auth: TokenInfo | null)=REDACTED_SET_LOCALLY
  /**
...
function resolveNgrokAuthtoken(): string | null {
  let authtoken=REDACTED_SET_LOCALLY
...
function isRootRequest(req: Request): boolean {
  const token=REDACTED_SET_LOCALLY
  return token !=REDACTED_SET_LOCALLY
}
...
async function handleCommandInternalImpl(
  ...
  if (tokenInfo && tokenInfo.clientId !=REDACTED_SET_LOCALLY
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  if (!cfg.authToken || cfg.authToken.length < 16) {
    throw new Error('buildFetchHandler: cfg.authToken must be a non-empty string >=REDACTED_SET_LOCALLY
  }
  ...
  const { authToken, browserManager: cfgBrowserManager, startTime, beforeRoute, browsePort }=REDACTED_SET_LOCALLY
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (beforeRoute) {
      const auth=REDACTED_SET_LOCALLY
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      if (url.pathname === '/pty-session' && req.method === 'POST') {
        ...
        return new Response(JSON.stringify({
          ...
          attachToken: REDACTED_SET_LOCALLY
          ...
          ptySessionToken: REDACTED_SET_LOCALLY
      ...
      if (url.pathname === '/pty-dispose' && req.method === 'POST') {
        let body: any;
        try { body = await req.json(); } catch { body = null; }
        const authTokenFromBody=REDACTED_SET_LOCALLY
        // Accept either header bearer OR body authToken. Both must match
        // the root auth token; otherwise reject.
        const headerToken=REDACTED_SET_LOCALLY
        const authedByHeader=REDACTED_SET_LOCALLY
        const authedByBody=REDACTED_SET_LOCALLY
      ...
      if (url.pathname === '/token' && req.method === 'POST') {
        ...
        try {
          const tokenBody=REDACTED_SET_LOCALLY
      ...
      if (url.pathname.startsWith('/token/') && req.method=REDACTED_SET_LOCALLY
        if (!isRootRequest(req)) {
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        const authtoken=REDACTED_SET_LOCALLY
        if (!authtoken) {
          return new Response(JSON.stringify({
      ...
      //
      // Issues a short-lived view-only token in an HttpOnly SameSite=REDACTED_SET_LOCALLY
      ...
      if (url.pathname === '/activity/stream') {
        ...
        // withCredentials: true). The ?token=REDACTED_SET_LOCALLY
        // accepted — URLs leak to logs/referer/history. See N1 in the
        // v1.6.0.0 security wave plan.
        const cookieToken=REDACTED_SET_LOCALLY
      ...
      if (url.pathname === '/batch' && req.method === 'POST') {
        const tokenInfo=REDACTED_SET_LOCALLY
        if (!tokenInfo) {
          return new Response(JSON.stringify({ error: 'Unauthorized' }), {
            status: 401,
            headers: { 'Content-Type': 'application/json' },
