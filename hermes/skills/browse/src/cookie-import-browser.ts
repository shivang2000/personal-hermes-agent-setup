 ...
 *   │    - macOS v10: Keychain password, PBKDF2(..., iter=REDACTED_SET_LOCALLY
 *   │    - Linux v10: "peanuts", PBKDF2(..., iter=1)                  │
 *   │    - Linux v11: libsecret/secret-tool password, iter=REDACTED_SET_LOCALLY
 *   │                                                                  │
...
async function getDerivedKeys(match: BrowserMatch): Promise<Map<string, Buffer>> {
  if (match.platform === 'darwin') {
    const password=REDACTED_SET_LOCALLY
    return new Map([
  ...
  const linuxPassword=REDACTED_SET_LOCALLY
  if (linuxPassword) {
    keys.set(
      'v11',
