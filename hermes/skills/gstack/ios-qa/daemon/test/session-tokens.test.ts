...
describe('SessionTokenStore', ()=REDACTED_SET_LOCALLY
  test('mint returns a token with default 1h TTL', ()=REDACTED_SET_LOCALLY
    const now = 1_000_000;
  ...
  test('validate returns ok for fresh token at the required tier', ()=REDACTED_SET_LOCALLY
  ...
  test('validate rejects null/empty/unknown tokens', ()=REDACTED_SET_LOCALLY
    const store = new SessionTokenStore();
  ...
  test('validate rejects expired tokens', ()=REDACTED_SET_LOCALLY
    let now = 1_000_000;
    const store = new SessionTokenStore(() => now);
  ...
  test('validate rejects tokens with insufficient capability', ()=REDACTED_SET_LOCALLY
    const store = new SessionTokenStore();
  ...
  test('revoke removes a token', ()=REDACTED_SET_LOCALLY
    const store = new SessionTokenStore();
    const r = store.mint({ identity: 'u', capability: 'observe', origin: 'self_service' });
    if ('error' in r) throw new Error('unexpected');
  ...
  test('revokeByIdentity removes all tokens for one identity', ()=REDACTED_SET_LOCALLY
    const store = new SessionTokenStore();
  ...
  test('list returns all active tokens', ()=REDACTED_SET_LOCALLY
    const store = new SessionTokenStore();
