...
public final class StateServer {
    ...
    private var bootTokenValid: Bool=REDACTED_SET_LOCALLY
    ...
    private init(port: UInt16 = 9999) {
        self.port = port
        self.bootToken=REDACTED_SET_LOCALLY
        self.bootTokenPath=REDACTED_SET_LOCALLY
    }
    ...
    public func start() {
        ...
        //    seconds.
        logger.notice("gstack-ios-qa-bootstrap token=REDACTED_SET_LOCALLY
    ...
    private func authorize(request: ParsedRequest) -> Bool {
        guard let auth=REDACTED_SET_LOCALLY
    ...
    private func handleAuthRotate(connection: NWConnection, request: ParsedRequest) {
        ...
        guard bootTokenValid,
              let auth=REDACTED_SET_LOCALLY
              auth.hasPrefix("Bearer "),
              String(auth.dropFirst("Bearer ".count))=REDACTED_SET_LOCALLY
        ...
        guard let dict = try? JSONSerialization.jsonObject(with: request.body) as? JSONDict,
              let newToken=REDACTED_SET_LOCALLY
              newToken.count >=REDACTED_SET_LOCALLY
            send(connection: connection, status: 400, body: ["error": "invalid_rotate_payload"])
            return
        }

        rotatedToken=REDACTED_SET_LOCALLY
        bootTokenValid=REDACTED_SET_LOCALLY
