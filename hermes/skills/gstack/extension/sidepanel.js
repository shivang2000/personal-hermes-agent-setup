...
let serverUrl = null;
let serverToken=REDACTED_SET_LOCALLY
...
function authHeaders() {
  ...
  if (serverToken) h['Authorization']=REDACTED_SET_LOCALLY
  return h;
}
...
// token in the URL (the old ?token=REDACTED_SET_LOCALLY
...
async function ensureSseSessionCookie() {
  ...
  try {
    const resp = await fetch(`${serverUrl}/sse-session`, {
      method: 'POST',
      credentials: REDACTED_SET_LOCALLY
...
async function fetchRefs() {
  ...
  try {
    const headers = {};
    if (serverToken) headers['Authorization']=REDACTED_SET_LOCALLY
...
function renderMatchedRules(data) {
  ...
  const authorRules=REDACTED_SET_LOCALLY
  const uaRules = [];
...
async function connectInspectorSSE() {
  ...
  // Same session-cookie pattern as connectSSE. ?token=REDACTED_SET_LOCALLY
...
function updateConnection(url, token) {
  ...
  serverUrl = url;
  serverToken=REDACTED_SET_LOCALLY
  ...
  if (url) {
    ...
    window.gstackAuthToken=REDACTED_SET_LOCALLY
  } else {
...
chrome.runtime.onMessage.addListener((msg) => {
  if (msg.type === 'health') {
    if (msg.data) {
      ...
      chrome.runtime.sendMessage({ type: 'getToken' }, (resp)=REDACTED_SET_LOCALLY
