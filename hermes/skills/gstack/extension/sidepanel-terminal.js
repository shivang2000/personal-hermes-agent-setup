...
(function () {
  ...
  async function getAuthTokenForScan() {
    ...
    try {
      ...
      const body = await resp.json();
      const token=REDACTED_SET_LOCALLY
      if (token) window.__gstackPtyScanToken=REDACTED_SET_LOCALLY
      return token;
    } catch {
      return '';
    }
  ...
  async function connect() {
    ...
    const attachToken=REDACTED_SET_LOCALLY
    if (!attachToken) {
  ...
  async function forceRestart() {
    ...
    const { terminalPort, sessionId, attachToken, expiresAt: _expiresAt }=REDACTED_SET_LOCALLY
    const token=REDACTED_SET_LOCALLY
