...
describe("extractDiagramFences", () => {
  test("extracts a mermaid fence and replaces it with a token paragraph", ()=REDACTED_SET_LOCALLY
    const md = "# T\n\n```mermaid\ngraph LR\n  A --> B\n```\n\ntail";
  ...
  test("multiple fences get distinct ordinals and tokens", ()=REDACTED_SET_LOCALLY
    const md = "```mermaid\nA\n```\n\nmiddle\n\n```mermaid\nB\n```";
    const { fences } = extractDiagramFences(md);
...
describe("substituteSlots", () => {
  test("replaces the <p>-wrapped token with slot HTML", ()=REDACTED_SET_LOCALLY
