...
SSE endpoints (`/activity/stream`, `/inspector/events`) accept either a Bearer token or the HttpOnly `gstack_sse` cookie (minted via `POST /sse-session`, 30-minute TTL, stream-scope only — cannot be used against `/command`). As of v1.6.0.0 the `?token=REDACTED_SET_LOCALLY
...
- **SSE auth** uses a 30-minute HttpOnly SameSite=REDACTED_SET_LOCALLY
...
3. Save it: `echo 'NGROK_AUTHTOKEN=REDACTED_SET_LOCALLY
