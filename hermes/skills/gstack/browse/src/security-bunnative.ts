...
type HFTokenizerConfig=REDACTED_SET_LOCALLY
  model?: {
    type?: string;
...
let cachedTokenizer: TokenizerState | null=REDACTED_SET_LOCALLY
...
export function loadHFTokenizer(dir: string): TokenizerState {
  const tokenizerPath=REDACTED_SET_LOCALLY
  const raw = fs.readFileSync(tokenizerPath, 'utf8');
  const config: HFTokenizerConfig=REDACTED_SET_LOCALLY
 ...
 */
export function encodeWordPiece(text: string, tok: TokenizerState, maxLength: number=REDACTED_SET_LOCALLY
  ...
  for (const word of words) {
    ...
    let start = 0;
    const subTokens: number[]=REDACTED_SET_LOCALLY
    let badWord = false;
...
export function getCachedTokenizer(): TokenizerState {
  ...
  cachedTokenizer=REDACTED_SET_LOCALLY
...
export interface ClassifyResult {
  ...
  score: number;
  tokensUsed: REDACTED_SET_LOCALLY
}
...
export async function classify(text: string): Promise<ClassifyResult> {
  ...
  if (cls?.tokenizer?._tokenizerConfig) cls.tokenizer._tokenizerConfig.model_max_length=REDACTED_SET_LOCALLY
