...
const tokens=REDACTED_SET_LOCALLY
let rootToken: string=REDACTED_SET_LOCALLY
...
export function initRegistry(root: string): void {
  ...
  if (rootToken !=REDACTED_SET_LOCALLY
    throw new Error(
  ...
  rootToken=REDACTED_SET_LOCALLY
}
...
export function isRootToken(token: string): boolean {
  ...
  const tokenBytes=REDACTED_SET_LOCALLY
  const rootBytes = Buffer.byteLength(rootToken, 'utf8');
  if (tokenBytes !=REDACTED_SET_LOCALLY
...
export function createToken(opts: CreateTokenOptions): TokenInfo {
  ...
  const info: TokenInfo=REDACTED_SET_LOCALLY
    token,
    clientId,
    type: 'session',
    scopes,
...
export function exchangeSetupKey(setupKey: string, sessionExpiresSeconds?: number | null): TokenInfo | null {
  ...
  setup.issuedSessionToken=REDACTED_SET_LOCALLY
...
export function listTokens(): TokenInfo[] {
  const now = new Date();
  const result: TokenInfo[]=REDACTED_SET_LOCALLY
...
export function serializeRegistry(): TokenRegistryState {
  const agents: TokenRegistryState['agents']=REDACTED_SET_LOCALLY
