...
const INTERNAL_TOKEN=REDACTED_SET_LOCALLY
/**
...
const validTokens=REDACTED_SET_LOCALLY
    ...
    fetch(req, server) {
      ...
      if (url.pathname === '/internal/grant' && req.method === 'POST') {
        return internalHandler(req, (body) => {
          if (typeof body?.token=REDACTED_SET_LOCALLY
      ...
      if (url.pathname === '/ws') {
        ...
        let token: string | null=REDACTED_SET_LOCALLY
...
const INTERNAL_TOKEN_FILE=REDACTED_SET_LOCALLY
try {
