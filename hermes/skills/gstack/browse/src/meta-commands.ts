...
function tokenizePipeSegment(segment: string): string[] {
  const tokens: string[]=REDACTED_SET_LOCALLY
  let current = '';
  let inQuote = false;
  for (let i = 0; i < segment.length; i++) {
    ...
    if (ch === '"') {
      ...
      if (current) { tokens.push(current); current=REDACTED_SET_LOCALLY
    } else {
      current += ch;
    }
  }
...
export interface MetaCommandOpts {
  ...
  executeCommand?: (body: { command: string; args?: string[]; tabId?: number }, tokenInfo?: TokenInfo | null)=REDACTED_SET_LOCALLY
