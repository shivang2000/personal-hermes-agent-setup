...
const SENTINEL_TOKEN=REDACTED_SET_LOCALLY
...
describe('buildFetchHandler ownsTerminalAgent gate', () => {
  ...
  let realTokenBackup: string | null=REDACTED_SET_LOCALLY
  let realAgentRecordBackup: string | null = null;

  beforeAll(() => {
    realPortBackup = readIfExists(PORT_FILE);
    realTokenBackup=REDACTED_SET_LOCALLY
  ...
  afterAll(() => {
    ...
    if (realTokenBackup !=REDACTED_SET_LOCALLY
      fs.mkdirSync(stateDir, { recursive: true });
