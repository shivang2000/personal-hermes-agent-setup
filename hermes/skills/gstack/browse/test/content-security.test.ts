...
describe('Centralized wrapping', () => {
  ...
  test('scoped tokens get enhanced wrapping', ()=REDACTED_SET_LOCALLY
    expect(SERVER_SRC).toContain('wrapUntrustedPageContent');
  });

  test('root tokens get basic wrapping (backward compat)', ()=REDACTED_SET_LOCALLY
...
describe('Snapshot split format', () => {
  test('snapshot uses splitForScoped for scoped tokens', ()=REDACTED_SET_LOCALLY
  ...
  test('resume also uses split format for scoped tokens', ()=REDACTED_SET_LOCALLY
