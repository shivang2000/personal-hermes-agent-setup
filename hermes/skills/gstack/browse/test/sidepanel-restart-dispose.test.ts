...
describe('sidepanel-terminal: forceRestart via /pty-restart (v1.44+)', () => {
  test('1. mintSession callers read the 4-tuple (sessionId + attachToken)', ()=REDACTED_SET_LOCALLY
...
describe('sidepanel: pagehide → sendBeacon /pty-dispose (v1.44+)', () => {
  ...
  test('8. pagehide payload carries sessionId + authToken in body (sendBeacon-compat)', ()=REDACTED_SET_LOCALLY
    const src = fs.readFileSync(SIDEPANEL_JS, 'utf-8');
