...
beforeAll(() => {
  ...
  internalToken=REDACTED_SET_LOCALLY
});
...
describe('terminal-agent: /internal/grant', () => {
  test('accepts grants signed with the internal token', async ()=REDACTED_SET_LOCALLY
    const resp = await grantToken('test-cookie-token-very-long-yes');
    expect(resp.status).toBe(200);
  });

  test('rejects grants with the wrong internal token', async ()=REDACTED_SET_LOCALLY
...
describe('terminal-agent: PTY round-trip via real WebSocket (Cookie auth)', ()=REDACTED_SET_LOCALLY
  ...
  test('Sec-WebSocket-Protocol auth path: browser-style upgrade with token in protocol', async ()=REDACTED_SET_LOCALLY
  ...
  test('Sec-WebSocket-Protocol auth: rejects unknown token even with valid Origin', async ()=REDACTED_SET_LOCALLY
    const resp = await fetch(`http://127.0.0.1:${agentPort}/ws`, {
      headers: {
        'Connection': 'Upgrade',
        'Upgrade': 'websocket',
