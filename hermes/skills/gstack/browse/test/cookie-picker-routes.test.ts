...
describe('cookie-picker-routes', () => {
  ...
  describe('JSON responses (with auth)', ()=REDACTED_SET_LOCALLY
  ...
  describe('routing', () => {
    test('unknown path returns 404 (with auth)', async ()=REDACTED_SET_LOCALLY
      const { bm } = mockBrowserManager();
  ...
  describe('session cookie auth', ()=REDACTED_SET_LOCALLY
    ...
    test('HTML response does NOT contain auth token', async ()=REDACTED_SET_LOCALLY
      const { bm } = mockBrowserManager();
  ...
  describe('auth gate security', ()=REDACTED_SET_LOCALLY
    test('GET /cookie-picker/browsers returns 401 without auth', async ()=REDACTED_SET_LOCALLY
      const { bm } = mockBrowserManager();
      const url = makeUrl('/cookie-picker/browsers');
      const req = new Request('http://127.0.0.1:9470', { method: 'GET' });
    ...
    test('POST /cookie-picker/import returns 401 without auth', async ()=REDACTED_SET_LOCALLY
      const { bm } = mockBrowserManager();
      const url = makeUrl('/cookie-picker/import');
    ...
    test('GET /cookie-picker/browsers works with valid Bearer auth', async ()=REDACTED_SET_LOCALLY
      const { bm } = mockBrowserManager();
      const url = makeUrl('/cookie-picker/browsers');
      const req = new Request('http://127.0.0.1:9470', {
        method: 'GET',
