...
describe('server: PTY lease routes (v1.44+ Commit 2)', () => {
  test('1. /pty-session returns the 4-tuple shape (sessionId, attachToken, leaseExpiresAt)', ()=REDACTED_SET_LOCALLY
  ...
  test('2. /pty-session/reattach validates lease + mints fresh attachToken', ()=REDACTED_SET_LOCALLY
    const src = fs.readFileSync(SERVER_TS, 'utf-8');
  ...
  test('4. /pty-dispose accepts body-token (sendBeacon-compatible)', ()=REDACTED_SET_LOCALLY
    const src = fs.readFileSync(SERVER_TS, 'utf-8');
    ...
    expect(block).toContain('authTokenFromBody=REDACTED_SET_LOCALLY
  });
  ...
  test('6. grantPtyToken loopback carries sessionId binding', ()=REDACTED_SET_LOCALLY
    const src = fs.readFileSync(SERVER_TS, 'utf-8');
