...
async function startMockUpstream(opts: MockUpstreamOpts = {}): Promise<MockUpstream> {
  ...
  const requireAuth=REDACTED_SET_LOCALLY
  ...
  const server = net.createServer((sock) => {
    sock.once('data', (greeting) => {
      ...
      const supportsNoAuth=REDACTED_SET_LOCALLY
...
describe('startSocksBridge', () => {
  ...
  test('byte-for-byte round trip through bridge → auth upstream → echo', async ()=REDACTED_SET_LOCALLY
  ...
  test('rejects connection when upstream auth fails', async ()=REDACTED_SET_LOCALLY
