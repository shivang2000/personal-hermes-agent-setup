...
describe('buildSpawnEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.GITHUB_TOKEN=REDACTED_SET_LOCALLY
    process.env.OPENAI_API_KEY=REDACTED_SET_LOCALLY
    process.env.MY_PASSWORD=REDACTED_SET_LOCALLY
    process.env.NPM_TOKEN=REDACTED_SET_LOCALLY
    process.env.AWS_SECRET_ACCESS_KEY=REDACTED_SET_LOCALLY
    process.env.GSTACK_TOKEN=REDACTED_SET_LOCALLY
  ...
  it('untrusted: drops $HOME and secrets', ()=REDACTED_SET_LOCALLY
  ...
  it('untrusted: injects GSTACK_PORT + GSTACK_SKILL_TOKEN', ()=REDACTED_SET_LOCALLY
  ...
  it('trusted: still strips GSTACK_TOKEN (defense in depth)', ()=REDACTED_SET_LOCALLY
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.GSTACK_TOKEN).toBeUndefined();
  });

  it('trusted: keeps developer secrets (intentional)', ()=REDACTED_SET_LOCALLY
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.GITHUB_TOKEN).toBe('gh-secret');
  });

  it('GSTACK_PORT/GSTACK_SKILL_TOKEN can never be overridden by parent env', ()=REDACTED_SET_LOCALLY
    process.env.GSTACK_PORT = '99999'; // attacker-set
    process.env.GSTACK_SKILL_TOKEN=REDACTED_SET_LOCALLY
...
describe.skipIf(SKIP_SPAWN)('spawnSkill: lifecycle', () => {
  it('happy path: returns stdout, exit 0, token revoked', async ()=REDACTED_SET_LOCALLY
    ...
    expect(listTokens().filter(t=REDACTED_SET_LOCALLY
  });

  it('untrusted spawn: GSTACK_SKILL_TOKEN visible, root env scrubbed', async ()=REDACTED_SET_LOCALLY
  ...
  it('timeout fires, exit code 124, token revoked', async ()=REDACTED_SET_LOCALLY
