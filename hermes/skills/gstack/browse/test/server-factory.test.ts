...
describe('server.ts factory API surface', () => {
  describe('resolveConfigFromEnv', () => {
    test('honors AUTH_TOKEN env var', ()=REDACTED_SET_LOCALLY
      const orig = process.env.AUTH_TOKEN;
      process.env.AUTH_TOKEN=REDACTED_SET_LOCALLY
      try {
        ...
        else process.env.AUTH_TOKEN=REDACTED_SET_LOCALLY
      }
    });

    test('falls back to randomUUID when AUTH_TOKEN env is empty', ()=REDACTED_SET_LOCALLY
      const orig = process.env.AUTH_TOKEN;
      process.env.AUTH_TOKEN=REDACTED_SET_LOCALLY
      try {
        const cfg = resolveConfigFromEnv();
    ...
    test('falls back to randomUUID when AUTH_TOKEN is whitespace-only', ()=REDACTED_SET_LOCALLY
      const orig = process.env.AUTH_TOKEN;
      process.env.AUTH_TOKEN=REDACTED_SET_LOCALLY
      try {
        const cfg = resolveConfigFromEnv();
    ...
    test('AUTH_TOKEN whitespace is stripped (including unicode whitespace)', ()=REDACTED_SET_LOCALLY
      const orig = process.env.AUTH_TOKEN;
    ...
    test('AUTH_TOKEN shorter than 16 chars after stripping falls back to randomUUID', ()=REDACTED_SET_LOCALLY
      const orig = process.env.AUTH_TOKEN;
    ...
    test('AUTH_TOKEN of only zero-width unicode whitespace falls back to randomUUID', ()=REDACTED_SET_LOCALLY
      const orig = process.env.AUTH_TOKEN;
    ...
    test('returns a populated config object with the expected shape', () => {
      ...
      expect(cfg).toMatchObject({
        authToken: REDACTED_SET_LOCALLY
...
describe('buildFetchHandler factory contract', () => {
  ...
  test('2a. cfg.authToken authenticates /health (positive — bearer accepted)', async ()=REDACTED_SET_LOCALLY
  ...
  test('2b. wrong bearer to /command returns 401 (negative)', async ()=REDACTED_SET_LOCALLY
    const cfg = makeMinimalConfig();
    const handle = buildFetchHandler(cfg);
  ...
  test('3. throws on empty cfg.authToken', ()=REDACTED_SET_LOCALLY
    expect(() => buildFetchHandler(makeMinimalConfig({ authToken: '' }))).toThrow(/authToken/i);
  });

  test('3b. throws on short cfg.authToken (under 16 chars)', ()=REDACTED_SET_LOCALLY
  ...
  test('7. passes valid TokenInfo to beforeRoute for authed requests', async ()=REDACTED_SET_LOCALLY
    const cfg = makeMinimalConfig();
    let capturedAuth: any=REDACTED_SET_LOCALLY
    const handle = buildFetchHandler({
      ...cfg,
      beforeRoute: async (_req, _surface, auth)=REDACTED_SET_LOCALLY
    });
    const req = new Request('http://127.0.0.1/health', {
      headers: { Authorization: `Bearer ${cfg.authToken}` },
    });
  ...
  test('8. passes null to beforeRoute for unauthenticated requests', async ()=REDACTED_SET_LOCALLY
    let capturedAuth: any=REDACTED_SET_LOCALLY
    const handle = buildFetchHandler(makeMinimalConfig({
      beforeRoute: async (_req, _surface, auth)=REDACTED_SET_LOCALLY
    }));
  ...
  test('10. tunnel surface fires beforeRoute with surface===tunnel', async () => {
    ...
    const handle = buildFetchHandler({
      ...cfg,
      beforeRoute: async (_req, surface, _auth)=REDACTED_SET_LOCALLY
    });
  ...
  test('11. initRegistry idempotent under same-token re-init', ()=REDACTED_SET_LOCALLY
    __resetRegistry();
    initRegistry('same-token-pad-to-16-chars');
    expect(() => initRegistry('same-token-pad-to-16-chars')).not.toThrow();
  });

  test('12. initRegistry throws under different-token re-init', ()=REDACTED_SET_LOCALLY
    __resetRegistry();
