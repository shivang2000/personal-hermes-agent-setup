...
const TOKENIZER_AVAILABLE=REDACTED_SET_LOCALLY

describe('bun-native tokenizer', ()=REDACTED_SET_LOCALLY
  test.skipIf(!TOKENIZER_AVAILABLE)('loads HF tokenizer.json into a WordPiece state', async ()=REDACTED_SET_LOCALLY
    const { loadHFTokenizer }=REDACTED_SET_LOCALLY
  ...
  test.skipIf(!TOKENIZER_AVAILABLE)('encodes simple English into [CLS] ... [SEP] frame', async ()=REDACTED_SET_LOCALLY
    const { loadHFTokenizer, encodeWordPiece }=REDACTED_SET_LOCALLY
    const tok = loadHFTokenizer(MODEL_DIR);
  ...
  test.skipIf(!TOKENIZER_AVAILABLE)('truncates to max_length', async ()=REDACTED_SET_LOCALLY
    const { loadHFTokenizer, encodeWordPiece }=REDACTED_SET_LOCALLY
    const tok = loadHFTokenizer(MODEL_DIR);
  ...
  test.skipIf(!TOKENIZER_AVAILABLE)('unknown tokens fall back to [UNK]', async ()=REDACTED_SET_LOCALLY
    const { loadHFTokenizer, encodeWordPiece }=REDACTED_SET_LOCALLY
    const tok = loadHFTokenizer(MODEL_DIR);
  ...
  test.skipIf(!TOKENIZER_AVAILABLE)('matches transformers.js for a regression set', async ()=REDACTED_SET_LOCALLY
    ...
    const { loadHFTokenizer, encodeWordPiece }=REDACTED_SET_LOCALLY
    const { env, AutoTokenizer }=REDACTED_SET_LOCALLY
    ...
    if ((ref as any)?._tokenizerConfig) {
      (ref as any)._tokenizerConfig.model_max_length=REDACTED_SET_LOCALLY
    }
...
describe('bun-native benchmark harness', () => {
  test.skipIf(!TOKENIZER_AVAILABLE)('benchClassify returns well-shaped latency report', async ()=REDACTED_SET_LOCALLY
