...
describe('Server auth security', ()=REDACTED_SET_LOCALLY
  // Test 1: /health serves token conditionally (headed mode or chrome extension only)
  test('/health serves token only in headed mode or to chrome extensions', ()=REDACTED_SET_LOCALLY
  ...
  test('/refs endpoint requires authentication', ()=REDACTED_SET_LOCALLY
  ...
  test('/activity/history requires authentication', ()=REDACTED_SET_LOCALLY
  ...
  // (N1: ?token=REDACTED_SET_LOCALLY
  test('/activity/stream requires authentication with inline token check', ()=REDACTED_SET_LOCALLY
  ...
  test('/command endpoint sits ABOVE the blanket root-only auth gate', ()=REDACTED_SET_LOCALLY
  ...
  test('/command uses getTokenInfo for auth, not validateAuth', ()=REDACTED_SET_LOCALLY
  ...
  test('/tunnel/start requires root token', ()=REDACTED_SET_LOCALLY
  ...
  test('/pair requires root token', ()=REDACTED_SET_LOCALLY
  ...
  test('/pair creates setup keys, not session tokens', ()=REDACTED_SET_LOCALLY
    const pairBlock = sliceBetween(SERVER_SRC, "url.pathname === '/pair'", "/tunnel/start");
  ...
  test('tab ownership check runs before command dispatch for scoped tokens', ()=REDACTED_SET_LOCALLY
  ...
  test('handleMetaCommand accepts tokenInfo for chain scope checking', ()=REDACTED_SET_LOCALLY
    const metaSrc = fs.readFileSync(path.join(import.meta.dir, '../src/meta-commands.ts'), 'utf-8');
  ...
  test('server passes tokenInfo to handleMetaCommand', ()=REDACTED_SET_LOCALLY
  ...
  test('activity events include clientId from token', ()=REDACTED_SET_LOCALLY
  ...
  test('/batch endpoint sits ABOVE the blanket root-only auth gate', ()=REDACTED_SET_LOCALLY
  ...
  test('/batch uses getTokenInfo for auth, not validateAuth', ()=REDACTED_SET_LOCALLY
  ...
  test('cookie-picker UI does not accept or inline auth token', ()=REDACTED_SET_LOCALLY
    ...
    expect(uiSrc).not.toContain("AUTH_TOKEN=REDACTED_SET_LOCALLY
    expect(uiSrc).not.toContain("AUTH_TOKEN=REDACTED_SET_LOCALLY
  });
