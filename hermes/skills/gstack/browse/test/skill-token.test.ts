...
describe('skill-token', ()=REDACTED_SET_LOCALLY
  beforeEach(() => {
  ...
  describe('mintSkillToken', ()=REDACTED_SET_LOCALLY
    it('mints a session token for the spawn', ()=REDACTED_SET_LOCALLY
    ...
    it('minted token validates and grants browser-driving scope', ()=REDACTED_SET_LOCALLY
      const info = mintSkillToken({
    ...
    it('minted token denies admin commands (eval, js, cookies, storage)', ()=REDACTED_SET_LOCALLY
      const info = mintSkillToken({
        skillName: 'hn', spawnId: 's1', spawnTimeoutSeconds: 60,
      });
      const validated = validateToken(info.token);
    ...
    it('minted token denies control commands (state, stop, restart)', ()=REDACTED_SET_LOCALLY
      const info = mintSkillToken({
        skillName: 'hn', spawnId: 's1', spawnTimeoutSeconds: 60,
      });
      const validated = validateToken(info.token);
    ...
    it('two spawns of the same skill mint distinct tokens', ()=REDACTED_SET_LOCALLY
  ...
  describe('revokeSkillToken', ()=REDACTED_SET_LOCALLY
    it('revokes the token for a given spawn', ()=REDACTED_SET_LOCALLY
