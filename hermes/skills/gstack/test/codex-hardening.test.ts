...
describe('gstack-codex-probe: auth probe', ()=REDACTED_SET_LOCALLY
  test('CODEX_API_KEY set → AUTH_OK', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
      const r = runProbe({
  ...
  test('OPENAI_API_KEY set → AUTH_OK', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: '_gstack_codex_auth_probe',
  ...
  test('${CODEX_HOME:-~/.codex}/auth.json exists → AUTH_OK', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
  ...
  test('no env + no file → AUTH_FAILED with exit 1', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
      const r = runProbe({ snippet: '_gstack_codex_auth_probe', home });
  ...
  test('both CODEX_API_KEY and OPENAI_API_KEY set → AUTH_OK', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: '_gstack_codex_auth_probe',
  ...
  test('empty-string env vars + no file → AUTH_FAILED', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: '_gstack_codex_auth_probe',
  ...
  test('whitespace-only env vars + no file → AUTH_FAILED', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: '_gstack_codex_auth_probe',
...
describe('gstack-codex-probe: telemetry event emission', () => {
  ...
  test('payload never contains prompt content, env values, or auth tokens (schema check)', ()=REDACTED_SET_LOCALLY
    const home = tempHome();
    try {
      const r = runProbe({
        ...
        env: {
          _TEL: 'community',
          CODEX_API_KEY: REDACTED_SET_LOCALLY
