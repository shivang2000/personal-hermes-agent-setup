...
function runVerify(token: string, url: string): { code: number; stdout: string; stderr: string } {
  const result = spawnSync(VERIFY_BIN, [url], {
    env: {
      ...
      GBRAIN_MCP_TOKEN: REDACTED_SET_LOCALLY
    },
    encoding: 'utf-8',
  });
  return {
...
describe('gstack-gbrain-mcp-verify', () => {
  ...
  test('AUTH: HTTP 401', ()=REDACTED_SET_LOCALLY
  ...
  test('AUTH: HTTP 403', ()=REDACTED_SET_LOCALLY
    makeFakeCurl({ httpCode: '403', bodyOnInit: '{}' });
    const r = runVerify('badtoken', 'https://example.com/mcp');
    expect(JSON.parse(r.stdout).error_class).toBe('AUTH');
  });

  test('AUTH: HTTP 500 with stale-token-shaped body', ()=REDACTED_SET_LOCALLY
    makeFakeCurl({
      httpCode: '500',
  ...
  test('MALFORMED: HTTP 500 without auth-shape (e.g., real server crash)', ()=REDACTED_SET_LOCALLY
  ...
  test('REGRESSION: token never appears in argv (must be in env, not command line)', ()=REDACTED_SET_LOCALLY
    const initBody =
      'event: message\ndata: {"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{}},"serverInfo":{"name":"gbrain","version":"0.27.1"}},"jsonrpc":"2.0","id":1}';
    makeFakeCurl({ httpCode: '200', bodyOnInit: initBody });
  ...
  test('USAGE: missing GBRAIN_MCP_TOKEN env exits 2', ()=REDACTED_SET_LOCALLY
    makeFakeCurl({});
