...
describe('gstack-gbrain-supabase-verify', () => {
  ...
  test('rejects empty password', ()=REDACTED_SET_LOCALLY
    const r = runVerify(
  ...
  test('error messages never echo the URL password', ()=REDACTED_SET_LOCALLY
...
describe('gstack-gbrain-lib.sh read_secret_to_env', ()=REDACTED_SET_LOCALLY
  test('reads secret from piped stdin into the named env var', ()=REDACTED_SET_LOCALLY
  ...
  test('redacted preview uses the provided sed expression (password masked)', ()=REDACTED_SET_LOCALLY
    const r = runLibSnippet(
      `
  ...
  test('secret value never appears on stdout', ()=REDACTED_SET_LOCALLY
