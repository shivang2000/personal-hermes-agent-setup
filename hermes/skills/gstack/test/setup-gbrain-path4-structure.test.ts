...
describe('setup-gbrain Path 4 (Remote MCP) — structural contract', () => {
  ...
  test('Step 4 collects the bearer via read_secret_to_env, never argv', ()=REDACTED_SET_LOCALLY
  ...
  test('Step 8 explicitly says the bearer is never written to CLAUDE.md', ()=REDACTED_SET_LOCALLY
  ...
  test('Step 9 smoke test on Path 4 prints a placeholder, never the real token', ()=REDACTED_SET_LOCALLY
...
describe('setup-gbrain Path 4 — token security regressions', ()=REDACTED_SET_LOCALLY
  test('the template never inlines a real-shaped bearer string', ()=REDACTED_SET_LOCALLY
    ...
    const realTokenShape=REDACTED_SET_LOCALLY
    expect(tmpl).not.toMatch(realTokenShape);
  });

  test('Path 4 always uses env-var $GBRAIN_MCP_TOKEN, never inline strings', ()=REDACTED_SET_LOCALLY
    ...
    const bearerLines=REDACTED_SET_LOCALLY
