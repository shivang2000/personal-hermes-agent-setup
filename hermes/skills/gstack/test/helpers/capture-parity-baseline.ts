...
export interface SkillBaselineEntry {
  ...
  skillMdBytes: number;
  skillMdLines: number;
  estTokens: REDACTED_SET_LOCALLY
...
export interface ParityBaseline {
  ...
  estTotalCatalogTokens: REDACTED_SET_LOCALLY
...
export interface BaselineDiff {
  ...
  catalogTokensDelta: REDACTED_SET_LOCALLY
  catalogTokensDeltaPct: REDACTED_SET_LOCALLY
  perSkill: Array<{
    skill: string;
    beforeBytes: number;
    afterBytes: number;
...
export function diffBaselines(before: ParityBaseline, after: ParityBaseline): BaselineDiff {
  ...
  const totalCorpusDeltaPct = before.totalCorpusBytes
    ...
    : 0;
  const catalogTokensDelta=REDACTED_SET_LOCALLY
  const catalogTokensDeltaPct=REDACTED_SET_LOCALLY
