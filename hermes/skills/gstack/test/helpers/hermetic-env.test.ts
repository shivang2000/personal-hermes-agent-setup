...
const CONTAMINATED: NodeJS.ProcessEnv = {
  ...
  ANTHROPIC_API_KEY: REDACTED_SET_LOCALLY
  ...
  OPENAI_API_KEY: REDACTED_SET_LOCALLY
  VOYAGE_API_KEY: REDACTED_SET_LOCALLY
  GH_TOKEN: REDACTED_SET_LOCALLY
  SSH_AUTH_SOCK: REDACTED_SET_LOCALLY
  GIT_AUTHOR_NAME: REDACTED_SET_LOCALLY
};
...
describe('buildHermeticEnv allowlist', () => {
  ...
  test('keeps named auth vars but not the broad ANTHROPIC_ prefix', ()=REDACTED_SET_LOCALLY
  ...
  test('drops session-context and operator-credential vars', ()=REDACTED_SET_LOCALLY
    for (const k of [
  ...
  test('promotes GSTACK_ANTHROPIC_API_KEY when canonical absent (shared shim fn)', ()=REDACTED_SET_LOCALLY
    const base = { ...CONTAMINATED } as NodeJS.ProcessEnv;
    delete base.ANTHROPIC_API_KEY;
    base.GSTACK_ANTHROPIC_API_KEY=REDACTED_SET_LOCALLY
    const e = buildHermeticEnv(base, HERMETIC_VARS);
    expect(e.ANTHROPIC_API_KEY).toBe('REDACTED_SECRET');
...
describe('buildSeedConfig', () => {
  test('stores only the 20-char key suffix and trusts the given dirs', () => {
    ...
    expect('REDACTED_SECRET'.endsWith(approved[0])).toBe(true);
  ...
  test('apiKey undefined → omits customApiKeyResponses, does not throw', ()=REDACTED_SET_LOCALLY
