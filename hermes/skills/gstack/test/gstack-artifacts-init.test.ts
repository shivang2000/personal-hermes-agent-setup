...
function makeFakeGh(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'already-exists' | 'fail'; webUrl?: string }=REDACTED_SET_LOCALLY
  const authStatus=REDACTED_SET_LOCALLY
  const repoCreate = opts.repoCreate ?? 'success';
  const webUrl = opts.webUrl ?? `https://github.com/testuser/gstack-artifacts-testuser`;
  const script = `#!/bin/bash
echo "gh $@" >> "${ghCallLog}"
case "$1" in
  auth) ${authStatus=REDACTED_SET_LOCALLY
  repo)
    shift
    case "$1" in
      create)
...
function makeFakeGlab(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'fail'; webUrl?: string }=REDACTED_SET_LOCALLY
  const authStatus=REDACTED_SET_LOCALLY
  const repoCreate = opts.repoCreate ?? 'success';
...
describe('gstack-artifacts-init provider selection', () => {
  ...
  test('both gh and glab authed → interactive prompt picks GitHub by default (Enter=REDACTED_SET_LOCALLY
    makeFakeGh({});
    makeFakeGlab({});
    const r = run([], { input: '\n' });
    expect(r.status).toBe(0);
  ...
  test('both gh and glab authed → user picks 2 → glab is used', ()=REDACTED_SET_LOCALLY
    makeFakeGh({});
    makeFakeGlab({});
    const r = run([], { input: '2\n' });
    expect(r.status).toBe(0);
  ...
  test('only gh authed → defaults to github (no prompt)', ()=REDACTED_SET_LOCALLY
    makeFakeGh({});
    // No glab installed.
    const r = run([]);
    expect(r.status).toBe(0);
  ...
  test('only glab authed → defaults to gitlab (no prompt)', ()=REDACTED_SET_LOCALLY
    makeFakeGlab({});
    const r = run([]);
    expect(r.status).toBe(0);
    expect(readCalls(glabCallLog).some((c) => c.startsWith('glab repo create'))).toBe(true);
  ...
  test('neither authed → falls through to manual URL paste', ()=REDACTED_SET_LOCALLY
    // No gh, no glab fakes.
