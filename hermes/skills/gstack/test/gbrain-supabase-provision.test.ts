...
describe('list-orgs', () => {
  ...
  test('sends Authorization: Bearer <PAT> header', async ()=REDACTED_SET_LOCALLY
    let authHeader=REDACTED_SET_LOCALLY
    mock = startMock({
      'GET /v1/organizations': (req) => {
        authHeader=REDACTED_SET_LOCALLY
        return jsonResp([]);
      },
    });
    ...
    expect(authHeader).toBe('REDACTED_SECRET');
  });

  test('exits 3 with auth error when SUPABASE_ACCESS_TOKEN is missing', async ()=REDACTED_SET_LOCALLY
    const r = await runBin(['list-orgs']);
    expect(r.status).toBe(3);
    expect(r.stderr).toContain('SUPABASE_ACCESS_TOKEN is not set');
  });

  test('exits 3 on 401 Unauthorized', async ()=REDACTED_SET_LOCALLY
    mock = startMock({
