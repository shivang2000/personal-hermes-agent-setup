...
`;
  ...
  if (voyageKey === undefined) {
    ...
    baseEnv.VOYAGE_API_KEY=REDACTED_SET_LOCALLY
  }
...
describe("voyage-code-3 default for gstack-driven PGLite init", () => {
  it("passes voyage-code-3 flags when VOYAGE_API_KEY is set", ()=REDACTED_SET_LOCALLY
  ...
  it("omits voyage flags when VOYAGE_API_KEY is unset", ()=REDACTED_SET_LOCALLY
    const env = makeFakeEnv();
    try {
  ...
  it("treats empty-string VOYAGE_API_KEY the same as unset (no false positive)", ()=REDACTED_SET_LOCALLY
    const env = makeFakeEnv();
    try {
...
describe("template alignment: the .tmpl actually contains the voyage gate", () => {
  ...
  it("setup-gbrain template gates the embedding-model flag on VOYAGE_API_KEY", ()=REDACTED_SET_LOCALLY
