...
describe("HIGH credential patterns", ()=REDACTED_SET_LOCALLY
  const cases: Array<[string, string]> = [
    ["aws.access_key", "key = AKIA1234567890ABCDEF"],
    ["aws.secret_key", "aws_secret_access_key=REDACTED_SET_LOCALLY
  ...
  test("openai.key flags modern sk-proj-/sk-svcacct-/sk-admin- shapes (#1868)", () => {
    ...
    for (const key of missed) {
      expect(ids(`OPENAI_API_KEY=REDACTED_SET_LOCALLY
    }
  ...
  test("openai.key does not over-match prose / malformed sk- strings (#1868 calibration)", () => {
    ...
    const benign = [
      "the REDACTED_SECRET", // hyphenated prose
      "REDACTED_SECRET",
  ...
  test("twilio.auth_token needs an SID nearby", ()=REDACTED_SET_LOCALLY
  ...
  test("db.url_with_password flags real password, skips placeholder/env-var", ()=REDACTED_SET_LOCALLY
...
describe("MEDIUM demoted credential-shaped patterns (TENSION-1)", ()=REDACTED_SET_LOCALLY
  ...
  test("google.api_key is MEDIUM", ()=REDACTED_SET_LOCALLY
  ...
  test("env.kv fires on high-entropy, skips placeholder", () => {
    expect(ids("API_TOKEN=REDACTED_SET_LOCALLY
    expect(ids("API_KEY=REDACTED_SET_LOCALLY
    expect(ids("API_KEY=REDACTED_SET_LOCALLY
  });
});
...
describe("placeholder suppression (per-span)", () => {
  ...
  test("a real secret on a line that ALSO contains EXAMPLE still flags", ()=REDACTED_SET_LOCALLY
...
describe("no visibility-based tier promotion (TENSION-2-followup)", () => {
  ...
  test("demoted credential patterns stay MEDIUM on public", ()=REDACTED_SET_LOCALLY
...
describe("tool-attributed fence WARN-degrade (TENSION-3)", () => {
  test("placeholder-shaped credential in tool fence → WARN", ()=REDACTED_SET_LOCALLY
  ...
  test("live-format credential in tool fence STILL blocks", ()=REDACTED_SET_LOCALLY
...
describe("taxonomy integrity", () => {
  ...
  test("autoRedactable patterns have a redactToken", ()=REDACTED_SET_LOCALLY
