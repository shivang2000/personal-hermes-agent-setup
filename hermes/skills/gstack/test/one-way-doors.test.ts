...
describe("one-way-door credential keyword net (#1839)", ()=REDACTED_SET_LOCALLY
  ...
  test('"rotate the database password" classifies one-way', ()=REDACTED_SET_LOCALLY
    const r = classifyQuestion({ summary: "rotate the database password" });
    expect(r.oneWay).toBe(true);
    expect(r.reason).toBe("keyword");
  });

  test("revoke/reset/rotate are all parallel for password", ()=REDACTED_SET_LOCALLY
  ...
  test("rotate still catches the other credential nouns", ()=REDACTED_SET_LOCALLY
