...
const EXPECTED_v144_NUMBERS = {
  ...
  estTotalCatalogTokensMin: REDACTED_SET_LOCALLY
  estTotalCatalogTokensMax: REDACTED_SET_LOCALLY
};
...
describe('parity-baseline-v1.44.1.json integrity (v1→v2 reference)', () => {
  ...
  test('estTotalCatalogTokens matches the CHANGELOG-cited ~9,319', ()=REDACTED_SET_LOCALLY
    const baseline = JSON.parse(fs.readFileSync(BASELINE_PATH, 'utf-8'));
