...
describe('secret-sink-harness — positive controls', ()=REDACTED_SET_LOCALLY
  ...
  test('catches a seed leaked in base64-encoded form (auth header pattern)', async ()=REDACTED_SET_LOCALLY
...
describe('secret-sink-harness — real bins (negative controls)', ()=REDACTED_SET_LOCALLY
  test('supabase-verify does not leak a URL password on reject', async ()=REDACTED_SET_LOCALLY
    const bin = path.join(ROOT, 'bin', 'gstack-gbrain-supabase-verify');
    const seedPassword=REDACTED_SET_LOCALLY
  ...
  test('lib.sh read_secret_to_env does not leak stdin via captured channels', async ()=REDACTED_SET_LOCALLY
  ...
  test('supabase-provision does not leak a PAT on auth-failure path', async ()=REDACTED_SET_LOCALLY
    ...
    const r = await runWithSecretSink({
      ...
      seeds: [seedPat],
      env: {
        SUPABASE_ACCESS_TOKEN: REDACTED_SET_LOCALLY
