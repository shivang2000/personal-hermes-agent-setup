...
`;
  ...
  it("--scan-secrets opt-in: skips files with gitleaks findings, lets clean files through", ()=REDACTED_SET_LOCALLY
    const home = makeTestHome();
    const gstackHome = join(home, ".gstack");
    mkdirSync(gstackHome, { recursive: true });
...
`;
    ...
    expect(r.exitCode).toBe(0);
    // Bulk report shows skipped (secret-scan) >=REDACTED_SET_LOCALLY
