...
for (const currentHost of hostsToRun) {
  ...
  try {
    ...
    const tokenBudget: Array<{ skill: string; lines: number; tokens: number }>=REDACTED_SET_LOCALLY
    ...
    for (const tmplPath of findTemplates()) {
      ...
      const TOKEN_CEILING_BYTES=REDACTED_SET_LOCALLY
...
`;
    ...
    if (!DRY_RUN && tokenBudget.length > 0) {
      tokenBudget.sort((a, b)=REDACTED_SET_LOCALLY
      const totalLines = tokenBudget.reduce((s, t) => s + t.lines, 0);
      const totalTokens=REDACTED_SET_LOCALLY
