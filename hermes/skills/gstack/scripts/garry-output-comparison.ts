 ...
 *      GSTACK_AUTHOR_EMAILS=REDACTED_SET_LOCALLY
...
function resolveAuthorEmails(argv: string[]): string[] {
  ...
  process.stderr.write(
    ...
    'set GSTACK_AUTHOR_EMAILS=REDACTED_SET_LOCALLY
  );
  process.exit(1);
}
...
function enumerateCommits(year: number, repoPath: string, authorEmails: string[]): string[] {
  ...
  const authorFlags=REDACTED_SET_LOCALLY
  try {
...
function analyzeRepo(repoPath: string, year: number, authorEmails: string[], sccAvailable: boolean, now: Date=REDACTED_SET_LOCALLY
...
function main() {
  ...
  const authorEmails=REDACTED_SET_LOCALLY
