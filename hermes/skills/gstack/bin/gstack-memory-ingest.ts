...
interface CliArgs {
   ...
   */
  scanSecrets: REDACTED_SET_LOCALLY
}
...
interface BulkResult {
  written: number;
  skipped_secret: REDACTED_SET_LOCALLY
...
function parseArgs(): CliArgs {
  ...
  let scanSecrets=REDACTED_SET_LOCALLY
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (a) {
      ...
      case "--scan-secrets": scanSecrets=REDACTED_SET_LOCALLY
      case "--limit":
...
function preparePages(
  ...
  skippedSecret: REDACTED_SET_LOCALLY
  skippedDedup: number;
  ...
  let skippedSecret=REDACTED_SET_LOCALLY
  let skippedDedup = 0;
