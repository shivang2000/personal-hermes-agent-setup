...
afterEach(() => {
  ...
  else process.env.OPENAI_API_KEY=REDACTED_SET_LOCALLY
  if (originalNodeEnv === undefined) delete process.env.NODE_ENV;
  else process.env.NODE_ENV = originalNodeEnv;
  fs.rmSync(tmpDir, { recursive: true, force: true });
});

describe("resolveApiKeyInfo", ()=REDACTED_SET_LOCALLY
  test("uses ~/.gstack/openai.json before OPENAI_API_KEY", ()=REDACTED_SET_LOCALLY
  ...
  test("uses OPENAI_API_KEY when no config file exists", ()=REDACTED_SET_LOCALLY
    process.env.OPENAI_API_KEY=REDACTED_SET_LOCALLY
  ...
  test("reports when OPENAI_API_KEY matches current-directory .env", ()=REDACTED_SET_LOCALLY
    fs.writeFileSync(path.join(tmpDir, ".env"), "OPENAI_API_KEY=REDACTED_SET_LOCALLY
    process.env.OPENAI_API_KEY=REDACTED_SET_LOCALLY
  ...
  test("detects quoted and exported env-file values", () => {
    fs.writeFileSync(path.join(tmpDir, ".env.local"), "export OPENAI_API_KEY=REDACTED_SET_LOCALLY
    process.env.OPENAI_API_KEY=REDACTED_SET_LOCALLY
...
describe("requireApiKey", ()=REDACTED_SET_LOCALLY
