...
- **Anthropic help center URLs rot — old article IDs 404 after restructures.** The classic Anthropic support article IDs (e.g. `8325612` for rate limits) now return "UH OH. THAT PAGE DOESN'T EXIST." Anthropic restructured their help center from `support.anthropic.com` to `support.claude.com` with new article IDs. The reliable pattern: (1) navigate to the collection page (`https://support.claude.com/en/collections/4078531-claude`), (2) use `browser_console` to extract actual hrefs from link text: `Array.from(document.querySelectorAll('a')).filter(a => a.textContent.includes('your text')).map(a => a.href)`, (3) `browser_navigate` to the returned URL. Do NOT guess article IDs or try old URLs — they won't redirect. Also note: `browser_click` on snapshot link refs sometimes silently fails to navigate (returns success but stays on same page) — `browser_console` + `browser_navigate` is the reliable fallback for help center navigation.
...
- **OpenAI/ChatGPT pricing pages are Cloudflare-blocked with client-side rendered prices.** As of Aug 2026, `chatgpt.com/pricing/`, `openai.com/chatgpt/pricing/`, and `help.openai.com/en/articles/*` all return Cloudflare "Just a moment..." challenges to curl and browser_navigate. The Wayback Machine snapshot renders the full page, BUT dollar amounts are not in the HTML source — they are resolved at runtime from token identifiers (`chatgpt.plus`, `chatgpt.go`, `chatgpt.pro.5x`) that map to currency tables. To extract rendered prices: `browser_navigate` to the Wayback URL, then `browser_console` with `document.querySelector('body').innerText` to get the post-render DOM text. Raw HTML parsing alone will find feature names and comparison table structure but NOT the actual dollar amounts. See `references/openai-chatgpt-pricing-2026-08.md` for a full extracted snapshot and the `bot-blocked-web-fetch` skill for the Wayback fallback technique.

## Shape 6 — Reverse-engineering SPA JS bundles for data behind login walls

When a consumer SaaS pricing page (e.g. kimi.com) requires login to view subscription tiers, but the SPA loads JS bundles that contain the plan/subscription logic, you can extract structural information (tier names, billing cycles, currencies, feature gates) without authenticating.

Pattern:
1. `curl` the landing page HTML and grep for `modulepreload` or `script` tags pointing to static asset CDN URLs (e.g. `statics.moonshot.cn/kimi-web-seo/assets/`).
2. Identify bundles named after the feature you need: `subscription-*.js`, `check-*.js`, `goods_pb-*.js`, `kimi.utils-*.js`.
3. `curl` each bundle and grep for business logic keywords: `isFreePlan`, `isPaidPlan`, `isMonthlyPlan`, `isAnnuallyPlan`, `isUSDPlan`, `isCNYPlan`, `Membership_Level`, `billingCycle`, `priceInCents`.
4. The JS will reveal: membership tier enum values (FREE, BASIC/INTERMEDIATE, higher), which features are gated to which tier, supported currencies, billing cycles, trial duration logic.

**Limitations**: This gives you the *structure* (how many tiers, what features are gated, what currencies/billing cycles exist) but NOT the *prices* (those are typically fetched from an authenticated API at runtime). Always state this limitation clearly in output — "tier structure verified from JS bundle, exact prices not confirmed without auth."

Example session: kimi.com consumer subscription — JS bundles revealed FREE/BASIC/INTERMEDIATE tiers, monthly+annual billing, USD+CNY support, 7-day trial, Kimi Claw/Code Highspeed gated to INTERMEDIATE+, but exact $/month prices were not extractable without login.

- **All major search engines can be simultaneously CAPTCHA-blocked from the agent environment.** Google, Bing, DuckDuckGo (HTML + lite), Brave Search, and Startpage all returned CAPTCHA/bot-detection pages in a single August 2026 session. Do NOT loop through them retrying — the block is network/IP-level, not per-engine. Fallback ladder: (1) go directly to known official URLs (docs sites, platform homepages, blog indexes) and navigate via sidebar/clickable links, (2) if you don't know the URL, check `references/sources-by-domain.md` for the domain, (3) use `browser_navigate` to the root domain and look for Docs/Pricing/Blog links in the snapshot, (4) for historical snapshots, use the Wayback Machine (`web.archive.org/web/2026/{url}`). Never fabricate a source URL — if you can't reach it, say so.
- **SPA docs sites may redirect direct deep-link navigation to the overview page, while sidebar click navigation works.** Observed on `platform.kimi.com/docs/pricing/kimi-k3` — `browser_navigate` to this URL always landed on `/docs/overview` regardless of the path. But clicking the sidebar "Kimi K3" link after navigating to the overview loaded the correct page content. This is because the SPA's client-side router doesn't parse the initial URL for deep links but does handle in-app navigation. **Fix**: navigate to the docs root, take a snapshot to find the sidebar link refs, then `browser_click` the relevant link. Do NOT retry the direct URL — it will redirect every time.
...
- `references/consumer-ai-subscription-comparison.md` — reusable evidence checklist and answer format for ~$20 consumer AI plans. Use it to distinguish confirmed plan access and dynamic shared quotas from API pricing/model existence, and to weight compatibility with the user's actual agent setup.
- `references/openai-chatgpt-pricing-2026-08.md` — ChatGPT plan pricing, model access matrix, context windows, Codex access, and usage limits as of August 2026. Extracted from the Cloudflare-blocked `chatgpt.com/pricing/` via Wayback Machine + `browser_console` rendering. Includes the technique for extracting client-side-rendered prices from token identifiers.
- `references/kimi-moonshot-pricing-2026-08.md` — Kimi K3 and K2.x API pricing, rate limit tiers, model list, consumer subscription tier structure, and canonical URLs as of August 2026. K3 API pricing extracted from official docs pages; consumer subscription tier structure extracted from SPA JS bundles via Shape 6 technique.