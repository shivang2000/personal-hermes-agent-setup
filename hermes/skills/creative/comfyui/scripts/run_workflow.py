...
Usage:
    ...
    export COMFY_CLOUD_API_KEY=REDACTED_SET_LOCALLY
    python3 run_workflow.py --workflow workflow_api.json \
        --args '{"prompt": "a cat"}' \
...
class ComfyRunner:
    def __init__(
        ...
        api_key: str | None=REDACTED_SET_LOCALLY
    ...
    def headers(self) -> dict[str, str]:
        h: dict[str, str] = {}
        if self.api_key:
            h["X-API-Key"]=REDACTED_SET_LOCALLY
        return h
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--use-partner-key-as-auth", action=REDACTED_SET_LOCALLY
