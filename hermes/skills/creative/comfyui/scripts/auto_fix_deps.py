...
def install_model(url: str, folder: str, filename: str | None = None,
                  *, dry_run: bool = False, comfy_cmd: str = "comfy",
                  hf_token: str | None=REDACTED_SET_LOCALLY
    cmd = comfy_cmd.split() + [
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--api-key", help=REDACTED_SET_LOCALLY
    p.add_argument("--models-from-file",
                   help="JSON file mapping {model_filename: download_url} for models that need install")
    p.add_argument("--hf-token", help=REDACTED_SET_LOCALLY
    p.add_argument("--civitai-token", help=REDACTED_SET_LOCALLY
