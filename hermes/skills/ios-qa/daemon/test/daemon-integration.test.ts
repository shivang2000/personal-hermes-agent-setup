...
let workDir: string;
const STATE_SERVER_TOKEN=REDACTED_SET_LOCALLY
  ...
  return new Promise((resolve) => {
    ...
    const server = createServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        if (!auth || auth !=REDACTED_SET_LOCALLY
...
describe('daemon — loopback listener', () => {
  ...
  beforeAll(async () => {
    ...
    const tunnel: DeviceTunnel = {
      ...
      ipv6Addr: '127.0.0.1',
      port: stub.port,
      bootTokenRotated: REDACTED_SET_LOCALLY
    };
  ...
  test('proxies /screenshot to stub StateServer with the rotated bearer', async ()=REDACTED_SET_LOCALLY
...
describe('daemon — tailnet listener (mocked tailscaled)', () => {
  ...
  test('/auth/mint rejects unknown identity (mocked WhoIs)', async ()=REDACTED_SET_LOCALLY
  ...
  test('/auth/mint succeeds for allowlisted identity, then proxies are bearer-gated', async ()=REDACTED_SET_LOCALLY
    ...
    const { session_token }=REDACTED_SET_LOCALLY
    ...
    const tapNoAuth=REDACTED_SET_LOCALLY
      headers: { 'content-type': 'application/json' },
  ...
  test('capability tier enforced — observe token cannot call /tap (interact-tier)', async ()=REDACTED_SET_LOCALLY
  ...
  test('rate limit kicks in at 11th /auth/mint per identity', async ()=REDACTED_SET_LOCALLY
    await grantIdentity({ identity: 'caller@example.com', capability: 'observe', path: listPath });
    let last = 0;
    for (let i = 0; i < 11; i++) {
      const r = await fetchWith('POST', `http://127.0.0.1:${daemon.tailnetPort}/auth/mint`, {
  ...
  test('boot token never appears in tailnet responses', async ()=REDACTED_SET_LOCALLY
    await grantIdentity({ identity: 'caller@example.com', capability: 'interact', path: listPath });
    const mintR = await fetchWith('POST', `http://127.0.0.1:${daemon.tailnetPort}/auth/mint`, {
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ capability: 'interact' }),
