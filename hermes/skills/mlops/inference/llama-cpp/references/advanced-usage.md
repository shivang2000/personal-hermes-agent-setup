...
def convert_with_custom_vocab(model_path, output_path):
    ...
    special_tokens=REDACTED_SET_LOCALLY
...
def benchmark(model_path, prompt, n_tokens=REDACTED_SET_LOCALLY
    llm = Llama(
    ...
    # Warmup
    llm(prompt, max_tokens=REDACTED_SET_LOCALLY
    ...
    tokens_per_sec=REDACTED_SET_LOCALLY
    ...
    for n_gpu_layers in range(0, 50, 5):
        for n_batch in [128, 256, 512, 1024]:
            try:
                ...
                # Quick benchmark
                start = time.time()
                llm("Hello", max_tokens=REDACTED_SET_LOCALLY
