...
Reply scan blocked: X API returned `402 CreditsDepleted` for both candidate and duplicate-check searches. Authentication is healthy (`@shivangchheda22` verified via `xurl whoami`), but no replies were posted. X Developer Console credits need topping up before the scanner can resume.
