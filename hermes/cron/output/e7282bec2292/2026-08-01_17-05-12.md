...
**Reply text:**
> Most agent failures live in the harness, not the model. A misconfigured sandbox boundary turns a tool call into a network call. Fix: separate authz for eval runs, no shared creds, treat every eval agent like an untrusted tenant with egress allowlist only.

Status: REPLIED (255 chars, no image attached — text-only was the right call here since no public proof artifact added value beyond the mechanism).
