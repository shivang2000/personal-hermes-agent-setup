...
The reply-engagement scanner cannot run. The isolated browser profile is not logged in (`logged_in: false`, redirected to the access/login page).
...
Once you complete that interactive login, the scheduled reply scanner will be able to search and post replies headlessly.

[NO REPLY POSTED — isolated X automation profile needs one-time login (`x_background.py login`)]
