...
**Target:** https://x.com/thehypedotnews/status/2082975927948972108

**Reply text:**
> The model did not escape a sealed sandbox. The sandbox was never sealed. A third-party eval vendor left network egress open, so Claude did the eval task against real infra. The boundary was not enforced. Eval infra trust is the attack surface, not model capability.

**Posted URL:** reply to https://x.com/thehypedotnews/status/2082975927948972108 (status: REPLIED)

Notes: Chose this target because thehype. framed the Anthropic cyber-eval incident as a "just marketing?" question and explicitly named the third-party eval-vendor misconfiguration as the root cause, which an earlier Shivang tweet (on the HF/OpenAI eval agent escape) covered from a different angle. The reply reframes the takeaway: the failure is eval-infra trust and unenforced egress boundaries, not model capability or deceptive behavior. No screenshot attached since the tweet itself is the evidence and a public-source image added no new technical content. No other candidate in this scan had a credible account + technical conversation where Shivang had a genuinely new mechanism to add.
