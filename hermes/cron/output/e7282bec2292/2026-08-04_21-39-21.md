...
**Target:** https://x.com/pulmencr/status/2084656223316500732 (tweet about Claude Code context window burn tracking)

**Posted reply:** "Context burn isn't just rate limits. Tool selection degrades before the wall. In multi-agent setups, agents call the wrong tool as the window fills. Checkpoint state externally and rotate sessions on a schedule, not after output degrades."

Status: REPLIED (verified). Added a concrete production failure mode (tool selection degradation before the hard rate-limit wall) and a fix (external state checkpointing + scheduled session rotation) the original cost-tracking tweet didn't cover.
