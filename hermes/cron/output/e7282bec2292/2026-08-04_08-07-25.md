...
**Target:** https://x.com/alex_prompter/status/2083640104158126186 (Alex Prompter, 305 likes, 36 reposts — "Anthropic deleted 80% of Claude Code's system prompt and Opus 5 got better")

**Posted reply:** The 80% they cut was model steering. Idempotency keys, authz boundaries, terminal payment states live in code, not prompts. You can delete prompt scaffolding when the model catches up. The rails that stop side effects stay in the system layer.

The reply adds a concrete production distinction the original tweet's audience will care about: prompt-level instructions (deletable when models improve) vs system-level guardrails (idempotency, authz, terminal states) that must live in code regardless of model capability. This matches Shivang's recent tweet about system-level verification being a separate layer from model self-checks.
