...
- **Target:** https://x.com/cyrilXBT/status/2083470539361079770 (CyrilXBT, 731 likes on a recent post, this tweet at 14 likes and climbing — about Opus 5 costing half of Fable 5 and model selection becoming the real skill)
- **Reply text:** "cost-per-token is the wrong unit. a model that's 2x cheaper per token but loops 3x to finish costs more per completed task. the real routing decision is task complexity → model tier, not just price comparison"
- **Status:** REPLIED

The reply adds a concrete production failure mode (looping/retry cost eroding per-token savings) that the original tweet doesn't cover, and extends Shivang's own Jul 30 angle on cost-per-completed-task. No screenshot attached — text-only was the right call here since there's no benchmark chart or terminal output that would add evidence.
