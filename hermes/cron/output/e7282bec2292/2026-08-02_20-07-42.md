...
**Target:** https://x.com/boring_simpson/status/2083872853410087268 (Artem's post about DeepSeek V4 Flash beating GPT-5.6 Luna and Claude Sonnet on Terminal-Bench 2.1 at 14-36x lower cost)

**Reply posted:**
> 82.7 vs 80.4 is a 2.3-point gap, within run variance for agent evals. Cost matters at volume. In production the real question is failure behavior: does it retry idempotently on timeout? Checkpoint state? Benchmarks test clean runs, not recovery.

The reply adds a concrete production-engineering angle the original post misses: benchmark gaps at this magnitude are within noise, and the cost advantage is real, but what actually determines model choice for production agents is failure/recovery behavior (idempotent retries, state checkpointing) that benchmarks don't measure.
