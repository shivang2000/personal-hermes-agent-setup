...
- **Target:** https://x.com/supabase/status/2083282155170340898 (Supabase Evals launch, 565 likes / 85 reposts)
- **Reply:** "The deterministic checks are the strong half. The LLM judge is where it gets shaky. It catches structure but misses subtle data-loss paths and RLS gaps that only show up under concurrent load."

The reply adds a concrete production concern about the eval harness design: deterministic checks are reliable, but the LLM judge layer tends to catch surface-level structure while missing subtle data-loss paths and row-level security gaps that only surface under concurrent load. This is a real tradeoff anyone running production evals will recognize.
