...
The isolated browser profile at `~/.hermes/browser-profiles/x-automation` is not logged in (it is parked on the Cloudflare interstitial). The one-time manual login is required:

```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

Run that in a normal (non-cron) session, complete the X login in the window that opens, return to the terminal and press Enter. Reply scanning cannot run headlessly until `check` reports `"logged_in": true`.
