...
**Reply:** "The harness bugs are the silent ones. Tool returns malformed JSON, agent reasons on garbage, produces code that compiles and passes weak tests, merge goes in. Schema validation on tool responses catches it before the agent sees the output."
