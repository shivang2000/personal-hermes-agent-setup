...
**Target:** https://x.com/ValsAI/status/2084364164655694236
**Reply text:** "Vals Index 66.1 is solid. The production gap: Qwen 3.8 Max is 2.4T params (95B active MoE). At that size, serving cost per agent task dwarfs the leaderboard delta vs smaller open weights. Rank tells you capability, not deploy economics."
**Status:** REPLIED

Chose the Vals AI tweet (official eval account, 134 likes, 17 reposts) on the Qwen 3.8 Max open-weight release. The reply adds a concrete production tradeoff the leaderboard framing misses: a 2.4T param MoE model's serving cost per agent task matters more than its rank delta against smaller open weights. Humanizer applied — no AI-isms, direct builder voice, one clear mechanism (MoE param count vs. deploy economics).
