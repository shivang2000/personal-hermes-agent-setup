...
Reply scan blocked: X API returned `402 credits depleted`. Auth is valid, but search could not run, so no replies were posted. X Developer Console credits need to be topped up before the scanner can resume.
