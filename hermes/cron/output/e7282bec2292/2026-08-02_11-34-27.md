...
Reply posted to Cameron Wolfe's tweet on agent eval difficulty:

**Target:** https://x.com/cwolferesearch/status/2083588813675274301
**Reply:** "Agent evals miss the real failure: irreversible side effects. A correct answer that called a non-idempotent endpoint twice or wrote a duplicate DB row is still a production incident. Eval the full action trajectory and system state at each tool boundary, not just the response."

Chose this post (198 likes, 33 reposts, credible ML researcher) because it directly opened a technical conversation about *why* agent evals are hard, which maps onto Shivang's recent posts about system-level verification vs model self-checks. The reply adds a concrete failure mode (non-idempotent endpoints, duplicate DB rows) and a specific production lesson (eval the action trajectory and system state at tool boundaries), rather than generic praise. 277 chars, humanized, no AI-isms. `status: REPLIED` confirmed.
