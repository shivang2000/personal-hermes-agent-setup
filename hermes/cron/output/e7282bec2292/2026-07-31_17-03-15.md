...
**Target:** https://x.com/trailofbits/status/2083145852873261430
(Trail of Bits — multi-agent hijacking, image prompt injection, browser exfiltration; 16 likes)

**Posted reply:**
> The vector people miss: tool output is untrusted input. A vendor API response (flight result, hotel description, search snippet) can carry injection that reroutes an agent. If your agent reasons over tool output, that's the real injection surface, not the user prompt.

Status: REPLIED. No screenshot attached — the target tweet is a text-only research summary with no public proof artifact that would add value, so text-only was the right call per the screenshot policy.
