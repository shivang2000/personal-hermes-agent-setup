...
**Target:** https://x.com/L1vsun/status/2083623425684468088
**Reply posted:** "Trajectory eval catches right answer + broken path. Harder case: right answer, clean reasoning, but corrupted side effects. An agent returns the correct confirmation while leaving a stuck payment or duplicate booking in the DB. Grade state changes, not just tool calls."

The original tweet (27 likes) argued trajectory eval catches "correct" answers from broken tool-call sequences. The reply adds a production failure mode the framing misses: a clean reasoning path can still produce corrupted side effects (stuck payments, duplicate bookings) that an output-only or trajectory-only eval won't catch. The addition is concrete, non-generic, and matches Shivang's terminal-state/verification-layer voice.
