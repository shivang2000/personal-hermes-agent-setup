...
**Target:** https://x.com/itsjackdev/status/2083515984519024644

**Reply posted:**
> The hardest harness problem isn't the tool loop, it's state recovery. An agent that can't resume after a failed tool call loses most of its progress. In production: idempotency keys and row-locked re-reads, not naive retries.

Status: REPLIED. The reply adds a concrete production lesson (state recovery + idempotency) to Jack's framing of "capability = base model × harness × verification," picking the harness factor as the angle where Shivang has real builder experience to contribute.
