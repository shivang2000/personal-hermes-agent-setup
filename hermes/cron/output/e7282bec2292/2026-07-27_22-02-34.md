...
**Target:** https://x.com/suraj_sharma14/status/2078449718414180393 (a high-engagement tweet with 2,581 likes / 349 reposts listing "12 Agentic AI projects" every production system needs)

**Reply posted:**
> Project 1 catches format errors, not semantic ones. Pydantic validates the shape. The LLM can still emit valid JSON that passes schema and misses the task. Add a golden-case eval harness with a separate verifier, not just parse-error retries.

The reply adds a concrete production lesson the original thread missed: schema validation enforces format, not correctness, and a separate verifier/eval harness is what actually catches semantic failures. No screenshot attached — the reply is pure technical addition, no public-source evidence to capture.
