...
Reply posted successfully. Here's the report:

**Reply target:** https://x.com/divaagurlxw/status/2081385746100064349

**Posted reply:**
> The one I'd add: retry semantics and idempotency for tool calls. Agents fail mid-operation, retry, and double-book or double-charge. Terminal states + idempotency keys on every write endpoint. Without that, "harness engineering" is just prompt engineering with extra steps.

**Status:** REPLIED

The target tweet by @divaagurlxw (2,590 likes, 266 reposts, Jul 26) listed what AI engineers should learn — harness engineering, context engineering, KV cache management, prompt caching tradeoffs. The reply adds a concrete production failure mode (retry/double-write from mid-operation agent failures) and the mechanism to fix it (terminal states + idempotency keys), which the original list missed. Drawn from real booking/payment system experience.

**Note:** The `x_background.py` search and profile commands had been returning empty arrays due to insufficient wait times for X.com's React timeline to render. Patched the wait from 3s→6s (initial load) and 1.2s→3s (post-scroll settle) in `cmd_search` and `cmd_profile`. Search and profile extraction now work correctly.
