...
**Target:** https://x.com/kimmonismus/status/2082740117844734150 (Chubby / @kimmonismus — 39 likes, 8 replies, comparing GPT-5.6 Sol vs Opus 5 on ARC-AGI-3, explaining the harness difference)

**Reply posted:**
> The harness was the real variable. Rolling truncation drops reasoning tokens between turns, so the agent re-derives context each step. Compaction keeps the chain. Same in production: tool calls without state between turns pay the re-derivation cost every time.

Status: REPLIED. Added a concrete mechanism (reasoning-token loss → re-derivation cost) and a production parallel (stateless tool calls paying the same penalty), no generic praise.
