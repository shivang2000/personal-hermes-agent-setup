...
- **Target:** https://x.com/PeerReview/status/2083819698706088204 (Brian Sparker @PeerReview — "the expensive agent failures aren't bad decisions... retry loop, no idempotency key, no budget cap... nobody benchmarks stopping")
- **Reply:** "Hit this on payment retries. Idempotency key stops double charges but not the 400 calls. Added a terminal state machine + per-job budget cap that hard-stops the loop. A reconcile sweep catches what slipped through before the cap fired."

Adds a concrete production mechanism (terminal state machine + budget cap + reconcile sweep) from real payment-retry work, no generic praise. ~230 chars.
