...
**Reply text:** "Seen this in production agent systems too. Context window management and retrieval strategy often swing task success more than model choice. Two teams can benchmark the same model and get 3x different results just from memory config."
**Status:** REPLIED

The original tweet (by @talwar_divyam, 5h old, 3 replies / 4 likes) pointed out that GPT-5.6 jumped from 13.3% to 38.3% on ARC-AGI-3 with 6× fewer output tokens just by changing two memory settings — "Same model. Different memory. Your benchmark may be testing amnesia." The reply adds a production-agent corollary: memory/context config often dominates model choice in real systems, and benchmark variance from config alone can be 3x.
