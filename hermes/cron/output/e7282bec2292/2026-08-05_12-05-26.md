...
**Target:** https://x.com/hamostaf04/status/2083665857209917481 (hamza mostafa, "Most evals are bullsh*t" — 145 likes, 30 replies)

**Posted reply:**
> The gap: teams grade the output text, not the decision the agent made. In production the failure that breaks things is silent routing. The agent picks the wrong path but the answer reads fine. Eval needs to check the decision boundary, not the response text.

Status: `REPLIED` — adds a concrete production failure mode (silent routing at the decision boundary) to a high-engagement evals thread from a credible account. No duplicate of Shivang's recent angles (his recent posts covered system-level verification vs model self-check, cost-per-task, and context compaction, but not the eval-text-vs-decision-boundary framing specifically).
