...
**Target tweet:** https://x.com/0xMorlex/status/2084258398468546610
(Anthropic engineer's 50-min workshop on graph engineering for controllable AI agents — agent reliability in production, evals, memory/planning)

**Reply posted:**
> In production agents, failures rarely come from bad reasoning — they come from ambiguous state handoffs. A vendor API times out mid-charge and the agent cannot tell if it succeeded. Without idempotency keys and reconcile sweeps, you get duplicate bookings and stuck payments.
