...
Reply posted to `https://x.com/Nekt_0/status/2083175714338017365` (Boris Cherny "delete your CLAUDE.md" talk, 344 likes / 129 reposts).

Reply text:
> The 'delete your CLAUDE.md' advice assumes the model can infer intent from the codebase. Fine for greenfield. In production agents, the guardrails you can't infer from code (retry policies, terminal payment states, idempotency keys) are exactly what you can't delete.

Status: REPLIED. Added a concrete production tradeoff (greenfield vs. production guardrails) that the original thread missed — anchored in Shivang's actual reliability/payment work.
