...
**Target:** https://x.com/VivekIntel/status/2083934892283371702 (ADR: Agentic AI Detection & Response Framework — 299 likes, 48 reposts)

**Reply:** https://x.com/shivangchheda22/status/2083934892283371702 (reply)
> Detection is half the problem. The quieter half is cost: an agent retrying a flaky tool call burns API budget without tripping any security rule. Same pattern as payments without terminal states. The retry looks like progress but it's just burning money.

Added a concrete production lesson the original post missed: security detection frameworks focus on *what* the agent did, but the cost dimension (unbounded retries burning budget silently) is the failure mode that won't show up in any detection rule. Linked it to Shivang's real payment-systems experience with terminal states — a genuine builder insight, not generic praise. 254 characters, humanized.
