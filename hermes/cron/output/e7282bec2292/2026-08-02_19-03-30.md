...
Target: https://x.com/yacinelearning/status/2082536499355033996 (232 likes, eval landscape walkthrough)

Reply: "Two different evals: benchmarks measure the model, production evals measure the system around it. Terminal states, idempotent retries, cost-cap degradation. A model can score well on ARC-AGI and still loop on a stuck payment. Harness evals catch that."

Status: REPLIED → https://x.com/shivangchheda22/status/2082784082459775045 (thread)

Humanizer note: removed AI-isms ("crucial", em dashes, rule-of-three, negative parallelism), kept Shivang's direct builder voice with concrete mechanisms (terminal states, idempotent retries, cost-cap degradation) the original thread doesn't cover. No praise or engagement farming — the reply adds a real mechanism distinction (benchmark evals vs harness/system evals) anchored to Shivang's production travel-agent work.
