...
**Target:** https://x.com/shubh6200/status/2083074517510029474
**Reply:** "Call succeeds, but the response times out before the agent sees it. Agent retries, side effect runs twice. Idempotency keys on every write endpoint fix this. Not better prompts or retry budgets."

The original post (56 likes, 7 reposts) argued that messy APIs break agent execution via schema validation failures. The reply adds a concrete production failure mode the original doesn't cover: the timeout-then-retry double-execution path that causes duplicate side effects, and the actual fix (idempotency keys on write endpoints) rather than prompt-level or retry-budget workarounds.
