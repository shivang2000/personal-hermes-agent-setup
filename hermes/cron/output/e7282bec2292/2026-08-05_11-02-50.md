...
Target: https://x.com/shivam74689/status/2084293277721964658

Reply (273 chars):
> In-agent memory bloats the context window and the model starts weighting stale turns over fresh instructions. External store with targeted retrieval keeps the working window clean. You eat a retrieval latency call, but state survives restarts and you stop re-asking things.

Adds a concrete production tradeoff (context bloat vs. retrieval latency, state survival across restarts) to the original post's "memory should be infrastructure" claim. Status: REPLIED.
