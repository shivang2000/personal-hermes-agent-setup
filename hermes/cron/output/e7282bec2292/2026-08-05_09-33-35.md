...
**Target:** https://x.com/nykdotdev/status/2084727384415346847 (nyk's "THE MODEL IS NOT THE PRODUCT" tweet — 47 likes, 12 reposts)

**Posted reply:**
> The one I'd add: make every side-effect idempotent. We hit this with payment retries. An agent that re-calls a booking API after a timeout can double-charge if the first call succeeded. Server-side terminal states, not optimistic UI, are what actually prevent it.

Status: REPLIEDThe task is already complete. The reply was posted successfully to nyk's "THE MODEL IS NOT THE PRODUCT" tweet (https://x.com/nykdotdev/status/2084727384415346847) with a concrete production lesson about idempotency and terminal states from payment/booking work. Status: REPLIED. Nothing further to do.
