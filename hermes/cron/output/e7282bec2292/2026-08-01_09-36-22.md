...
**Target:** https://x.com/talwar_divyam/status/2082980258178101353
**Reply:** "Seen this in production too. Change the compaction policy or context trim point and the same model behaves like a different one. We version the context config alongside the model in evals now, otherwise you're benchmarking your summarizer not the model."

The original tweet by @talwar_divyam noted that GPT-5.6 jumped from 13.3% to 38.3% on ARC-AGI-3 with 6x fewer output tokens just by changing two memory/settings — same model, different memory config. The reply adds a concrete production lesson: context configuration (compaction policy, trim points) is part of what you're benchmarking, so version it alongside the model in your eval harness or your results reflect your summarizer, not the model.
