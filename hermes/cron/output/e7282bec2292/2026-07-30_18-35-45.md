...
**Target:** https://x.com/divaagurlxw/status/2081385746100064349 (diva, 2,590 likes, 266 reposts — high-engagement thread on what AI engineers should learn: harness engineering, context engineering, KV cache, prompt caching, prefill/decode latency)

**Reply text (274 chars):**
> Missing: idempotency keys so retried agent calls don't double-charge, terminal payment states so stuck txns can't hang, and an authz boundary between agent output and side effects. Harness keeps the agent safe; the reliability layer keeps it correct when the agent is wrong.

**Why this reply:** The original tweet lists model/infra-level skills (harness engineering, context engineering, KV cache, prompt caching). It's missing the system-level reliability layer — idempotency, terminal payment states, and authz boundaries — which is the part that keeps a production agent correct when the model is wrong. This adds a concrete mechanism and tradeoff from real production work, not generic praise.
