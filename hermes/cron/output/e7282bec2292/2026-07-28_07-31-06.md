...
The isolated browser profile at `~/.hermes/browser-profiles/x-automation` reports `logged_in: false`. The reply scanner cannot post without auth.

**Action needed:** The isolated X profile needs its one-time manual login. Run:

```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

Log in to X in the separate Chromium window that opens, return to the terminal, and press Enter. After that, the scanner can resume posting replies headlessly.
