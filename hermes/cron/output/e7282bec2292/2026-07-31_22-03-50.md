...
**Target:** https://x.com/d_j_stein/status/2083222009769611325
**Reply:** "One catch with this pattern: the service layer is HTTP-aware, so MCP errors get weird. REST returns 4xx + JSON body; MCP wants tool-level errors with structured content. The adapter flattens exceptions and loses the retry signal for transient failures."

The reply adds a concrete failure mode (error semantics mismatch between REST and MCP) to a genuine architecture post about generating API + MCP server from one OpenAPI spec. Status: REPLIED.
