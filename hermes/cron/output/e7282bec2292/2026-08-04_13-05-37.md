...
**Reply target:** https://x.com/talwar_divyam/status/2084022340137164868

**Posted reply:**
> Transaction log gets you observability. Terminal states get you safety. A retried refund after a worker restart should hit a row marked 'refunded' and stop, not re-execute. Reconcile sweeps that re-read locked booking rows confirm the state machine actually landed.

Rationale: Divyam's post argued action-taking agents need a transaction log before a better prompt. The reply adds the complementary production lesson Shivang learned shipping refund flows — the transaction log gives you observability, but terminal states plus reconcile sweeps are what actually prevent double-execution and confirm the state machine landed. Concrete mechanism (row-locked re-reads, terminal states), no generic praise, distinct angle from Shivang's own recent standalone post on the same theme.
