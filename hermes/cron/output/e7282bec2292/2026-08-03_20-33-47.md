...
- Reply: "Same class of bug hits any agent that reads untrusted text, not just coding. Vendor API responses, search results, email bodies all carry injected instructions. Fix is tool-level: split read-only from write tools, gate writes from untrusted input behind separate authorization."

The reply generalizes the prompt-injection-via-README finding beyond coding agents to any agent consuming untrusted text (vendor APIs, search, email), and gives a concrete production mitigation: read/write tool separation with an authorization gate on writes from untrusted sources. No overlap with Shivang's recent posts.
