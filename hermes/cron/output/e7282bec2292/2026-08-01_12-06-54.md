...
**Target:** https://x.com/Nekt_0/status/2083175714338017365 (Boris Cherny's YC talk on deleting 80% of Claude Code's system prompt — 388 likes, 132 reposts)

**Reply:** "The deletion works because prompt scaffolding was compensating for weaker model judgment. Same instructions on a stronger model over-constrain. System prompts are model-dependent. Re-audit after every model bump, not on a fixed schedule."

Status: REPLIED. The reply adds a concrete mechanism (over-constraining stronger models with stale instructions) and a production lesson (re-audit prompts on model upgrades, not calendar schedules) rather than generic praise.
