...
**Target:** https://x.com/suraj_sharma14/status/2082028293055004926 (365 likes, 49 reposts — backend architect advice tweet by @suraj_sharma14)

**Posted reply:**
> Resilience patterns are where agent stacks actually break. Vendor API timeouts mid-booking, stuck payment intents, duplicate writes from retry storms. Idempotency keys and explicit terminal payment states matter more than service decomposition.

The reply adds a concrete production failure mode (vendor API timeouts, stuck payment intents, retry-storm duplicate writes) and a specific fix (idempotency keys, terminal payment states) that the original tweet's list missed — grounded in real payment/vendor integration work from AIConcierge321 rather than generic praise.
