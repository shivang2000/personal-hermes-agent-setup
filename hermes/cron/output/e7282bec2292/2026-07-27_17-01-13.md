# Cron Job: Reply-engagement scanner for AI accounts (30m)

**Job ID:** e7282bec2292
**Run Time:** 2026-07-27 17:01:12
**Schedule:** */30 7-22 * * *
...
You are Shivang's Twitter reply-engagement scanner. Your job is to find high-engagement AI tweets from large accounts and reply with useful technical additions that put Shivang's handle in front of their audience.

Load the `ai-tech-twitter-system` and `xurl` skills first.

## Active window
This job runs every 30 minutes during 12:45 PM IST – 3:45 AM IST (7:00 UTC – 22:00 UTC). If outside this window, return [SILENT].

## Scanner workflow

1. Search for recent high-engagement AI tweets:
   `xurl search "AI OR LLM OR agent OR Claude OR GPT OR benchmark OR open source" -n 20`

2. Filter for tweets that:
   - Have 100+ likes (check `public_metrics.like_count` in the JSON)
   - Are from accounts with 5k+ followers (use `xurl user @handle` to verify if unsure)
   - Are from the last 3 hours
   - Are original posts (not retweets)

3. Check Shivang's recent replies to avoid duplicates:
   `xurl search "from:shivangchheda22" -n 10`

4. For each candidate (max 2-3 per cycle), draft a **useful technical addition**:
   - Add a mechanism, tradeoff, or production lesson the original tweet missed
   - Share a relevant data point or benchmark
   - Correct a technical inaccuracy (politely)
   - Ask a sharp technical question that shows depth
   - Share a "we hit this in production when..." style lesson from Shivang's AIConcierge321 experience

5. Apply the humanizer checklist before posting.

6. Post with `xurl reply POST_ID "Reply text"`.

7. Report each reply to #tweets-automation with the original tweet URL and the reply text.
...
## Quota management
- Limit to 2-3 replies per scan cycle.
- If no good candidates found, return [SILENT].
- Each `xurl search` + `xurl reply` is an API call — be efficient.

## Safety
- Never read or inspect `~/.xurl`.
- Never use `--verbose`.
- Apply the humanizer skill before every reply.
- Keep replies under 280 chars.
- Do not reply to the same tweet more than once.

## Response

**Reply-Engagement Scanner Report — 2026-07-27 11:30 UTC (5:00 PM IST)**

**Status: [SILENT] — X API credits depleted, no replies possible this cycle.**

---

**What happened:**
- Time check: 11:30 UTC = 5:00 PM IST → within the active window (12:45 PM IST – 3:45 AM IST). ✅
- Auth check: `xurl auth status` shows `my-app` with `oauth2: shivangchheda22` properly configured. ✅
- `xurl whoami` works fine (cheap endpoint still passes). ✅
- `xurl search "AI OR LLM OR agent OR Claude OR GPT OR benchmark OR open source" -n 20` → **HTTP 402: `credits depleted`** ❌
- `xurl search "from:shivangchheda22" -n 10` (duplicate check) → **same 402** ❌

This is the **X API tier-quota failure pattern** documented in the skill. The `whoami` endpoint still works because it's cheap, but all search (read) and post (write) endpoints are blocked by a $0 credit balance or exhausted monthly cap on the X Developer Portal billing account.

**Fleet health:**
All four X-facing cron jobs show `active` with `last_status: ok` — they haven't paused yet because the 402 from `xurl search` is a credits issue, not a 429 rate-limit. The scheduler doesn't auto-pause on 402 the way it does on persistent 429s. However, every scheduled run that hits search/post/reply is burning cycles for nothing until credits are restored.

**Action needed from Shivang:**
Top up X API credits at https://developer.x.com/en/portal/dashboard → Billing (minimum $5). Once credits are restored, the reply scanner, breaking-news trigger, opportunity scan, and daily autopost will all resume working on their next scheduled runs without any manual intervention needed — the jobs are still active and will pick up cleanly.

No replies posted this cycle. No quota was wasted on write attempts since reads failed first.
