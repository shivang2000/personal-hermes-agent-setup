...
**Target:** https://x.com/leopardracer/status/2084685250077270125 (47 likes, 8 replies — active conversation about Not Diamond's cache-aware coding-agent router)

**Reply:** "Routing to a cheaper model can break prompt cache locality. Switch mid-session and you invalidate the KV cache, so the next premium-model call pays full input cost. The savings get eaten by cache misses. Cache-aware routing isn't optional."

Added a concrete mechanism (KV cache invalidation on model switch) and a production cost tradeoff that the original tweet implied but didn't spell out — complements Shivang's recent cache-reads post without duplicating it. Status: REPLIED.
