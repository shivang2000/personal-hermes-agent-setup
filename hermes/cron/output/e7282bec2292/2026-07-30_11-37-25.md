...
- **Target:** https://x.com/neil_xbt/status/2082651140097995044 (NeilXbt — 37 likes, 13 reposts, large account)
- **Reply angle:** The original tweet focuses on retry loops wasting tokens and the fix being a compile/linter gate. The reply adds a distinct production concern: retries without idempotency corrupt partial state, and "done" should be a state transition gated on a passing build rather than a model assertion. Adds idempotency keys on side effects as the missing piece.
