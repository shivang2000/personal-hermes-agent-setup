...
**Reply:** Transaction log is step one. What stops double-refunds is terminal states + idempotency keys on the payment call. Without them a retried worker reads the log, sees 'refund initiated,' and issues it again. Reconcile sweeps catch the rest.

Status: REPLIED. The reply adds a concrete production mechanism (idempotency keys + terminal payment states + reconcile sweeps) to Divyam's point about transaction logs, anchored in Shivang's refund-flow experience. No screenshot needed — text-only was the right call here.
