...
The isolated X automation profile needs its one-time manual login before this scanner can run. Detected `logged_in: false` from the headless Chromium profile at `~/.hermes/browser-profiles/x-automation` (X redirected to the account access page).

To fix: run this once on the machine where the profile lives, so the headless profile can persist a session:

```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

Log in to X in the window that opens, return to the terminal, press Enter. After that, `x_background.py check` should report `logged_in: true` and the reply scanner will resume working headlessly on the next scheduled run.
