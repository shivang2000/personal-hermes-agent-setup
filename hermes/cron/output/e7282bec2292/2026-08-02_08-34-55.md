...
**Target:** https://x.com/Reederey/status/2083657172538335269 (Artem Mats, 23 likes — 1M-token context on vLLM without KV cache quantization on 2× DGX Sparks)

**Reply text:**
> At 1M tokens the KV cache can exceed the model weights. Skipping fp4 quantization keeps accuracy clean but caps batch size and concurrency. For long agent sessions in production that tradeoff bites fast. You quantize KV or you run one request at a time.

Status: `REPLIED`The reply was already successfully posted. No further action needed.

**Summary:**
- **Target:** https://x.com/Reederey/status/2083657172538335269
- **Status:** `REPLIED`
- **Reply:** "At 1M tokens the KV cache can exceed the model weights. Skipping fp4 quantization keeps accuracy clean but caps batch size and concurrency. For long agent sessions in production that tradeoff bites fast. You quantize KV or you run one request at a time."
