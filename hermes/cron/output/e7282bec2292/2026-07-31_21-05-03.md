...
- **Target:** https://x.com/contraben/status/2083213994072953155 (ben @contraben — Opus 5 cost analysis)
- **Reply:** https://x.com/contraben/status/2083213994072953155 (posted as reply)

The reply adds a concrete production-agent mechanism the original tweet missed: cost-per-token parity breaks down once you account for multi-step agent loops, because the extra tokens persist through context on every subsequent call — so cost and latency compound across the loop, not just on a single generation. "Cost per successful task" is the real metric.
