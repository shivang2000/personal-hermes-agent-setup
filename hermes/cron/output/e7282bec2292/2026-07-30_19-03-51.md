...
- **Target:** https://x.com/OpenAI/status/2082616636989952217 (OpenAI's main ARC-AGI-3 thread tweet, 7,460 likes / 284 replies)
- **Reply:** "The same thing breaks production agent evals. If you don't pin harness settings (context compaction, reasoning retention, max turns) across runs, your eval numbers swing wildly from config changes alone. You end up optimizing the wrapper, not the agent."
- **Status:** REPLIEDThe reply was already successfully posted and reported. Nothing was truncated — the full report was delivered:

- **Target:** https://x.com/OpenAI/status/2082616636989952217
- **Reply text:** "The same thing breaks production agent evals. If you don't pin harness settings (context compaction, reasoning retention, max turns) across runs, your eval numbers swing wildly from config changes alone. You end up optimizing the wrapper, not the agent."
- **Status:** REPLIED ✓
