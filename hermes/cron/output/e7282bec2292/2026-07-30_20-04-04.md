...
- **Target:** https://x.com/kimmonismus/status/2082740117844734150 (Chubby @kimmonismus, 356 likes, 48 replies — explaining GPT-5.6 Sol vs Opus 5 on ARC-AGI-3)
- **Reply:** "38.3% vs 30.2% requires Retained Reasoning + Compaction enabled. Standard ARC-AGI-3 env drops Sol to 7.8%. The gap between 38.3% and 7.8% is the harness, not the model."

The reply adds a concrete data point the original thread was missing: the 38.3% score depends on non-standard API settings (Retained Reasoning + Compaction), and Sol drops to 7.8% in the standard environment. The punchline reframes the benchmark gap as a harness difference, not a model capability difference — a production-relevant distinction for anyone choosing between the two.
