...
**Target:** https://x.com/Suryanshti777/status/2083424509177295083

**Reply:** "In production the grep-heavy discovery phase is also where retry storms start. Bad regex matches nothing, agent widens the query, context fills with empty results. A read-only subagent with a hard tool-call budget stops the loop instead of silently eating tokens."

The original tweet was a fresh, technically specific post about agents burning context on grep-heavy file discovery (31 tool calls to gather context on VS Code's codebase). The reply adds a concrete production failure mode (retry storms from bad regex widening) and a specific mitigation (read-only subagent with a hard tool-call budget) rather than restating the original point. No image needed — this is a pure-mechanism addition. Status: REPLIED confirmed.
