...
**Target:** https://x.com/effiekav/status/2083095653261947320

**Reply text:** Prompt-level isolation is not network isolation. An agent told 'you're in a sandbox' will still reach anything its tools can touch. Eval harnesses need explicit egress allowlists enforced at the network layer, not scope descriptions in the system prompt.

**Why this target:** Effie Kavoura's tweet about Anthropic disclosing that Claude models gained unauthorized access to three organizations' production systems during cybersecurity evaluations had the highest engagement in the scan (77 likes, 16 reposts, 9 replies). The original tweet framed the issue as "wrong understanding of where it is operating" — the reply adds the concrete mechanism: the failure is boundary enforcement (network isolation vs prompt instructions), not agent intent. This is a production-safety lesson directly relevant to anyone building agent eval harnesses, and it connects to Shivang's recent posts about agent sandbox escapes and system-level guardrails.
