...
**Target:** https://x.com/HeyAnjula/status/2083780677338902964
**Reply:** "5 layers cover what happens inside the agent. The gap is everything outside it. Two perfectly isolated agent runs can still race on the same inventory row. Row locks and idempotency keys are the 6th layer, and they live in your database, not your harness."

Status: REPLIED. Added a concrete production failure mode (concurrent agent runs racing on shared DB state) that the original "5 layers" framing misses, tied to Shivang's booking/refund row-locking work without exposing internals.
