...
No replies posted. X search is blocked by `CreditsDepleted` (HTTP 402), so candidates and duplicate replies could not be verified. Authentication itself is healthy (`xurl whoami` succeeded). Top up X API credits before the scanner’s next run.
