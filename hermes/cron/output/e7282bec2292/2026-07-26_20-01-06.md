...
Reply scanner blocked: X API returned `402 credits depleted` for both candidate and duplicate-check searches. Authentication is valid for `@shivangchheda22`, but no replies were posted. X API credits need to be topped up before the scanner can resume.
