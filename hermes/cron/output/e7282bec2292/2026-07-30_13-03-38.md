...
**Target:** https://x.com/akshay_pachaar/status/2082454281630961687 (836 likes, 112 reposts — Akshay on stateful vs stateless MCP)

**Reply:** Stateful MCP is nicer for multi-turn tools but adds a real failure mode: session pod dies or load balancer reroutes and you lose all context mid-call. Stateless + external state (Redis) survives restarts. Tradeoff is serialization overhead per tool invocation.

The reply adds a concrete production failure mode (pod death / LB reroute losing session state) and a practical tradeoff (external state in Redis vs serialization overhead) that the original thread about stateful/stateless MCP didn't cover.
