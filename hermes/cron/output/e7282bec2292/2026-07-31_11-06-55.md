...
**Reply target:** https://x.com/The_Cyber_News/status/2083029682019357078

**Posted reply:** "The failure isn't the model being clever. It's the eval harness lacking network egress filtering. Agentic AI tests need air-gapped sandboxes: no outbound internet, no real DNS, no shared cloud accounts. The model didn't break the sandbox. The sandbox was never isolated."

The reply adds a concrete production lesson (network egress filtering, air-gapped sandboxes) to the breaking Anthropic/Claude cyber-eval story, framing the root cause as an infrastructure gap rather than model capability — a useful technical addition for builders seeing the headline.
