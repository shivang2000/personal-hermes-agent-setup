...
**Target:** https://x.com/RedHat_AI/status/2082835471940366657
**Reply:** Priority queuing helps for new requests. But once a batch token is mid-generation, preempting it means dropping the KV cache and recomputing later. Shorter prefill chunks and smaller batch windows on the interactive path is what actually fixes TTFT.

The Red Hat AI tweet described llm-d's flow control for the noisy-neighbor problem (batch summarization saturating a GPU shared with a live coding chatbot, causing TTFT spikes). The reply adds the concrete production nuance: priority queuing only governs admission for new requests, but mid-generation preemption forces KV-cache eviction and recomputation. The real TTFT fix for the interactive path is shorter prefill chunks and tighter batch windows so the scheduler doesn't have to preempt decode steps at all.
