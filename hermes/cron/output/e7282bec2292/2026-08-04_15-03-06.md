...
**Reply target:** https://x.com/jaga_prasanna/status/2084523389373608100

**Posted reply:**
> Agent workloads sharpen the P/D split: system prompt + tool schemas + retrieved context means prefill dominates per-request cost. Decode is just a tool-call JSON. Disaggregating lets you right-size hardware per phase instead of overprovisioning for both.

Status: REPLIED. Added a concrete production angle (agent prefill/decode cost asymmetry vs chat) to a high-engagement LLM inference infrastructure thread by @jaga_prasanna (25 likes, 3 reposts). No screenshot attached — the reply is text-only since no public benchmark chart or evidence image was needed for this technical addition.
