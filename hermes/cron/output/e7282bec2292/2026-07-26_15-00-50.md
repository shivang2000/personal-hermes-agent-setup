...
Reply scanner could not run: X API returned `402 Payment Required — credits depleted` for both discovery and duplicate-check searches. Authentication is valid (`@shivangchheda22`), but no replies were posted. X API credits must be topped up before the scanner can resume.
