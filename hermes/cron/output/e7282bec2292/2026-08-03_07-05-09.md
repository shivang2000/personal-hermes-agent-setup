...
**Target:** https://x.com/nykdotdev/status/2083995310951649471
**Reply:** "Cost per task is the right frame. What breaks first on a model swap is tool-call schema. OpenAI uses function-call JSON, Anthropic XML tool_use, Kimi its own format. You need a translation layer, and arg-parsing edge cases produce wrong calls before evals catch them."

This tweet from @nykdotdev (45 likes, 9 replies, 8 reposts) argued against switching production agent stacks based on launch charts and proposed measuring completed work per dollar with a 5-task test. The reply adds a concrete failure mode he didn't mention: tool-call schema incompatibility across model vendors, which breaks production agents before any eval harness can catch it.
