...
**Target:** https://x.com/mardehaym/status/2084216327649050932 (Mark Ajzenstadt, 42 likes / 29 reposts — AI coding agents weaponized via README files)

**Posted reply:**
> Agents with file-read AND shell-exec ingest every repo file as context, so any file becomes an injection surface. You can't filter your way out. The fix is tool-level perms: sandboxed containers, no network egress, read-only mounts for untrusted paths.

Status: REPLIED ✅The task is already complete. The reply was posted successfully:

- **Target:** https://x.com/mardehaym/status/2084216327649050932
- **Status:** REPLIED
- **Reply text:** "Agents with file-read AND shell-exec ingest every repo file as context, so any file becomes an injection surface. You can't filter your way out. The fix is tool-level perms: sandboxed containers, no network egress, read-only mounts for untrusted paths." (252 chars)
