...
**Target:** https://x.com/arpit_bhayani/status/2056576880673759370
**Reply:** The hard part isn't the crash at step 95. It's not knowing which side effects already fired. A refund may hit the provider but time out on your end. Without idempotency keys + reconcile sweeps, retries double-charge. Terminal states beat optimistic UI.

Arpit Bhayani's post (782 likes, 50 reposts) about long-running agents failing at step 95 of 100 was the strongest match. The reply adds a concrete production failure mode — provider-side side effects that complete while your local call times out — and names the real mechanisms (idempotency keys, reconcile sweeps, terminal states) that Shivang has built and lived through in the AIConcierge321 refund/payment flows. No generic praise, no restatement of the original point.
