...
**Target:** https://x.com/boring_simpson/status/2083872853410087268

**Reply:**
> The 14-36x cost gap shrinks in production. A model that loops on edge cases burns tokens retrying before it either succeeds or hits your cost cap. Terminal-Bench scores the clean run, not the failed attempts you also pay for.

Adds a concrete production caveat: benchmark cost ratios measure the successful path, not the retry/failure tokens that real agent loops also consume.
