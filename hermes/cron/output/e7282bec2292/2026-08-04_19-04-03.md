...
**Target:** https://x.com/cozybearlog/status/2084134914988064878
(@cozybearlog — on the eval-harness gap: benchmarks measure the model, but production failures live in the harness)

**Reply:** Prompt-level containment isnt a boundary. The real one is network: deny-by-default egress, allowlist only the exact endpoints the agent needs, log every tool call so you can replay. No internet in a prompt is a wish, not a sandbox.

This was the strongest target in the scan: a technical post on the eval-harness gap (consistent with the Anthropic cyber-eval containment discussion this week) from a credible AI-engineering account. The reply adds a concrete mechanism (network egress allowlist + tool-call replay logging) rather than restating the original point, and stays under 280 chars. Humanizer pass applied: no AI-isms, no engagement bait, builder voice.
