...
The isolated X automation profile at `~/.hermes/browser-profiles/x-automation` reports `logged_in: false` (stuck on the login/access page). The reply scanner cannot run this cycle.

**Action needed:** the profile needs its one-time manual login:
...
After that completes, subsequent reply-scanner runs will proceed headlessly. `[SILENT]` for this cycle otherwise — no reply was posted.
