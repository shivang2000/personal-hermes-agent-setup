...
**Target:** https://x.com/svpino/status/2084680605623828530
**Reply:** "Cache state is the hidden cost. A retry invalidates prompt cache, so the next turn pays full input price instead of 10x cheaper cache reads. One bad routing decision cascades into a cache-cold run that can cost more than just picking the expensive model upfront."

Status: REPLIED. Added a concrete mechanism (retry → cache invalidation → full input price) to Santiago's agent cost optimization post, directly extending his "cache state" and "retries" bullets with a production failure mode Shivang has real experience with.
