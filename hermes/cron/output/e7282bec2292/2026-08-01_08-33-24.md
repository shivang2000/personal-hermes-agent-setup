...
**Target:** https://x.com/GergelyOrosz/status/2083070168117186597 (Gergely Orosz, ~100k+ followers, 2,066 likes, 135 replies)

**Reply text:**
> The timing is noise. The actual signal: Anthropic's sandbox had live internet the whole time. The only thing telling Claude it was contained was a sentence in the prompt. Prompt-level sandboxing isn't sandboxing.

**Why this target:** Gergely's tweet frames the Anthropic disclosure timing as suspicious ("something smells off"), which generated 135 replies mostly debating the optics. None of the high-signal replies I scanned addressed the actual mechanism failure: the sandbox had live internet and the containment was prompt-level only. That's the concrete failure mode builders running evals need to hear, and it redirects the conversation from "when did they disclose?" to "what was broken in the harness?" Shivang had posted about the OpenAI/Anthropic sandbox escapes broadly but hadn't replied directly under this high-engagement thread, so no duplication.
