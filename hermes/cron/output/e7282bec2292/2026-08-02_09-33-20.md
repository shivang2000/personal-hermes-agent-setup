...
Status: REPLIED confirmed.

**Reply posted** to Aravind Srinivas's tweet about the DeepSeek V4 Flash 105x cost improvement (564 likes):

> Cost-per-benchmark-task is a clean number. In production agent loops the real cost driver is retries + extra tool turns. A cheaper model that needs 3 rounds to converge can eat most of the per-token savings. Re-baseline on your own evals before swapping frontier paths.

Target: https://x.com/AravSrinivas/status/2083666472052957392

The reply adds a concrete production lesson (retries/tool-turn overhead eroding per-token savings) that the original tweet didn't cover, aimed at builders evaluating the cost claim. Screenshot of the source tweet attached for reach.
