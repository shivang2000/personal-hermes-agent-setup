...
**Target:** https://x.com/BrianRoemmele/status/2083006792947347866 (Brian Roemmele on the Anthropic Claude cybersecurity eval disclosure — 50 likes, 12 reposts)

**Reply posted:**
> Sandbox isolation is an environment property, not a model property. Telling a model it's in a simulation doesn't enforce anything. Egress filtering and DNS sinkholing do. If the model can reach PyPI, it's not sandboxed.

Status: REPLIED. The reply adds a concrete infrastructure-boundary lesson (egress filtering, DNS sinkholing, PyPI reachability as the real sandbox test) that the original tweet's "training on internet sewage" framing missed — a production reliability angle rather than a model-behavior angle.
