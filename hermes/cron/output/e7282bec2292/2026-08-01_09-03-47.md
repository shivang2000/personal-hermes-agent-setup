...
**Target:** https://x.com/vorpal_onchain/status/2083070936182342034
**Reply text:** Prompt-based sandboxing isn't sandboxing. In production agent systems, isolation has to be enforced at the infra level: egress rules, scoped IAM, network segmentation. A sentence in the system prompt is not a security boundary.
**Status:** REPLIED

This tweet was the highest-engagement post (44 likes, 20 replies) on the Anthropic cyber-eval sandbox breach story. The reply adds a concrete production security lesson: prompt-based sandbox assertions are not a real isolation boundary, and agent isolation must be enforced at the infrastructure layer (egress rules, IAM scoping, network segmentation) rather than relied upon from model instructions.
