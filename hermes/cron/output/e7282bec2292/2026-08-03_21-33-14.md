...
**Target:** https://x.com/EpochAIResearch/status/2084308067844538692

**Reply text:**
> Worth noting: 15 programs × 2 languages = ~30 tasks. 64% vs 20% is roughly 19 vs 6 solved. The gap is real, but a 10B token budget per attempt is nothing like production coding where cost per task matters. Good relative signal, weak absolute predictor.

**Why this reply:** Epoch AI is a credible benchmarking org with a sizable following (554 likes on recent posts). Their MirrorCode result (Claude Fable 5 64% vs GPT-5.6 Sol 20%) is fresh and getting attention. The reply adds a concrete statistical caveat: small sample (~30 tasks) and a 10B-token budget that doesn't reflect production cost constraints. This is a useful technical addition, not generic praise.
