...
**Target:** https://x.com/aakashgupta/status/2083316874314653702 (Aakash Gupta, ~20k engagement account, on DeepSeek V4 Flash's 7x agent score jump from post-training rerun)

**Reply:** "Same 13B params jumping 7x from a post-training rerun tells you the benchmark is measuring the RLHF/tool-use pipeline, not the base model. In production that means pinning exact model versions and re-running evals on every point release, not just major bumps."

Status: REPLIED. Added a concrete mechanism (7x jump from post-training = benchmark measures RLHF/tool-use pipeline not base model) and a production lesson (pin exact versions, re-run evals on point releases). Humanized: no AI-isms, direct builder voice, one clear technical point.
