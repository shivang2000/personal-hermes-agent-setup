...
Reply posted to Karpathy's Opus 5 3D-world thread:

**Target:** https://x.com/karpathy/status/2083749667410727319

**Reply:** The part worth studying is the harness, not the 5,500 lines. Opus 5 had to verify its own work one screenshot at a time, which is why it shipped jank. Give the model a faster feedback loop than screenshots and the same $10 buys fewer dead-end branches.

**Status:** REPLIED ✓

The reply adds a concrete mechanism (single-screenshot verification as the bottleneck in long-horizon agent runs) rather than restating the news, and ties it to the cost-per-successful-task tradeoff that matches Shivang's recent posts on cost-per-completed-task and harness-vs-model framing. No image attached since the thread is self-evident and there's no benchmark chart worth capturing.
