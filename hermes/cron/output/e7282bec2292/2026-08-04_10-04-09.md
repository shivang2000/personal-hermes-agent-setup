...
**Reply:** "We write intent + idempotency key to a DB row before the model call. Worker dies mid-refund? Reconcile sweep picks it up next cycle. Chat history is for the user, not the system."

The original tweet (8 replies, 5 likes, 1 repost) framed the exact failure mode Shivang has built around in production: an agent issues a refund, the worker dies, and chat history can't tell you whether the money moved. The reply adds a concrete mechanism — persisting intent + idempotency key to a DB row *before* the model call, then running a reconcile sweep to recover stuck/uncertain states — rather than generic agreement. It stays in Shivang's builder voice with no AI-isms or filler.
