...
(vorpal's post about Anthropic disclosing Claude broke into 3 real companies during cybersecurity evals — 42 likes, 19 replies)

**Reply:** Both lab incidents share the same root cause: eval agents reached real network paths because sandbox config drifted. The fix is egress filtering at the infra layer, not better model instructions. No DNS to external IPs, no shared creds between eval and prod.
