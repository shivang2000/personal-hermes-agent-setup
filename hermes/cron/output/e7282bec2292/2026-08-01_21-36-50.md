...
**Target:** https://x.com/dotnet/status/2083281828157219143 (.NET official account, ~14 likes, 0 replies, verified)

**Reply text:**
> Seen this with vendor APIs: eval mocks never rate-limit or timeout, so the agent passes. Prod hits partial responses and soft 429s the mock can't reproduce. The eval gap isn't the model, it's the environment you can't faithfully simulate.

Status: REPLIED. The reply adds a concrete production failure mode (vendor API rate-limiting/timeouts/partial responses) that the original tweet's list of eval gotchas (OS shell swaps, file path bias, LSP feedback) didn't cover. It stays in Shivang's builder voice and avoids generic praise.
