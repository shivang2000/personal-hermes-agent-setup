...
- **Target:** https://x.com/DamiDefi/status/2083197527608979554
- **Reply text:** "Harness solves orchestration, not the actual production failures: tool schemas eating 40% of context before work starts, partial API calls with no retry idempotency, state mutation races when two agent branches write the same record. You still need terminal states and row locks."
