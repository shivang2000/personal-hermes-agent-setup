...
Reply posted to @leopardracer's tweet about when to wire an agent graph vs. a single loop.

**Target:** https://x.com/leopardracer/status/2084558434628755706

**Reply text:**
> Signal isn't parallelism, it's divergent failure semantics. Flights, hotels, eSIMs each need their own retry/idempotency rules. Supervisor/subagent lets each own its error contract. If steps share state and hit the same API, the graph is just complexity tax.

Status: REPLIED. The reply adds a concrete production signal from Shivang's AIConcierge321 multi-vendor agent work (retry/idempotency per vendor API as the actual trigger for graph vs. loop), grounded in real architecture rather than generic praise.
