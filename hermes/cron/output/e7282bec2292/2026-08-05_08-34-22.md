...
**Target:** https://x.com/seeconvm/status/2084648101839556721 (seeco's "7 defaults that separate a demo from a production agent" — 48 likes, 6 reposts, 7 replies)

**Reply text:**
> The one I'd add: idempotency keys on every side-effecting tool call. A retry after a timeout can double-charge or double-book. The agent thinks the first call failed, the vendor already processed it. Terminal payment states catch the cases idempotency misses.

Status: `REPLIED` confirmed. 259 chars, no screenshot needed (pure technical addition, no image evidence to attach). Shivang had not previously replied to this tweet.
