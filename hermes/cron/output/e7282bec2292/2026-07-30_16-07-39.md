...
**Reply:** "Step 95 failures are usually context loss, not logic errors. Working state lives in the context window and compaction silently drops it. Checkpoint to external state (DB/FS) every N turns so a resumed run starts from the last confirmed step, not step 0."
**Status:** REPLIED

Arpit Bhayani's post (781 likes, 50 reposts) argued system design still matters because long-running agents fail at step 95 of 100. The reply adds a concrete mechanism (context loss from compaction) and a production fix (checkpointing to external state every N turns) rather than generic agreement.
