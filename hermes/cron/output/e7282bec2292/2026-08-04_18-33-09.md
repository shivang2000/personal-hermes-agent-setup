...
**Target:** https://x.com/omarsar0/status/2084367708439949343
(elvis @omarsar0, "Model or Harness" — 41 agent failure modes paper, 297 likes / 47 reposts)

**Reply:** "The tool/harness edge is where most production agent failures live. Vendor returns 200 with partial flight data, schema drift between supplier versions, idempotency gaps on retry. No model upgrade fixes those. The fix is in the harness, not the prompt."

Status: REPLIED. No screenshot attached — text-only was the right call here since the addition is a production-lesson framing, not a benchmark or data claim needing visual evidence.
