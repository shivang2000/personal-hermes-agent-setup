...
**Reply:** "The one that bites in production: retrieval returns the right chunk but it's stale. Eval passes because the chunk matched the query, agent acts on outdated state. Add a freshness check - timestamp the chunk, flag anything older than your update SLA."

Added a concrete production failure mode (stale retrieval passing recall-based evals) and a specific fix (timestamp + SLA freshness flag) to the Vectorize agent-memory evals thread. No image needed — text-only reply, status `REPLIED` confirmed.
