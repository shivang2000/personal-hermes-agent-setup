...
**Target:** https://x.com/talwar_divyam/status/2084022340137164868

**Reply text:**
> Hit this exact pattern with payment refunds. Agent calls refund API, worker dies before logging, now money moved with no record. Idempotency keys on the call plus a durable log written BEFORE the API call is what saved us. Chat history is not a source of truth.

The original tweet by @talwar_divyam described an agent issuing a refund and then the worker dying before recording whether the money actually moved, making the point that action-taking agents need a transaction log before a better prompt. The reply adds a concrete production failure mode (payment refunds in a booking system) and the specific fix that worked: idempotency keys on the API call plus a durable log written before the call. This is a real mechanism from Shivang's AIConcierge321 work (refund flows, stuck-payment recovery, row-locked booking re-reads), shared as a builder lesson rather than self-promotion.
