...
**Target:** https://x.com/ArtificialAnlys/status/2083306229074739285
(Artificial Analysis announcement of DeepSeek V4 Flash open weights — 610 likes, 58 reposts, 21 replies)

**Posted reply:**
> 13B active out of 284B: per-token cost scales with active params, not total. That's the right cost shape for agent loops doing many cheap calls. But you still need 284B in VRAM to serve it. Cheap inference, expensive hosting.

Adds a concrete MoE cost tradeoff the original post skips: the cost asymmetry between per-token inference (scales with 13B active) and hosting/serving (needs the full 284B in memory). Builder-relevant for anyone considering self-hosting vs API.
