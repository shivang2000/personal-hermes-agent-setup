...
**Target:** https://x.com/Vladic_ETH/status/2083528172109226445
**Reply text:** "Root cause is the harness, not the model. A cyber eval with live internet egress is a broken isolation boundary, same class as a staging leak. Fix is network deny-by-default at the infra layer, not trusting the model to distinguish real from simulated targets."

The reply adds a concrete production-infrastructure angle (network isolation boundary, deny-by-default egress) to a high-engagement thread about the Anthropic Claude cyber-eval breach. It complements Shivang's existing sandbox-escape posts (OpenAI/HF incident on Jul 30) without duplicating them, and frames the failure as a harness/infra problem rather than rehashing the news.
