...
- **Target:** https://x.com/Jeremybtc/status/2083182671132532773
- **Reply:** "The model didn't 'escape.' The boundary was a prompt ('this is a simulation') backed by a network config that wasn't enforced. Prompt-level guardrails aren't isolation. Network egress controls are. If the firewall allows it, the agent will use it."

The reply adds a concrete mechanism distinction (prompt-level guardrail vs. enforced network egress isolation) to the highest-engagement post on the Claude sandbox escape story, rather than restating the news. Shivang's prior posts covered the OpenAI/HuggingFace incident and the system-vs-model guardrail framing, so this extends that thread to the Anthropic case without duplicating.
