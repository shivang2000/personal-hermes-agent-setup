...
**Target:** https://x.com/xabzxbt/status/2084141691196342319 (xabz's "Last Week in AI" post about MCP 2026-07-28 going stateless)

**Reply posted:** "Stateless MCP fixes deployment but shifts state to the client. No sessions means the caller re-establishes context on every tool call. In production you need explicit state serialization or mid-flight tool chains break between invocations."

Status: REPLIED. Added a concrete production tradeoff (state shifting to client, mid-flight tool chain breakage) to a thread about MCP's stateless protocol shift — a topic directly relevant to Shivang's termbridge MCP work.
