...
**Target:** https://x.com/talwar_divyam/status/2084146742417932766

**Posted reply:**
> Hit this in production. The subtler case: first call times out client-side but succeeds server-side. No idempotency key + terminal state lookup means the retry double-charges. We row-lock the booking, re-read payment status, and only proceed if still pending.

The original tweet was about how over-privileged creds get unauthorized access, but even tightly scoped agents can double-fire payments on retry — and the fix is an idempotency key plus a real state machine for committed state. The reply adds a concrete failure mode the original didn't cover (client timeout vs server success asymmetry) and ties it to Shivang's actual production experience with row-locked booking re-reads and terminal payment states. 259 chars, no image needed.
