...
**Reply:** "Seen this in booking flows. Each layer retrying independently turns one vendor timeout into a cascade that looks like a real outage. Fix: retry caps at the gateway not per hop, idempotency keys on every write, jittered backoff."

The target post by Divyam described how one failed AI tool call cascades across 5 layers (Model → Runner → Worker → Gateway → SDK) into 243 downstream requests via retry loops — a self-inflicted outage pattern. The reply adds a concrete production lesson from booking-flow work: per-hop retry budgets compound failures, so the fix is gateway-level retry caps, idempotency keys on writes, and jittered backoff, which directly counters the multiplication pattern Divyam identified.
