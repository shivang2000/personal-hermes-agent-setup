...
The isolated X automation profile (`~/.hermes/browser-profiles/x-automation`) is not logged in — `x_background.py check` returned `logged_in: false`. The reply scanner cannot run until the one-time manual login is completed:

```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

This requires Shivang to log in manually in the spawned browser window, then return to the terminal and press Enter. No reply was posted.
