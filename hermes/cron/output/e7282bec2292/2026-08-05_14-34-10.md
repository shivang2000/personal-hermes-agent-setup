...
**Target:** https://x.com/_A_Radwan_/status/2084922848641007742 (Ahmed Radwan, on LFM2.5 2.6B for unattended tool loops)

**Reply:** "The failure mode with a 2.6B model in an unattended tool loop: it hallucinates a call, gets back garbage, and keeps going. You need retry caps and idempotency keys on writes, or one bad turn cascades into a loop that burns your API budget before anyone notices."

Adds a concrete failure mode (hallucinated tool call → garbage response → runaway loop) and two production mitigations (retry caps, idempotency keys) the original tweet didn't mention. No image needed — no public evidence source for this claim.
