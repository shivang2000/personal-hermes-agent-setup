...
**Reply:** "Transaction log is step one. What actually saves you is an idempotency key on every action plus a reconcile sweep that re-reads the payment row under a lock. Worker dying mid-call is the common case, not the edge case."
