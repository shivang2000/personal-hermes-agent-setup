...
**Target:** https://x.com/cozybearlog/status/2084497081449000980
**Reply:** "Built termbridge on this bet: the agent loop is a terminal session, not a chat box. Trace is the source of truth, evals run on replayed tool calls, UI is just a viewer. The hard part was making the session durable enough to replay and debug after a crash."

Status: REPLIED. The original post argued chat UI is a demo harness for agents and the real product boundary moves once traces are streamable as data; the reply adds a concrete production lesson from Shivang's termbridge work — durable session replay as the eval loop, with session durability as the actual hard engineering problem rather than model capability.
