...
The isolated profile at `~/.hermes/browser-profiles/x-automation` needs its one-time manual login. Run this once to fix it:

```bash
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

Log into X in the window that opens, return to the terminal, and press Enter. After that, `check` should report `logged_in: true` and the scanner can run on its next scheduled fire.
