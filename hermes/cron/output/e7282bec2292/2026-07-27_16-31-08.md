...
X API credits are depleted (`HTTP 402: credits-depleted`). Search and reply endpoints are unavailable, so the reply-engagement scanner cannot function this cycle.

`xurl whoami` works (it's cheap) but `xurl search` returns 402, confirming the X API billing/credits issue is blocking all search and write operations.

**Action needed (outside agent session):** Top up X API credits in the Developer Console → Billing (min $5). The credit depletion is also likely affecting the other autopost/news cron jobs in the fleet — they should be checked and may show as `paused` in `hermes cron list --all`.
