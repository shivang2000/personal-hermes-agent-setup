...
python3 /Users/shivang/.hermes/scripts/x_background.py login
```
Log in manually in the separate window that opens, then return to the terminal and press Enter. Subsequent scheduled reply scans will run headless after that.

`logged_in: false` — skipping reply run (no `x_background.py reply` issued).
