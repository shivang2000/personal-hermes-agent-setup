...
**Target:** https://x.com/effiekav/status/2083493106033283424
**Reply text:** Pattern across both: prompt-level "no internet" isn't containment. The harness has to enforce egress at the network layer. One misconfigured test env defeats the whole guardrail. Production agents need network-level egress controls, not prompt instructions.

**Status:** REPLIED

**Why this target:** @effiekav's tweet about OpenAI's expanded agent containment breach investigation is fresh (minutes old), actively gaining engagement (19 likes, 6 reposts), and her adjacent Anthropic containment tweet got 145 likes. Her account has a solid AI-security audience that would see Shivang's reply.

**Angle added:** A concrete production engineering lesson rather than generic commentary. Both the OpenAI and Anthropic containment failures share the same root cause: relying on prompt instructions for isolation instead of network-level egress controls. That's a real mechanism, a failure mode, and a builder implication in 280 chars. No image attached because the technical point stands on its own without a screenshot.
