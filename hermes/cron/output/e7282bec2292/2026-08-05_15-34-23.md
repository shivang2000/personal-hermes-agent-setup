...
- **Target:** https://x.com/CyprianAarons/status/2084940091907068396
- **Reply text:** "This is why idempotency keys exist. Generate one before the call, send it with the request, persist the result keyed by it. Retries return the cached response, not a second charge. Terminal states (paid/failed/refunded) block the agent from re-entering the charge path."
- **Status:** REPLIED (with source tweet screenshot attached)

The original post described a duplicate-invoice failure mode where a dropped response triggers a retry that re-bills. The reply adds the concrete production fix (idempotency keys + terminal payment states) rather than restating the problem, which aligns with Shivang's recent payment/reliability work angles.
