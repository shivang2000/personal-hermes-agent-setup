...
X API credits are depleted — `xurl search` returns HTTP 402 `credits-depleted` for both the candidate search and Shivang's recent replies check. `xurl whoami` works (cheap endpoint) but all read/write search and reply endpoints are blocked.

**Status:** Reply-engagement scanner cannot run this cycle.

**Fleet health note:** The reply scanner job (`e7282bec2292`) shows `active` but its last run errored with `HTTP 403`. The X API credits need topping up in the Developer Console → Billing (min $5). This will also block the breaking-news trigger and opportunity scan jobs once they hit the same wall. Shivang needs to top up X API credits to restore the autopost fleet.
