...
X API credits are depleted — `whoami` works (it's cheap) but search/read endpoints return `credits-depleted` (HTTP 402). Without search, I cannot find high-engagement tweets to reply to.

[SILENT]

X API credits are depleted (`credits-depleted` / HTTP 402 on all search and read endpoints). `xurl whoami` still works but `xurl search` cannot retrieve candidate tweets. This scanner cannot operate without search access. Credits need to be topped up in the X Developer Console → Billing before the next cycle.
