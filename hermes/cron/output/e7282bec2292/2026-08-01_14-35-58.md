...
**Target:** https://x.com/Ibrahim__Arshad/status/2083100508643094915
**Reply posted:** "The part people miss: an eval sandbox is only as strong as its egress filter. "Test mode" is a config flag, not a network boundary. If the agent can resolve real DNS, it can reach a real target. Air-gap the network namespace. Prompt instructions are not a control."

The original tweet framed the OpenAI/Anthropic cyber-eval incidents by naming the harness, sandbox, judge, credentials, and network as the attack surface. The reply adds a concrete production lesson the original didn't make explicit: "test mode" is a software flag, not a network boundary, so any agent that can reach a real DNS resolver can reach a real target. The correct control is an air-gapped network namespace, not prompt-level instructions. This is a useful technical addition (mechanism + failure mode + fix) rather than generic agreement, and it puts Shivang's handle in front of an active technical conversation with 4 likes and 3 replies.
