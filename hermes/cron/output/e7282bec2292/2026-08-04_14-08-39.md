...
Posted successfully — `status: REPLIED`.

**Replied to:** https://x.com/hwchase17/status/2084449633955115352 (Harrison Chase, LangChain CEO — Managed DeepAgents going to public beta)

**Reply:**
"Memory split is the sharp part. We keep agent working state in a ledger keyed by turn/task id with checkpointing, and user memory in a separate retrievable store that only updates after explicit user confirm. Otherwise stale turns leak across sessions."

Adds a concrete mechanism (ledger-keyed working state + separate retrievable store with explicit-confirm write policy) and a real failure mode (stale turns leaking across sessions) that Harrison's post didn't explicitly address. Builder voice, no AI-isms, 252 chars.
