# Cron Job: Approved Post 2 - Multi-agent backend lesson

**Job ID:** 40dc67586c1b
**Run Time:** 2026-06-17 21:41:13
**Schedule:** once at 2026-06-17 21:40
...
Use the xurl skill. Verify `xurl auth status` and `xurl whoami` succeed, then post this exact main tweet text on X/Twitter with xurl:

Building a multi-agent travel backend made one thing obvious: the hard part isn’t routing prompts, it’s protecting state. Anonymous users, vendor-scoped memory, rate limits, and checkpointed recovery matter more than another polished agent demo.

After posting the main tweet, capture the returned tweet ID. Then add the following replies as a thread under the main post. Reply 1 should reply to the main tweet. Each later reply should reply to the previous reply so the thread stays ordered.

Reply 1 exact text:
If you want to build the same thing, don’t start with “agents.” Start with boundaries:

1. one supervisor that owns intent + handoff
2. one subagent/tool boundary per vendor/domain
3. one durable state store every agent must respect

Reply 2 exact text:
The state model matters more than the model prompt:

- anonymous session id before auth
- user id after login
- vendor-scoped memory, not one global blob
- booking/checkout state as explicit records
- every external call logged with idempotency keys

Reply 3 exact text:
For recovery, assume every vendor call can fail halfway.

Use checkpointed steps like: search → quote → reserve → pay → confirm. If the process restarts, the system should know exactly which step is terminal, retryable, or needs user action.
...
vendors = isolated tools
state machine = trust layer

That’s the difference between an agent demo and something users can actually book through.

After posting, return the main tweet ID/URL and reply IDs/URLs if available. If posting or any reply fails, report the exact error and which step failed. Do not modify any tweet/reply text.

## Response

Posted successfully with `xurl` after verifying both checks:

- `xurl auth status`: success; active app is `my-app` with OAuth2 user `shivangchheda22`
- `xurl whoami`: success; authenticated as `@shivangchheda22`

Thread details:

- Main tweet ID: `2067278819170582703`
  - URL: https://x.com/shivangchheda22/status/2067278819170582703

- Reply 1 ID: `2067278868852121609`
  - URL: https://x.com/shivangchheda22/status/2067278868852121609

- Reply 2 ID: `2067278871268147565`
  - URL: https://x.com/shivangchheda22/status/2067278871268147565

- Reply 3 ID: `2067278873344213467`
  - URL: https://x.com/shivangchheda22/status/2067278873344213467

- Reply 4 ID: `2067278875596624153`
  - URL: https://x.com/shivangchheda22/status/2067278875596624153
