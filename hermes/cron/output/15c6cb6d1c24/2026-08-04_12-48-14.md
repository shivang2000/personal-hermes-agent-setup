...
**Tweet text:**
When flight and hotel subagents share one state dict, vendor data leaks across responses. Fixed this with vendor-namespaced state per subagent and scoped context windows. Supervisor only sees merged summaries, not raw vendor fields. #AIAgents

**X URL:** https://x.com/shivangchheda22/status/2084539325354824107

**Source:** Proof-of-work from the AIConcierge321 multi-agent backend (`concierge-multiAgent-backend/CLAUDE.md`) — the LangGraph hierarchical agent system uses vendor-namespaced state to prevent data pollution between flight/hotel/train subagents.

**Image:** None (no public proof artifact suitable; the architecture is internal code, not a public page to screenshot).

**Why this post:** Shivang already posted 5 tweets today — 4 news-based (Qwen3.8-Max, DeepSeek V4-Flash, OpenAI Astra, SWE-Touch) and 1 builder lesson (refund terminal states). This adds a second builder/proof-of-work angle from the multi-agent architecture, showing concrete production judgment (state isolation, scoped context, supervisor abstraction) that recruiters and founders care about. 242 chars, one concrete mechanism (vendor-namespaced state), one hashtag.
