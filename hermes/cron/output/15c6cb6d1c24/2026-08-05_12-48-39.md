...
**Tweet text:**
> MCP standardizes the call shape, not the failure mode. A vendor that supports MCP still times out, returns partial data, drifts schema. In prod I keep a proxy layer: retry caps, idempotency keys, sanitized error mapping. Protocol does not save you when vendor flakes.

**URL:** https://x.com/shivangchheda22/status/2084901834444169506
**Status:** POSTED
**Image:** none (no clean public proof artifact for this angle; text-only, no filler)
**Source:** X search for "MCP" surfaced multiple tweets about MCP "support" at scale; angle is the production gap Shivang hits directly in AIConcierge321's proxy clients (Tripjack, Airalo, trains, transfers). Distinct from today's earlier posts (MoE sparsity, ternary quant, Liquid LFM2.5, Cloudflare tracing, harness-gap quote-tweet, Anthropic-Volta deal).
