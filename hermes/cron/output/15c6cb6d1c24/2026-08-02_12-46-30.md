...
- **Text:** Multi-currency booking trap: don't lock FX at booking time and the user sees one price, pays another. We freeze a snapshot: vendor + display amounts in integer minor units, locked rate, ISO 4217 exponent per currency. What they see is what they pay.
- **URL:** https://x.com/shivangchheda22/status/2083814138682458138
- **Source:** `/Users/shivang/dev/AIConcierge321/individual-platform/wt-urg4/app/services/money/lock.py` — `DisplayMoneySnapshot` currency-lock module
- **Image:** None (internal code, no public proof artifact available; posted text-only per screenshot policy)
- **Length:** 249 chars
- **Humanizer:** passed — no AI-isms, no em dashes, no significance inflation, direct builder voice, concrete mechanisms (integer minor units, ISO 4217 exponent, FX lock at booking time)
