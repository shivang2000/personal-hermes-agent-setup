...
Tweet not posted because the X API returned `402 credits depleted`. Authentication is valid; the account credit balance is the blocker.

Draft (234 characters):

> One CI lesson I relearned: an unpinned linter is a production dependency. A Ruff release turned a green deploy red without an app-code change. Pin the version, update it deliberately, and review the diff. Tooling drift is still drift.
