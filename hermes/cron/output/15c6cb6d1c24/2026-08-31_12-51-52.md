...
> Stripe doesn't guarantee webhook order. While hardening a refund flow, I had to make sure a late `pending` event couldn't overwrite `succeeded`. We now update under a row lock and reject regressions from terminal states.

https://x.com/shivangchheda22/status/2094324628407914751
