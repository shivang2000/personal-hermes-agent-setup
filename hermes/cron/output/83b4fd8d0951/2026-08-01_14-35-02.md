...
**Text:** DeepSeek V4 Flash 0731, MIT open weights. Same model size as preview, agent benchmarks jumped 2-7x: Cybergym 38.7->76.7, DeepSWE 7.3->54.4. Beats their V4-Pro on every agent eval with fewer activated params. Win is training, not scale. $0.14/1M input. #AIAgents

**URL:** https://x.com/shivangchheda22/status/2083479046437163181

**Source:** HuggingFace model card — deepseek-ai/DeepSeek-V4-Flash-0731 (MIT license, benchmark table confirmed: Terminal Bench 82.7, Cybergym 76.7, DeepSWE 54.4, $0.14/1M input pricing from Artificial Analysis Index v4.1)

**Image attached:** Yes — screenshot of the HuggingFace model card with the full benchmark comparison table (DeepSeek-V4-Flash-0731 vs preview vs V4-Pro vs GLM-5.2 vs Opus-4.8). 210KB, 1440×1000 RGB.

**Why this fired:** DeepSeek V4 Flash 0731 is a significant open-source model release (MIT weights, 304B params MoE) that beats their own larger V4-Pro preview on all 9 agent benchmarks while being cheaper. Released July 31, confirmed via HuggingFace model card and multiple X sources. No prior tweet from @shivangchheda22 on DeepSeek V4 Flash.
