...
> DeepSeek V4 Flash: $0.14/M input, $0.28/M output, 1M context. Cache hit drops to $0.0028/M. Claude Fable 5 is ~100x more expensive per token. Fine print: peak hours (Beijing 9-12, 14-18) will be 2x. Also ships an Anthropic API endpoint. #AIInfra

**X URL:** https://x.com/shivangchheda22/status/2084279051439722771

**Image attached:** Yes — screenshot of DeepSeek's official pricing page (api-docs.deepseek.com/quick_start/pricing), showing the model pricing table with deepseek-v4-flash and deepseek-v4-pro pricing, 1M context, and the peak-hours 2x note.

**Source:** DeepSeek API Docs — official pricing page, confirmed primary source. Corroborated by Reuters reporting and multiple X posts citing $0.14/M input and the 100x cost gap vs Claude Fable 5.

**Why it qualified as breaking:** Major API pricing change from a frontier-model provider — DeepSeek V4 Flash (0731) released with a 100x cost advantage over Anthropic's flagship, 1M context window, and an Anthropic API compatibility endpoint. Shivang's recent posts covered Qwen3.8-Max, AirLLM Kimi K3, OpenAI Claude Code plugin, and Astra Lean proofs, but not the DeepSeek pricing drop, so this fills a distinct angle on the same China-shipping-back-to-back story.
