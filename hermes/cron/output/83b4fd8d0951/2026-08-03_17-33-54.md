...
**Posted tweet:** https://x.com/shivangchheda22/status/2084248819169120558

**Text:** "Someone filed 50+ AI-hallucinated CVEs against SQLite. NVD rated them 9.8 Critical. The cited functions didn't exist in the target version. Line numbers pointed to comments. PoCs didn't crash under ASan. The CVE pipeline itself is the attack surface."

**Source:** JFrog Security Research article "SQLite Critical CVEs or LLM Slop?" (jfrog.com) — #1 on Hacker News, published 30 Jul 2026. Verified from primary source: GitHub repo `programmervuln/cveadvisory-` published 50+ CVEs; NVD/CISA ADP assigned up to 9.8 CVSS critical scores; JFrog verified 6 SQLite CVEs and found cited functions didn't exist in target versions, line numbers pointed to comments/allocations, PoC payloads didn't crash under AddressSanitizer, and GPTZero flagged advisories as AI-generated.

**Image attached:** Yes — screenshot of the JFrog article page with the CVE table.

**Event type:** AI safety/security finding (AI-hallucinated CVEs accepted into the NVD pipeline with critical scores).
