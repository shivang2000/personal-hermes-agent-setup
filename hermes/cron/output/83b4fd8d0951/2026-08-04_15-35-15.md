...
> Swiftlet streams MoE expert weights from SSD, keeping only the dense core in 4.3 GB RAM. 80B Qwen on a Mac, 35B on an iPhone. Only 3B params active per token so it writes big but recalls small. Decode is dispatch-bound, not IO-bound. Edge inference getting real. #LLM

**X URL:** https://x.com/shivangchheda22/status/2084581363798434134

**Image attached:** Yes — screenshot of the Swiftlet GitHub README showing the model table (80B Qwen in 4.3 GB RAM, 35B on iPhone at 7-11 tok/s decode)

**Source:** [github.com/leonickson1/Swiftlet](https://github.com/leonickson1/Swiftlet) — #1 on HN Show (181 points, 79 comments). Swift + Metal expert-streaming runtime for Qwen3 MoE models. Open source, Apache-2.0.

**Why this qualified as breaking:** A significant open-source launch — first known native run of a 35B+ model on an iPhone, with a novel architecture (streaming MoE expert weights from SSD instead of loading the full model into RAM). The 4.3 GB peak RAM for an 80B model is a genuine infra shift for edge inference.
