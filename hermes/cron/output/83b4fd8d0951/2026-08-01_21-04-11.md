...
Shivang has already posted on all four genuinely major events today:
1. DeepSeek V4 Flash 0731 Anthropic-compatible API (14:04)
2. GPT-5.6 Luna 80% price cut / Sol self-optimization (14:36)
3. Anthropic Claude cyber-eval sandbox escape (15:04)
4. OpenAI Astra Lean 4 math proofs (15:19)

The remaining search results are either re-statements of those four events, low-engagement commentary, or unconfirmed items (e.g. a White House AI eval-framework deadline claim from a single small account with no primary-source confirmation). Nothing else clears the "genuinely major release / benchmark / API change / safety finding / open-source launch" bar that hasn't already been posted in the last ~75 minutes.
