...
Post confirmed: `status: POSTED` with verified X URL.

---

**Breaking-news trigger fired** — GPT-5.6 pricing cut (80% on Luna, 20% on Terra), announced by OpenAI and trending on HN (#17, 489 points / 324 comments) and X.

**Posted text:**
> OpenAI cut GPT-5.6 Luna 80% to $0.20/$1.20 per M tokens. 6-7x cheaper than Gemini 3.6 Flash on similar coding benchmarks. At 10k agent tasks/day, token cost stopped being the bottleneck. Your harness and retry logic now cost more than the model. #AIInfra

**URL:** https://x.com/shivangchheda22/status/2083003587161129256
**Image attached:** No (OpenAI blog behind Cloudflare, AA chart screenshot quality unverified without vision — text-only was safer than a bad image)
**Source:** OpenAI blog (openai.com/index/advancing-the-price-performance-frontier-with-gpt-5-6), HN discussion, X search confirming $0.20/$1.20 vs Gemini 3.6 Flash $1.50/$7.50
**Duplicate check:** Shivang's recent tweets covered GPT-5.6 kernel optimization (11h ago) but not the pricing cut — distinct angle confirmed.
