...
**Tweet text:**
> Qwen3.8-Max: 2.4T params, 95B active, open weights next week. Frontier-scale open weights shift the moat downstream — who can serve it, fine-tune it, retrain it on their own hardware. The model is free; the GPU fleet to run 1.2TB of 4-bit weights is not.

**X URL:** https://x.com/shivangchheda22/status/2084679630393606374

**Source:** Multiple X posts from @bountyAIhunter (confirmed 4-bit = 1.2TB), @MaxITfinds (API pricing + open weights timing), @rosticz (95B active), Qwen blog at qwen.ai/blog?id=qwen3.8

**Image attached:** No — screenshot capture skipped (no single clean public chart available; model not yet on Artificial Analysis leaderboard)

**Event:** Alibaba Qwen3.8-Max release — 2.4T MoE model, frontier-scale open weights promised next week, API live at $2/$6 per M tokens. This qualifies as a major model release + significant open-source launch.
