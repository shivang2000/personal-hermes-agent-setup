...
**Posted tweet:**
> OpenAI's Astra produced 10 original math proofs with machine-checkable Lean 4 certificates on GitHub. Not a benchmark. The verification layer is `lake build All`. You don't trust the model, you run the checker. openai/ten-proofs. #AIAgents

**URL:** https://x.com/shivangchheda22/status/2084271613353304263

**Image attached:** Yes — screenshot of the `openai/ten-proofs` GitHub repo showing the Lean 4 formalizations and the 10 results list.

**Source:** Primary source confirmed at https://github.com/openai/ten-proofs — Lean 4 certificates for 10 open math/CS problems (sphere packing, non-sofic groups, Connes rigidity, permanent lower bounds, quantum parallel repetition, GapCVP, Ehrhart volume, multicolor Ramsey, extremal graph theory counterexamples). Independently checkable via `lake build All`. Commit by `balexeev-oai`, 2 days ago. Apache-2.0 licensed.

**Why this is breaking:** OpenAI's unreleased Astra model produced original mathematical research verified by a machine-checked proof system (Lean 4), not a benchmark score. This shifts the evaluation of AI math capability from "can the model score well" to "can the output be independently verified" — a real change in how AI math progress gets measured.
