...
**Breaking-news trigger fired — Seedance 2.5 (ByteDance)**

- **Posted text:** `ByteDance Seedance 2.5: 30s video in one pass, native audio, 4K, up to 50 reference assets. Sora 2 caps at 20s/1080p, Veo 3 at 8s. ByteDance bet on duration and multimodal reference. API soon via BytePlus ModelArk. #AIInfra`
- **X URL:** https://x.com/shivangchheda22/status/2083849215869083683
- **Length:** 223 chars
- **Image attached:** No — screenshot capture timed out on the ByteDance media-heavy blog page; posted text-only per protocol.
- **Source:** ByteDance official blog (seed.bytedance.com/en/blog/one-take-creation-flexible-referencing-introducing-seedance-2-5, dated 2026-07-31), corroborated by HN front page (334 pts) and multiple X posts from larger accounts.
- **Why it qualified:** Major new model release from ByteDance, 2x duration jump over previous gen, competitor-leader on single-pass video length. Not covered in Shivang's 4 earlier posts today (all LLM/agent-focused).
- **Humanizer check:** No AI-isms, builder voice, concrete mechanism + competitor caps + a tradeoff framing, no emoji/dashes/polish.
