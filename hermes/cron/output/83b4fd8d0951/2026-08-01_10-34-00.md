...
**Tweet text:**
> OpenAI cut GPT-5.6 Luna 80% to $0.20/$1.20 per M tokens, funded by a model that optimized its own serving stack. But cost-per-task on AA: DeepSeek V4 Pro $0.05, gpt-oss-120b $0.08, GPT-5.6 Sol $1.86. Token price cuts dont close the per-task gap for agents.

**X URL:** https://x.com/shivangchheda22/status/2083418393106055444

**Source:** Artificial Analysis Intelligence Index (artificialanalysis.ai) — Cost per Task chart; OpenAI pricing announcement via X search (multiple accounts confirming GPT-5.6 Luna 80% cut, Terra 20% cut).

**Image attached:** Yes — screenshot of the Artificial Analysis homepage showing the Intelligence Index, Speed, and Cost per Task charts with model names and dollar figures.

**Event:** OpenAI GPT-5.6 API price cut (Luna down 80%, Terra down 20%), announced July 31 2026, ~5 hours before this post. Confirmed against Artificial Analysis cost-per-task data showing the per-task gap between cheap token models (DeepSeek V4 Pro $0.05, gpt-oss-120b $0.08) and premium models (GPT-5.6 Sol $1.86) remains wide despite the token price reduction.

**Duplicate check:** No prior tweet from @shivangchheda22 covers the GPT-5.6 pricing event. His earlier tweets today covered agent sandbox escapes (Anthropic/Reuters) and DeepSeek V4-Flash open weights — different events.
