...
**Posted tweet:**
> OpenAI cut GPT-5.6 Luna 80% and Terra 20% three weeks post-launch. The mechanism: Sol rewrote production GPU kernels and speculative decoding inside Codex. A frontier model optimized its own inference stack. The price curve is dropping faster than agent usage scales.

**URL:** https://x.com/shivangchheda22/status/2083041039838072867

**Source:** OpenAI blog post "Advancing the price-performance frontier with GPT-5.6" (via HN front page, 526 points); corroborated by multiple X posts with exact pricing: Luna $1→$0.20/M input tokens, $6→$1.20/M output (80% cut); Terra $2.50→$2.00/$15→$12 (20% cut); Sol pricing unchanged. The self-optimization mechanism (Sol rewriting GPU kernels and speculative decoding inside Codex) confirmed by @aakashgupta and @MTSlive on X.

**Image:** Not attached. Text-only post. Image upload via `x_background.py post --image` encountered a composer clearing issue; the text-only post succeeded to avoid risking a duplicate. The Artificial Analysis models page screenshot was captured and optimized but could not be attached.

**Event classification:** Major API/pricing change from a primary AI provider — qualifies as breaking news. Shivang's recent tweets covered GPT-5.6 Sol's behavioral failure (Bottleneck Labs) but not the pricing/cost-optimization story. No duplicate.
