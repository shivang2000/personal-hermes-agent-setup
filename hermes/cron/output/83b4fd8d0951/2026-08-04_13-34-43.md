...
> Cloudflare on serving Kimi K2.6 + GLM 5.2: FP8 KV cache doubles in-memory context to 1.37M tokens. INT4 weights shrink GLM from 705GB to 421GB, decode +55% faster. Prefill stays FP8 (compute-bound), decode goes INT4 (bandwidth-bound). Same accuracy. #AIInfra

**URL:** https://x.com/shivangchheda22/status/2084551033410470365
**Image attached:** Yes (screenshot of Cloudflare's blog post on serving Kimi/GLM at scale)
**Source:** Cloudflare blog, "Smaller, faster, safer: running Kimi and GLM at scale" (Aug 3, 2026) — confirmed with benchmark tables showing FP8 KV cache vs BF16 concurrency numbers, INT4 vs FP8 weight compression results, and accuracy comparisons across GSM8K, ARC, MMLU, and MMLU-Pro.
**Why this qualified:** Major AI infra post from a primary source (Cloudflare) with concrete production numbers on serving large MoE models — FP8 KV cache doubling context capacity, INT4 weight compression reducing checkpoint size 40%, and the disaggregated prefill/decode strategy that lets them run the right precision for each phase. Strong fit for Shivang's infra/builder audience.
