...
- **Text:** Pokee-Isaac 28B: 10M-token context on one GPU, $0.15/$1.00 per 1M tokens. RULER 93.3 at full 10M length. GPT-5.6 Luna and Gemini 3.5 Flash Lite both overflow by 1M. First model that returns a usable score at every length up to 10M. #LLM #AIInfra
- **URL:** https://x.com/shivangchheda22/status/2084959026568397244
- **Source:** https://console.pokee.ai/model (Pokee AI technical report, Aug 3 2026)
- **Image attached:** Yes — benchmark comparison table from the model card showing RULER scores across context lengths and pricing for all baselines.
- **Character count:** 245 (under the 260 target)
- **Why it fired:** Significant open model release — first frontier-class 10M-token context model on a single GPU at a price point below every comparable baseline. Primary source confirmed via the official model card with full benchmark table. Not a duplicate of Shivang's recent posts.
