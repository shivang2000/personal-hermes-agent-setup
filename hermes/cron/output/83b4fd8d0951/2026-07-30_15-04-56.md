...
**Tweet text:**
> OpenAI claims GPT-5.6 Sol beats Opus 5 on ARC-AGI-3: 38.3% vs 30.2%. But that number uses custom API settings (Retained Reasoning + Compaction). In the standard ARC-AGI-3 env, GPT-5.6 Sol drops to 7.8%. The benchmark is measuring the wrapper, not just the model.

**X URL:** https://x.com/shivangchheda22/status/2082761798395318303
**Source:** THE DECODER / NextAI (ainews.littlegleam.com), cross-referenced against X search results and the article body. Confirmed: OpenAI's claimed 38.3% uses Retained Reasoning + Compaction API settings; standard ARC-AGI-3 environment score is 7.8%. ARC Prize / François Chollet dispute the fairness of non-standardized comparisons.
**Image attached:** No — the only available screenshot was a full article page (no clean benchmark chart), so text-only was the right call for legibility.
