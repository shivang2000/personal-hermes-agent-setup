...
**Tweet text:**
> DeepSeek V4 Flash (304B) runs on a single AMD MI300X. Open-sourced today. 192GB HBM fits the full model, no quant: 168 tok/s decode, 256K context. Catch: MI300X uses fnuz FP8, not OCP standard. Kernels assuming OCP are off by 2x in scale. Half an H100s cost.

**X URL:** https://x.com/shivangchheda22/status/2084627016016662852

**Image attached:** Yes — screenshot of the GitHub repo (ryanzhou/deepseek-v4-flash-mi300x) showing the README with performance results table.

**Source:** GitHub repo `ryanzhou/deepseek-v4-flash-mi300x` (commit 3 hours ago, HN front page 116 points). Primary source confirms: 304B checkpoint fits in 156.67 GiB HBM on a single MI300X (192GB), 168.6 tok/s single-stream decode, 256K context validated, fnuz FP8 variant vs OCP standard causing 2x scale-domain errors in unpatched kernels.
