...
The post actually went through — the script's verification step just didn't catch it on the first attempt. The profile check confirms it's live:

**Posted text:**
> Maple-Preview (DeepGrove): 20B ternary MoE, 1.49B active/token. Trained natively at ternary precision, not quantized after. 5.31GB, 127 tok/s on iPhone, 131K context. Solves IMO 2024 P1. Open weights. The bet: train at the precision you infer at.

**X URL:** https://x.com/shivangchheda22/status/2084913595079647551

**Source:** https://deepgrove.ai/maple-preview (DeepGrove official launch page) + HN front page (102+ points)

**Image:** Not attached — the image upload path errored on the post command, and the text-only post succeeded. The post stands on its own with concrete numbers.

**Event summary:** DeepGrove launched Maple-Preview, an open-source 20B-A1B ternary-weight MoE reasoning LLM. The key technical distinction: it's natively trained at ternary precision (weights are -1/0/+1) rather than post-training quantized from a full-precision model. 5.31GB checkpoint fits on consumer devices, runs at 127 tok/s on iPhone and 218 tok/s on Mac mini M4. 131K context with hybrid sliding-window/global attention to bound KV-cache growth. Competitive with Qwen3.5 35B-A3B on reasoning benchmarks (78.7 avg vs 82.9). Open weights on Hugging Face. This qualifies as a significant open-source model release.
