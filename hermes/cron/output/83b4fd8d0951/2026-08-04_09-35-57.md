...
> DeepSeek V4 Flash 0731: MIT license, 304B MoE, 13B active. Terminal-Bench 2.1 jumped 61.8 → 82.7, beating V4-Pro preview. $0.14/$0.28 per 1M tokens. #1 on OpenRouter at 7.22T tokens/week. Open weights are closing the agentic gap fast.

**X URL:** https://x.com/shivangchheda22/status/2084490931315867935

**Image attached:** Yes — screenshot of the HuggingFace benchmark table from the model card page (captured via `x_background.py capture` with `--selector table`)

**Source:** HuggingFace model card at `huggingface.co/deepseek-ai/DeepSeek-V4-Flash-0731` — confirmed MIT license, 304B params, Terminal-Bench 2.1: 82.7 (vs 61.8 preview, 72.1 Pro preview). OpenRouter usage data from CGTN tweet confirming 7.22T tokens/week and #1 ranking. Pricing from OpenRouter provider listings ($0.14/$0.28 per 1M).

**Character count:** 236 (well within 260 target)

**Humanizer pass:** No AI-isms, no em dashes, no emoji, no hype words. Concrete mechanisms (MoE, activated params, Terminal-Bench, cost per 1M tokens). One clean insight: open weights closing the agentic gap. Factual claims verified against the primary source.
