...
> GPT-5.6 Luna dropped 80%: $1.00/$6.00 → $0.20/$1.20 per M tokens. Cached input is $0.02/M. For agent loops replaying the same system prompt + tool schemas each turn, the cache rate is what matters, not the headline. Recompute per-task cost on the cache-hit path. #AIInfra

**URL:** https://x.com/shivangchheda22/status/2083871674768982421
**Image attached:** Yes — screenshot of OpenAI's official API pricing page (developers.openai.com/api/docs/pricing) showing the gpt-5.6-luna pricing table
**Source:** OpenAI official pricing page (primary), confirmed against multiple X reports and Artificial Analysis leaderboard

**Why this qualified as breaking:** Major API pricing change — OpenAI cut GPT-5.6 Luna by 80% and GPT-5.6 Terra by 20% on July 30-31, 2026, roughly three weeks after the model launch. This is a significant pricing event for anyone running production agent workloads.

**Angle chosen:** The cache-hit path, not the headline price, is what determines real per-task cost for agent loops that replay the same system prompt and tool schemas each turn. At $0.02/M cached input, the cost structure changes the routing math for production agent budgets. This complements Shivang's earlier DeepSeek V4 Flash cache-hit/miss pricing post without duplicating it.
