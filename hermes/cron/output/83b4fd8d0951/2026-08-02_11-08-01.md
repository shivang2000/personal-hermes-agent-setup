...
- **Text:** OpenAI's Astra solved 10 open math problems. The interesting part is the proof format: 522k lines of Lean 4, each verified by an independent kernel. You don't have to trust the model - you can check the output. Same pattern that matters for AI-generated code.
- **URL:** https://x.com/shivangchheda22/status/2083789343890739299
- **Image:** None attached. Attempted to capture https://openai.com/index/ten-advances-in-mathematics/ but the screenshot was mostly blank (page renders client-side; content region too small to serve as a legible proof image). Posted text-only per skill guidance rather than attaching filler.
- **Source:** Primary — openai.com/index/ten-advances-in-mathematics/ and github.com/openai/ten-proofs. Confirmed across multiple X posts: Astra (internal, unreleased next-gen model) produced 10 new results on decade-plus-open problems (Connes rigidity, non-sofic groups, sphere packing, GapCVP hardness, etc.). Proofs published as 522k lines of machine-checkable Lean 4 with independent-kernel verification. Compute cost reported ~$2,000.
- **Duplicate check:** Shivang's recent posts covered Opus 5 long-horizon codegen, Kimi K2.6/K3, Opus 5 effort control, and eval isolation breaches. No prior Astra coverage.
