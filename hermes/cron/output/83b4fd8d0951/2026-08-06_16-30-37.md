...
The isolated X automation profile needs its one-time manual login before headless posting can resume.

**Status:** `logged_in: false` — the persistent Chromium profile at `~/.hermes/browser-profiles/x-automation` is not authenticated. The `check` command landed on a Cloudflare/access challenge page (`x.com/account/access`), meaning the session cookie is expired or was never set.
...
**What this run did:** No search, no posting, no screenshot capture was attempted. All breaking-news trigger work is blocked until the profile is authenticated.
