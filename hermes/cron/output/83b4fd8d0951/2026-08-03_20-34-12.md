...
**Tweet text:**
> 54 LLM-hallucinated SQLite CVEs got NVD 9.8 critical scores. Cited functions didn't exist, patches had zero diff, PoCs didn't crash. SQLite's creator confirmed they're fake. The CVE pipeline verifies nothing. Your scanner trusts the submitter. #AIAgents

**X URL:** https://x.com/shivangchheda22/status/2084294209188806822

**Source:** JFrog Security Research — "SQLite Critical CVEs or LLM Slop?" (research.jfrog.com, July 30 2026, 407 pts on HN, 130 comments)

**Image attached:** Yes — screenshot of the JFrog research article showing the CVE audit table

**Event summary:** An LLM generated 54 fake SQLite vulnerability advisories, submitted them to MITRE's public CVE form (no identity verification, no PoC required), and NVD assigned them 9.8 critical scores. CISA's ADP enriched them. JFrog researchers verified every claim: cited functions didn't exist in the target version, referenced patches had zero diff, PoCs failed at the parser stage. SQLite's Richard Hipp confirmed they're fabricated. This is a systemic AI safety finding — the CVE ingestion pipeline has no proof-of-concept or reproduction requirement, so plausible-sounding LLM slop flows directly into enterprise scanners and compliance workflows.
