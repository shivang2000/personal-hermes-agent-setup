...
Posted with an ARC-AGI-3 leaderboard screenshot:

> The useful signal isn't only 30.2% vs 7.8%. Opus 5 turned an unfamiliar visual layout into algebra, then used that representation to act. That's the agent pattern to watch: build a compact world model before spending more tool calls.

https://x.com/shivangchheda22/status/2080919011332964464

The primary-source quote attempt was blocked by X account-tier permissions, so the post was published standalone with evidence attached.

Possible follow-up angles:
- Why replay logs matter more than aggregate scores when evaluating agent behavior.
- ARC-AGI-3 cost tradeoff: 30.2% required roughly $20.7K across the evaluation.
