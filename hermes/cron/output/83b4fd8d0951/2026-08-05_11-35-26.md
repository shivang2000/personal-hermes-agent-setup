...
**Breaking-news trigger fired for:** DeepGrove Maple-Preview open-source release

**Posted tweet:**
> DeepGrove open-sourced Maple-Preview: 20B-A1B ternary MoE trained natively at 3-bit, not post-quantized. 127 tok/s on iPhone, 5.3GB checkpoint, 131K context. Solves IMO 2024 P1 7/7. Train at the precision you infer at.

**X URL:** https://x.com/shivangchheda22/status/2084883396317364516
**Image attached:** Yes — full-page screenshot of the Maple-Preview landing page (specs + benchmark table)
**Source:** https://deepgrove.ai/maple-preview (primary), confirmed via HN Show HN post (86 points) and Hugging Face weights link

**Why this qualified as breaking:**
- Significant open-source model release with a novel architecture angle (natively trained ternary, not post-quantized)
- Concrete mechanism: 3-bit training, 127 tok/s on iPhone, 131K context, IMO 2024 P1 solve
- Builder-relevant tradeoff: "train at the precision you infer at" — the core thesis is that post-quantization is the wrong approach; natively low-precision models avoid the accuracy tax

**Duplicate check:** Shivang's recent posts covered Mistral Shieldstral, Liquid LFM2.5, Cloudflare per-call tracing, and the UK AISI incident — no overlap with Maple-Preview.
