...
**Breaking-news autopost — posted**

**Tweet text:**
> Cloudflare on serving Kimi K2.6 + GLM 5.2 at scale: FP8 KV cache doubles in-memory context (686K → 1.37M tokens), INT4 weights cut GLM from 705GB to 421GB. Both within 1 pt of FP8 on every benchmark. For long-context models, KV cache fills GPU memory before weights do.

**X URL:** https://x.com/shivangchheda22/status/2084498296656281825

**Source:** Cloudflare Blog — "Smaller, faster, safer: running Kimi and GLM at scale" (Aug 3, 2026) — https://blog.cloudflare.com/smaller-faster-safer-models/

**Image attached:** Yes — cropped screenshot of the article's intro and FP8 vs BF16 KV cache benchmark table showing 64 concurrent requests hitting 2,192 tok/s on FP8 vs OOM on BF16.

**Why this qualified as breaking:** Cloudflare published concrete production infra details for serving two of the largest open-weights MoE models (Kimi K2.6, GLM 5.2) — FP8 KV cache quantization, INT4 weight compression, and KV cache integrity checking. This is a significant AI infra/engineering post from a major provider, directly relevant to Shivang's builder/infra audience. Not previously covered in his recent tweets.
