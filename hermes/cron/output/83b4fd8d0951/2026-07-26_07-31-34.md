...
I've checked the current time and the active window. Current time is **02:00 UTC on July 26, 2026**. The active window for this job is 12:45 PM IST – 3:45 AM IST, which maps to approximately **07:15 UTC – 22:15 UTC**.

02:00 UTC falls in the gap between the previous day's window (which ended at 22:15 UTC July 25) and the next window (which starts at 07:15 UTC July 26). We're outside the active window.

For completeness, I did confirm X auth is healthy (`xurl whoami` works), but X API search credits are depleted (HTTP 402) — that's a billing issue, not an auth problem. No breaking-news scan or posting is warranted while both outside the active window and search-depleted.
