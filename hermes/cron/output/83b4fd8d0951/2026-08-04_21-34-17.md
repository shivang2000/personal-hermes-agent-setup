...
**Text:** DeepSeek V4 Flash (304B) on one AMD MI300X: 192GB HBM, no quantization, 168.6 tok/s, 256K context. The catch: MI300X uses FNUZ FP8 not OCP — kernels can be silently wrong by 2x. Open repo ships patches for MoE routing, AITER shapes, and KV offload.

**URL:** https://x.com/shivangchheda22/status/2084671673467605432

**Image attached:** Yes — screenshot of the GitHub repo README showing the benchmark table and configuration details.

**Source:** github.com/ryanzhou/deepseek-v4-flash-mi300x (244 points on HN, 56 GitHub stars, posted 6h ago)

**Why this qualified as breaking:** Significant open-source launch — a production-grade stack for running a 304B-parameter model on a single AMD MI300X GPU, with concrete benchmarks (168.6 tok/s decode, 256K context, 156.67 GiB HBM, no quantization) and real engineering patches for the FNUZ-vs-OCP FP8 incompatibility that can silently produce wrong results by a factor of 2x. The FP8 correctness issue is the kind of production gotcha that matters to builders deploying open-weight models on non-NVIDIA hardware.

**Humanizer check:** No AI-isms, no hype words, concrete mechanisms throughout (FNUZ FP8, OCP, MoE routing, AITER shapes, KV offload), builder voice preserved, 250 chars (within 260 target). No hashtags needed — the technical density carries it.

**Duplicate check:** Shivang's 6 most recent tweets (all from today) cover AIDE², Cloudflare KV quantization, Uber agent security, OpenAI Astra math proofs, and the HF sandbox escape. No overlap with the MI300X deployment angle.
