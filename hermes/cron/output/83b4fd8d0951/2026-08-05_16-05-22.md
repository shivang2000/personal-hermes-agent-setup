...
> MCP 2.0 (the 2026-07-28 spec) goes stateless: one HTTP POST instead of two. No session IDs, no server-side state, no sticky routing. Simon Willison already shipped mcp-explorer and datasette-mcp on it. Easier to audit than shell+curl agents too.

**URL:** https://x.com/shivangchheda22/status/2084951336760885443

**Source:** Simon Willison's blog post "Stateless MCP has recaptured my interest" (simonwillison.net, 31 July 2026) — #1 on HN with 228 points and 116 comments. Covers the MCP 2.0 spec (2026-07-28 Model Context Protocol) which eliminates the stateful session handshake in favor of a single stateless HTTP POST.

**Image attached:** Yes — screenshot of the Simon Willison blog article.

**Length:** 245 characters.

**Duplicate check:** Verified against @shivangchheda22's 15 most recent tweets — no prior coverage of MCP 2.0 or stateless MCP.
