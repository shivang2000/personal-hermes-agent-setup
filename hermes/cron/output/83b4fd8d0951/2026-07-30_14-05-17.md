...
**Breaking-news trigger fired and posted successfully.**

- **Tweet text:** Claude Mythos Preview found a new attack on HAWK, a NIST post-quantum candidate that survived 2 years of expert review. 60 hours, ~$100K API cost. Key strength halved (2^64 to 2^38 ops). No production crypto affected. Humans spent hundreds of hours verifying.
- **X URL:** https://x.com/shivangchheda22/status/2082746799291531268
- **Image attached:** Yes — screenshot of Anthropic's primary research page (anthropic.com/research/discovering-cryptographic-weaknesses)
- **Source:** Anthropic's official research blog post, "Discovering cryptographic weaknesses with Claude," published July 28, 2026. Confirmed via primary source: HAWK key recovery reduced from 2^64 to 2^38 operations, ~60 hours of autonomous agent work, ~$100K API cost per result, no production systems affected, human verification took hundreds of hours.
- **Dedup check:** Shivang's 5 most recent tweets covered Kimi K3-256k, ThunderAgent KV-cache, and TurboFieldfare/Gemma 4 — no overlap with this story.
