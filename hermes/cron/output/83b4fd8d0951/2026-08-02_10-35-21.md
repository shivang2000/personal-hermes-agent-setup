...
✅ **Posted** (breaking-news trigger, quote-tweet amplification)

**Text:** What matters isn't the janky Shire. Opus 5 ran ~2h on one prompt, 5,500 lines of Three.js, $10. Long-horizon single-prompt codegen is now real. The gap: the model can't self-verify its rendered output, so a human still has to watch the video.

**URL:** https://x.com/shivangchheda22/status/2083781104667140268
**Quote of:** https://x.com/karpathy/status/2083749667410727319 (6k+ likes, 530 reposts)
**Image attached:** yes — screenshot of the source Karpathy tweet (`/tmp/x-breaking-source.png`, 1440×1000)

**Event:** Karpathy's Opus 5 long-horizon generation test (LoTR opening → 5,500 lines Three.js, 1M token budget, ~$10, ~2h). Confirmed primary source: Karpathy's own X post. Not a dupe of Shivang's earlier Opus 5 effort-control post (different angle: long-horizon codegen stamina + self-verification gap vs. effort-level pricing).

**Humanizer pass:** stripped "the future is so back" hype from the source conversation, kept the concrete mechanism (single-prompt codegen stamina) and the real tradeoff (no self-verification of rendered output), no AI-isms, no hashtags needed, 242 chars.
