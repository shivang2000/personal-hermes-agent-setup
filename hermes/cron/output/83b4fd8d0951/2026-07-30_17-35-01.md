...
**Posted tweet:**
> Cline had GPT-5.6 Sol recursively improve the agent harness around Kimi K3. 17h, 0 weight changes. Terminal-Bench 2.1: 77.5% to 88.8%, cost 79 to 49.8 per run. Fixes: retry logic, loop detection, pkill self-kill. Harness was the bottleneck, not the model.

**X URL:** https://x.com/shivangchheda22/status/2082799580555686017

**Image attached:** Yes — cropped screenshot of the Cline blog post (recursive-self-improvement-for-coding-agents) showing the title and key stats.

**Source:** Cline blog post at https://cline.bot/blog/recursive-self-improvement-for-coding-agents (published July 24, 2026, by Ara Khan). Cross-referenced with Cline's official X account tweet at https://x.com/cline/status/2082544250148057240 (187.3k views, 3,899 likes). The blog post details 5 experiments: reasoning effort fix, 429 retry logic, output-aware loop detection, file-mention liveness fix, and pkill self-termination fix — all general harness improvements, no benchmark hacks.

**Duplicate check:** Verified against Shivang's recent 15 tweets. Today's existing posts covered GPT-5.6/GPU kernels, ARC-AGI-3 harness measurement, and the HF sandbox escape. No prior post on the Cline/Kimi K3 recursive harness improvement.
