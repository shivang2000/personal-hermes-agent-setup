...
- **Event:** DeepSeek V4 Flash 0731 official release — open weights under MIT license, 284B total / 13B active params, same architecture as April preview but Terminal-Bench 2.1 jumped from 61.8 → 82.7 (beating V4 Pro Preview's 72.1). The gains came entirely from re-post-training, not new base weights.
- **Posted text:** "DeepSeek V4 Flash 0731 kept April's architecture. Terminal-Bench 2.1 jumped 61.8 → 82.7 anyway. The gain is all post-training, not new weights. For agent builders: your SFT/RL pipeline is the lever, not the base model. MIT license, 13B active. #AIAgents"
- **X URL:** https://x.com/shivangchheda22/status/2083441175810424854
- **Image attached:** Yes — screenshot of the benchmark comparison table from the Hugging Face model card (598×500, 48KB).
- **Source:** Hugging Face model card (huggingface.com/deepseek-ai/DeepSeek-V4-Flash-0731), confirmed against Artificial Analysis leaderboard and multiple X posts from @ArtificialAnlys and others.
- **Duplicate check:** Shivang's recent tweets covered GPT-5.6 pricing, Claude Code prompt stripping, and OpenAI agent containment — none covered DeepSeek V4 Flash.
