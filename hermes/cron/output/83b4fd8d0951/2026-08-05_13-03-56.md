...
**Posted text:**
> Mistral's Shieldstral is a 3B open-weights (Apache 2.0) multimodal safety classifier. You pass the policy as plain English at inference — no retraining per deployment. Matches guard models 7x its size. Runs on one 16GB GPU. Policy lives in the prompt, not the weights.

**X URL:** https://x.com/shivangchheda22/status/2084905674946077068

**Source:** https://mistral.ai/news/shieldstral/ (Mistral AI official announcement, Aug 4 2026; HN #1 at 382 points)

**Image attached:** Yes — screenshot of the Mistral Shieldstral announcement page (converted to JPEG, 184KB).

**Why this fired:** Shieldstral is a significant open-source model release from a major AI lab — a 3B Apache-2.0 multimodal safety classifier with a novel policy-adaptive QA framing that matches models 7x larger. No prior tweet from Shivang covered it. The "policy in the prompt, not the weights" angle connects to his existing interest in guardrails, vendor failure modes, and production reliability.
