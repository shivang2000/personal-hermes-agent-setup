...
Post confirmed. Here is the report:

---

**Breaking-news trigger fired: DeepSeek-V4-Flash-0731 public beta launch**

**Event:** DeepSeek released the official `deepseek-v4-flash` API in public beta (July 31), updating to V4-Flash-0731. Same 284B MoE architecture (~13B active), zero new pre-training — pure post-training (RL/finetuning). Agent benchmarks jumped massively: Terminal-Bench 2.1 61.8→82.7, DeepSWE 7.3→54.4. Native Responses API support + Codex compatibility. This is a significant open-source model release and a direct competitor move against OpenAI's 80% Luna price cut announced the same week.

**Posted text:**
> DeepSeek-V4-Flash-0731 public beta: same 284B MoE arch, zero new pre-training. Pure post-training lifted Terminal-Bench 61.8→82.7, DeepSWE 7.3→54.4. The jump is all RL, not scale. Open-source answer to OpenAI's 80% Luna cut. #AIInfra

**X URL:** https://x.com/shivangchheda22/status/2083592911695048936
**Image attached:** No (DeepSeek's news page returned 404 for the announcement slug; their docs site doesn't expose a clean benchmark chart page. Posted text-only rather than attaching a filler image.)
**Source:** DeepSeek official API docs (api-docs.deepseek.com) confirm model update to V4-Flash-0731 + Responses API support; multiple independent X posts cite the benchmark numbers from DeepSeek's announcement.

**Duplicate check:** Verified against @shivangchheda22's recent 15 tweets — he posted about the OpenAI GPT-5.6 price cut, Anthropic Claude hacking incident, OpenAI Astra math proofs, and Supabase eval harness, but had not yet posted about DeepSeek V4-Flash. No duplicate.
