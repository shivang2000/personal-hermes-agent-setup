# Cron Job: AI/Tech breaking-news trigger (30m)

**Job ID:** 83b4fd8d0951
**Run Time:** 2026-07-27 18:03:13
**Schedule:** */30 7-22 * * *
...
You are Shivang's AI/Twitter breaking-news trigger. Your job is to detect major AI events and post immediately — not wait for a scheduled queue.

Load the `ai-tech-twitter-system` and `xurl` skills first.

## Active window
This job runs every 30 minutes during 12:45 PM IST – 3:45 AM IST (7:00 UTC – 22:00 UTC). If you are outside this window, return [SILENT].

## Detection workflow

1. Run `xurl search "AI OR LLM OR agent OR Claude OR GPT OR Gemini OR Llama OR benchmark" -n 20` to find recent high-engagement AI tweets.
2. Look for tweets from the last 2 hours that mention:
   - New model releases (GPT, Claude, Gemini, Llama, etc.)
   - Benchmark results from credible sources (Artificial Analysis, LMSYS, etc.)
   - Guardrail/safety behavior discoveries (silent fallback, rerouting, etc.)
   - Major API or pricing changes from AI providers
   - Significant open-source model releases
3. Cross-reference with Hacker News top stories for confirmation if needed.
4. Check Shivang's recent tweets: `xurl search "from:shivangchheda22 -is:retweet" -n 10` to avoid duplicating an existing post on the same event.

## What counts as breaking
- A new model release or major version
- A benchmark result from a credible source
- A guardrail/safety behavior discovery
- A major API change or pricing change
- A significant open-source model release

## What is NOT breaking
- Minor product updates, bug fixes, UX changes
- Opinion pieces or analysis by non-primary sources
- Rumors or unconfirmed leaks
- Generic "AI is changing" trend pieces

## Action when breaking event detected

1. Pick the strongest angle (news + technical takeaway + tradeoff).
2. Check if there's a high-engagement tweet to quote-tweet. If yes, use `xurl quote POST_ID "angle"`.
3. If no good quote target, post an original: `xurl post "angle"`.
4. If possible, attach a screenshot (benchmark chart, terminal output, etc.) using `xurl media upload` + `--media-id`.
5. Apply the humanizer checklist before posting.
6. Keep tweet under 280 chars, ideally under 260.
7. After posting, report the tweet text and URL to #tweets-automation.
8. If a breaking event is detected, also schedule 1-2 follow-up angles for the next 30-60 minutes by noting them in the report.

## Quota management
- Limit to 1-2 posts per scan cycle.
- If no breaking event is detected, return [SILENT].
- Each `xurl search` and `xurl post/quote` is an API call — be efficient.

## Safety
- Never read or inspect `~/.xurl`.
- Never use `--verbose`.
- Apply the humanizer skill before every post.
- Do not post rumors or unconfirmed leaks.

## Response

X API credits are depleted — both search and post endpoints return `credits depleted` (HTTP 402). `whoami` works because it's a cheaper endpoint, but write endpoints and search are blocked.

**⚠️ X API Credits Depleted — Breaking Event Detected But Cannot Post**

**Breaking event found:** Moonshot AI announced **Kimi-K3** — the world's first open 3T-class model. Key details:
- New architecture: Kimi Delta Attention + Attention Residuals
- Native agentic capabilities: tool calling, browsing, multi-step planning
- Extended context window for repository-scale code understanding
- Open weights on HuggingFace
- 497 points on HN front page, posted today (July 27, 2026)

**Prepared tweet (259 chars, humanized):**
> Moonshot AI announced Kimi-K3: first open 3T-class model. New Delta Attention arch, native tool calling, repo-scale context. Open weights on HuggingFace. A 3T open model with built-in agentic capabilities changes what agent builders can self-host. #OpenSource

**Blocker:** X API credits are depleted (`HTTP 402: credits depleted`). Both `xurl search` and `xurl post` fail. `xurl whoami` succeeds because it's a cheaper endpoint.

**Action needed:** X API credits must be topped up in the Developer Console → Billing (min $5). Once credits are restored, this tweet should be posted immediately as the event is still fresh. The follow-up angles below can also be posted then.

**Suggested follow-up angles (for 30-60 min after first post):**
1. **Cost/comparison angle:** "Kimi-K3 at 3T params is significantly larger than Llama 4 or DeepSeek V3. The open weights mean you can self-host, but inference cost for a 3T model is non-trivial — you're looking at multi-GPU setups just for serving."
2. **Mechanism angle:** "Kimi-K3's Delta Attention is the interesting piece. If it genuinely reduces attention compute for long-context agentic workloads, that matters more than the parameter count. Repo-scale context only helps if inference stays affordable at that length."

**Fleet health note:** All X-facing cron jobs (breaking-news trigger, reply scanner, opportunity scan, daily autopost) show `active` in `hermes cron list --all` but will fail on any write/search call until credits are restored. The `CreditsDepleted` error is a billing issue, not a rate-limit pause — jobs won't auto-pause but will keep failing silently.
