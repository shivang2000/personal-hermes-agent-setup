...
**Breaking-news post — OPENAI ARC-AGI-3 HARNESS FINDING**

**Posted text:**
> Two API settings tripled GPT-5.6 Sol on ARC-AGI-3 without changing the model. Retained reasoning keeps chain-of-thought between steps; compaction summarizes old context instead of truncating. 7.8% to 38.3%, 6x fewer tokens. Your context policy is your model.

**X URL:** https://x.com/shivangchheda22/status/2083192840767836652

**Image attached:** Yes — screenshot of The Decoder's article page containing OpenAI's benchmark chart showing the ARC-AGI-3 score jump from 7.8% to 38.3% with retained reasoning + context compaction.

**Source:** OpenAI blog post "Enabling two settings tripled our scores on the ARC-AGI-3 benchmark" (openai.com/index/how-two-settings-tripled-our-arc-agi-3-scores/), confirmed via Hacker News (35 points, posted ~2 days ago) and The Decoder's secondary analysis (Jul 30, 2026). Key facts cross-referenced across multiple X posts from independent accounts.

**Why this is breaking:** OpenAI demonstrated that two API-level settings (retained reasoning + context compaction via the Responses API) — not a model change — tripled GPT-5.6 Sol's ARC-AGI-3 score from 7.8% to 38.3%, surpassing Claude Opus 5's 30.2%, while using 6× fewer output tokens. This is a significant benchmark finding with direct implications for production agent builders: context/memory management policy can matter more than model selection.
