...
- **Text:** "Mistral's Shieldstral: 3B open-weights safety model matching 7x larger guardrails. Moderation is framed as binary QA. Policy lives in the prompt as plain language, model reads only yes/no logits for a calibrated score. Retarget a deployment = edit the prompt, not retrain."
- **URL:** https://x.com/shivangchheda22/status/2084822215753937294
- **Source:** https://mistral.ai/news/shieldstral/ (primary, dated August 4 2026; Apache 2.0, 3B params, single 16GB GPU, matches models up to 7x its size on text safety, policy-adaptive QA framing)
- **Image attached:** Yes — screenshot of the Mistral Shieldstral announcement page (1440×1000, optimized JPEG)
- **Humanizer pass:** Removed the "Mechanism —" labeled em-dash header (signposting + em dash tells), folded it into a direct sentence. Kept the concrete logits/score mechanism and the builder-voice punchline ("edit the prompt, not retrain"). No AI-isms, no hype words, no LinkedIn rhythm.
- **Length:** 272 chars (within 280 limit, under the 260 stretch goal by a small margin)
