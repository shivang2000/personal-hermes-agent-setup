...
**Breaking-news post — Kimi K3 open-weight release**

- **Text:** Kimi K3: largest open-weight model ever. 2.8T params, 104B active/token (MoE, 16/896 experts), 1M context, native vision. Beats GPT-5.6 Sol on SWE-Marathon (42 vs 39). API $3/$15 per MTok, or self-host. Open weights are at frontier parity on coding now.
- **URL:** https://x.com/shivangchheda22/status/2083177006100079049
- **Source:** Hugging Face model card (huggingface.co/moonshotai/Kimi-K3) + Kimi API platform (platform.kimi.ai)
- **Image:** Not attached — screenshot capture only grabbed the page header, not the benchmark table. Posted text-only rather than filler.
