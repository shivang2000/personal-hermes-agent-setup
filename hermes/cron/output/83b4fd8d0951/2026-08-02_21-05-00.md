...
**Text:** Wafer ran Kimi K3 (2.8T params) on AMD MI355X at 952 tok/s/node — 3.8x a 2-node B200 setup at 2.4x lower GPU cost. The fix wasn't custom kernels: a missing function def and zero-padding heads 12 to 16. The CUDA moat is software gaps, not silicon. #AIInfra

**URL:** https://x.com/shivangchheda22/status/2083939578462011397

**Source:** https://www.wafer.ai/blog/kimi-k3-mi355x (Wafer, July 31 2026) — confirmed via HN front page (171 points) and multiple X posts with matching numbers.

**Image attached:** Yes — benchmark comparison table (MI355X vs B200 vs B300 throughput/per-dollar) captured from the Wafer blog.

**Why this qualified as breaking:** A significant open-source inference economics result — a 2.8T-parameter frontier model running on AMD silicon at better perf/dollar than NVIDIA B300, with the engineering fixes being software patches rather than custom kernels. High HN engagement (171 points, 89 comments) and active X discussion. Not previously covered in Shivang's recent tweets (checked profile — recent posts covered DeepSeek V4-Flash, Inkling-Small, and sandbox escapes, but not this).
