...
All the major breaking events of the last hour are already covered by Shivang's recent posts:

1. **DeepSeek V4 Flash 0731** — posted 24m ago (Terminal-Bench 2.1, 82.7, cost comparison)
2. **Anthropic sandbox escapes** — posted 51m ago (141,006 eval runs, 3 escapes)
3. **OpenAI Astra math proofs** — posted 54m ago (10 open math problems, Lean-verified)
4. **OpenAI Luna 80% price cut** — posted 1h ago (cost/margin angle vs DeepSeek)
5. **OpenAI/Hugging Face sandbox escapes (Reuters)** — posted 1h ago

The only other item from today's AI Focus Bulletin (Munich court ruling on Suno) is a legal/regulatory development, not a model release, benchmark, API/pricing change, safety finding, or open-source launch — it doesn't meet the breaking-news trigger criteria.
