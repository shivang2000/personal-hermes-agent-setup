...
I've completed my breaking-news scan. Here's what I found:

- **X auth**: confirmed `logged_in: true`
- **Shivang's recent posts**: He already posted 3 tweets in the last ~1 hour covering the two biggest current AI events:
  1. Two posts on Karpathy's Pelican/Opus 5 LOTR Three.js experiment (~57 min ago)
  2. One post on DeepSeek V4 Flash 07311 open-source release (~27 min ago)

- **New candidate**: An X search surfaced a claim about "Alibaba Qwen3.8-Max" (2.4T params, beats "Opus 4.8"). This is from an unverified account (@fanofaliens, 1 like, 0 reposts), references a nonexistent model ("Opus 4.8"), and no credible primary source (Alibaba/HF/Artificial Analysis) confirms it. Per skill rules, rumors and unconfirmed leaks are not breaking news.

- **HN front page**: Nothing new qualifying as a major AI model release, benchmark, API/pricing change, safety finding, or significant open-source launch beyond what Shivang already covered.

No genuinely major, confirmed breaking AI event that Shivang hasn't already posted about.
