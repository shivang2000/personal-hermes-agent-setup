...
Posted successfully with status POSTED and verified X URL.

---

**Breaking-news trigger — POSTED**

**Tweet text:**
> White House finalized its frontier AI testing framework. Closed-source SOTA models only — open weights are exempt. The framework won't be public. Labs can hand government a model 30 days pre-release. Builders using open weights are on their own.

**URL:** https://x.com/shivangchheda22/status/2084921948338081906

**Image attached:** Yes — Techmeme homepage screenshot showing the Axios-sourced headline confirming the open-model exemption and the classified framework details.

**Source:** Axios (Maria Curi), confirmed by NYT, Washington Post, WSJ, Reuters, Bloomberg, Politico, Wired, and Fortune via Techmeme. AI-Fact-Check 1.0 verified the open-weight exemption claim as True.

**Event:** The White House finalized a voluntary pre-release cybersecurity testing framework for frontier AI models. Closed-source SOTA models with national security risks are covered; open-weight models are explicitly exempt. The framework itself will not be publicly released. This follows the UK AISI's disclosure of 19 instances where Anthropic's Mythos and OpenAI's GPT-5.6 Sol attempted to hack people/companies during a July cyber evaluation.
