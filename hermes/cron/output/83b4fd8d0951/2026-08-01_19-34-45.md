...
Excellent — confirmed from DeepSeek's official API docs. Key verified facts:
- **Model**: DeepSeek-V4-Flash-0731
- **Context**: 1M tokens, 384K max output
- **Pricing**: $0.14/1M input (cache miss), $0.28/1M output
- **Anthropic API compatibility**: Yes — `api.deepseek.com/anthropic` as drop-in replacement
- **Thinking mode**: Supported
- **Tool calls, JSON outputThe post is confirmed live on the profile. The "not found" error was just a timing race in the verification step — the tweet actually posted successfully with the image attached.

**Posted tweet:**

> DeepSeek V4 Flash 0731 ships an Anthropic-compatible API. api.deepseek.com/anthropic: point Claude Code or any Anthropic SDK at it, drop-in. $0.14/1M input, $0.28/1M output, 1M context, thinking mode. The moat isn't the model. It's the API surface. #AIAgents

**X URL:** https://x.com/shivangchheda22/status/2083554428976820649

**Source:** DeepSeek official API docs (api-docs.deepseek.com/quick_start/pricing) — confirmed model version DeepSeek-V4-Flash-0731, Anthropic API compatibility at `api.deepseek.com/anthropic`, pricing $0.14/1M input (cache miss) / $0.28/1M output, 1M context window, thinking mode supported.

**Image attached:** Yes — screenshot of the DeepSeek pricing/model details page.
