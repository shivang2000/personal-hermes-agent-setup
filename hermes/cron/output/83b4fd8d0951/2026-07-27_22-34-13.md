...
- **Kimi K3** (today's major event): Shivang already posted two angles today — the open-weights/harness take (13:15 IST) and the Composio cost/token comparison (16:42 IST). Covered.
- **Celeris-1**: launched July 24 (3 days ago), not breaking now.
- **MCP stateless spec**: goes final July 28 (tomorrow), announced weeks ago — not a breaking event today. The GitHub MCP Server adding support is a minor product update, not a major release.
- Nothing else in the search results qualifies as a genuinely major release, benchmark, API/pricing change, safety finding, or significant open-source launch that Shivang hasn't already covered.
