...
**Breaking-news post — confirmed and posted.**

**Posted text:**
> OpenAI cut GPT-5.6 Luna to $0.20/$1.20 per 1M tokens. Cached input: $0.02/1M. 1M context window, MCP + function calling built in. Agent loops that re-send context pay almost nothing. Price isn't the constraint anymore. The harness is. #AIInfra

**X URL:** https://x.com/shivangchheda22/status/2083826586466517156

**Source:** OpenAI Developer docs — `developers.openai.com/api/docs/models/gpt-5.6-luna` (primary, official). Confirmed pricing: input $0.20/1M, cached input $0.02/1M, output $1.20/1M, 1,050,000 context window, MCP + function calling + structured outputs supported. The 80% price cut was corroborated by multiple X posts citing the OpenAI blog post at `openai.com/index/advancing-the-price-performance-frontier-with-gpt-5-6/`.

**Image attached:** Yes — screenshot of the official OpenAI GPT-5.6 Luna model page showing pricing and specs (1440×1000, 146KB, legible).

**Duplicate check:** Ran `profile @shivangchheda22 15` — recent posts covered DeepSeek V4 Flash, Kimi K3, MCP spec, multi-currency booking, agent harness failures, and OpenAI Astra. No prior post on the GPT-5.6 Luna price cut.

**Character count:** 243 (within the 260 preferred budget, well under 280 limit).

**Humanizer pass:** No AI-isms ("game-changer", "the future is here", em dashes, rule-of-three, significance puffery). Concrete mechanism (cached input pricing, MCP support, context window). Builder voice with a real tradeoff framing ("price isn't the constraint anymore — the harness is"). One hashtag. Ends on substance, not a kicker.
