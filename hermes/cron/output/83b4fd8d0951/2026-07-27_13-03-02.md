...
I've completed a thorough scan. Here's my assessment:

**X API status:** Credits are depleted (`CreditsDepleted` / HTTP 402). `xurl whoami` works (cheap endpoint) but search and all write endpoints are blocked. This is an X API billing issue, not an auth problem.

**Breaking news scan results:**
- **Kimi-K3 (Moonshot AI):** Listed on HN, but the HuggingFace page shows "Upcoming release" with a countdown of ~7 hours. This is a pre-release teaser, NOT an actual model drop. Does not qualify as breaking.
- **"The new rules of context engineering for Claude 5 generation models"** (Anthropic blog, 446 HN points): Published ~11 hours ago (July 26, 20:42 UTC). Outside the 2-hour breaking window. Would have been relevant at the time but is no longer breaking.
- **"Open-weight AI is having its Kubernetes moment"** — opinion piece, not a release or benchmark.
- **"OpenAI's myopia keeps causing alignment problems"** — LessWrong analysis, not a guardrail discovery or official announcement.
- **"Running a 28.9M parameter LLM on an $8 microcontroller"** — interesting project, not a major model release.
- **"What is happening to jobs? Separating AI hype from reality"** — Stanford policy brief, not breaking.

No new model releases, benchmark results, guardrail discoveries, API changes, or open-source model releases detected within the last 2 hours.

**Fleet health note:** X API credits are depleted. This will block all X-facing cron jobs (autopost, reply scanner, breaking-news trigger) from writing or searching. The credits need to be topped up in the X Developer Console → Billing to restore the fleet. `hermes cron list --all` should be checked to see if any jobs have auto-paused due to the 402 errors.
