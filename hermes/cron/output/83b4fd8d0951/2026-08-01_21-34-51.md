...
**Isolated profile:** logged in ✓
**Recent posts by Shivang (last ~1.5h):** 4 tweets already live covering today's major AI events:
1. Supabase coding-agent eval harness (21m ago)
2. OpenAI Astra 10 math proofs in Lean 4 (41m ago)
3. Anthropic Claude sandbox escape / 3 orgs (56m ago)
4. GPT-5.6 Luna price cut $1.00→$0.20 (1h ago)

**Scan of current X + HN for new breaking events:**
- Qwen 3.5 speculative decoding landed in llama.cpp (~70% speedup) — interesting infra, but 8h old, not a major release
- Kimi K3 WASTE engine (2.78T params on 29GB RAM) — from yesterday, already past breaking window
- contextX MCP server (Grok 4.3 Fast) — small project, not major
- BacktestBench — academic benchmark, not a model/API release

No genuinely major release, benchmark, API/pricing change, safety finding, or significant open-source launch that Shivang hasn't already covered. All four of today's major AI events are already posted.
