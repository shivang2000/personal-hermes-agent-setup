...
- **Posted text:** "Kimi K3-256k: same 2.8T model, capped at 256k context, costs half the quota of the 1M version. Most coding sessions never hit 256k tokens anyway. Moonshot is basically discounting the context window most people actually use. #AIAgents"
- **X URL:** https://x.com/shivangchheda22/status/2082731495672914346
- **Source:** Primary source confirmed at https://www.kimi.com/code/docs/en/kimi-code/models — model comparison table shows k3-256k as a new model ID with 256k context window, same K3 (2.8T params), half the quota consumption of k3 (1M). Also surfaced on HN (411 pts) and X trending.
- **Image attached:** Yes — screenshot of the model comparison table from the official Kimi Code docs.
- **Duplicate check:** Verified via `profile @shivangchheda22` — Shivang's recent posts covered TurboFieldfare, the HF agent intrusion, ThunderAgent, and Tokenless, but not Kimi K3-256k. No duplicate.
