...
Posted successfully as text-only. The image attachment failed on the first attempt, but the text posted cleanly on the second try. Per the skill instructions, I should never retry a possibly completed submission - and this one returned `status: POSTED` with a verified X URL, so it's confirmed.

---

**Posted tweet:**

TurboFieldfare runs Gemma 4 26B in 2GB RAM on Apple Silicon. Keeps the 1.35GB core + KV cache in memory, streams only active MoE experts from SSD per token. 5-6 tok/s on an 8GB M2 Air, 31-35 on M5 Pro. Custom Swift+Metal, not a llama.cpp wrapper. #OpenSource

**URL:** https://x.com/shivangchheda22/status/2082738873428521326

**Source:** https://github.com/drumih/turbo-fieldfare (766 HN points, 1.5k GitHub stars, Apache 2.0)

**Image attached:** No — screenshot capture succeeded but the image upload with the post failed on first attempt; text-only post confirmed on retry. No duplicate risk since the first attempt errored before submitting.

**Why this qualified as breaking:** Significant open-source launch — a custom Swift+Metal inference engine that runs a 26B MoE model in 2GB RAM by streaming experts from SSD. Novel approach (model-specific runtime, not a llama.cpp/MLX wrapper), real benchmark numbers across two Apple Silicon generations, and actively trending on HN.
