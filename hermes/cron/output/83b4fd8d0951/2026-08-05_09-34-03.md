...
**Posted tweet:**
> Qwen3.8-Max went GA: 2.4T MoE, 95B active, 1M context, $2/$6 per M tokens. DeepSWE jumped 21.6→56.6 gen-over-gen. First Max-tier open weights promised next week. The agentic gap to closed labs is closing faster than the reasoning gap. #AIAgents

**X URL:** https://x.com/shivangchheda22/status/2084852862979543139

**Image attached:** Yes — screenshot of Arman Hezarkhani's X post summarizing the Qwen3.8-Max launch, saved as `/tmp/x-breaking-source.jpg` (compressed from PNG to JPEG, ~31KB original PNG).

**Source:** Synapse Brief (@Synapse_Brief, 4 Aug 2026) and Arman Hezarkhani (@ArmanHezarkhani, 3 Aug 2026) X posts, both with primary details on Alibaba's Qwen3.8-Max GA launch. Key facts verified against Arman's thread: 2.4T total parameters, 95B active, 1M context, $2/$6 per million tokens flat, DeepSWE jump 21.6→56.6, FrontierSWE 40.7→73.5, open weights promised for next week including the first-ever Max-tier open release and a 27B companion model. Pricing confirmed against Synapse Brief's post ($2/$6 flat across context window). This is Shivang's first post on the event (no duplicate in recent profile).
