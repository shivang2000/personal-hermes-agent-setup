...
**Text:** MiniMax H3: open-weights omni-modal video model. Text, image, audio in → 2K video with native stereo out. The infra trick: 40% of modulation weights pruned to a lookup table + int8 quant. 123.6GB → 42.5GB. Runs on a 3060. #OpenSource

**URL:** https://x.com/shivangchheda22/status/2084467612713726380

**Source:** ComfyUI Newsletter (comfy.org, Aug 3 2026) + Hugging Face model page (Comfy-Org/MiniMax-H3)

**Image attached:** Yes — Hugging Face model page screenshot (1440×1000, 181KB)

**Why this fired:** MiniMax H3 is a significant open-source launch — first open-weights video model from MiniMax, omni-modal (text/image/audio/video input), native stereo audio, 2K output. The key technical story for Shivang's builder audience: 66% memory reduction via modulation-weight pruning + int8 quantization brings it from 123.6GB down to 42.5GB, running on a consumer RTX 3060. HN front page at 259 pts, active X conversation with LoRA training and ComfyUI workflows already appearing within minutes of release.
