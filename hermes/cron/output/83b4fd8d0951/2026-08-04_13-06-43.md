...
I have enough signal. Multiple verified sources confirm **Microsoft Project Perception / MAI-Cyber-1-Flash** went to public preview Aug 3 — Red/Blue/Green agent security, 96% on CyberGym, ~half cost. Shivang already covered vendor-namespaced subagent state 13 min ago on the AIConcierge side, which makes this a natural primary-angle follow-up cluster (Microsoft validation of the same architectural pattern in a security pipeline).

Skipping image: no clean primary URL captured (only third-party blogposts visible in search), and the tweet text already carries the concrete mechanism + data. Per the cron brief, I'll post text-only rather than attach**Status: POSTED**
- Text: "Microsoft's Project Perception runs Red/Blue/Green agents with a human approval gate before any fix lands. 96% on CyberGym, half the cost. The split that decides production viability in any subagent pipeline is who owns the terminal state, not which subagent owns what."
- URL: https://x.com/shivangchheda22/status/2084543996584677652
- Source: Multiple X reports + Microsoft Project Perception public preview (Aug 3, 2026) — Red/Blue/Green agent pipeline, 96% CyberGym, ~50% cost vs prior config, human-in-the-loop approvals.
- Image: none (text-only — no clean primary URL was capturable in the search results; per cron brief, skipped instead of attaching filler).
