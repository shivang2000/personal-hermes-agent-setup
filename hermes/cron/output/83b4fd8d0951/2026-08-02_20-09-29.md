...
Now I have a clear picture. DeepSeek V4-Flash-0731 is a significant open-source model release from July 31. The key story: same architecture, same parameters, same price, but post-training alone caused a 7.4x jump on agent benchmarks (DeepSWEPosted successfully. Here's the report:

**Posted tweet:**
DeepSeek V4-Flash-0731: same 284B/13B MoE, same price, zero new params. Post-training alone: DeepSWE 7.3→54.4, Terminal Bench 61.8→82.7, beating V4-Pro at 72.1. Same model, same silicon. The gain was entirely in the training recipe. #LLM

**X URL:** https://x.com/shivangchheda22/status/2083925603502457132

**Image attached:** Yes — optimized screenshot of the HuggingFace model card benchmark table from `huggingface.co/deepseek-ai/DeepSeek-V4-Flash-0731`, showing the benchmark comparison table (V4-Flash-0731 vs V4-Flash Preview vs V4-Pro Preview vs GLM-5.2 vs Opus-4.8).

**Source:** Primary source confirmed via HuggingFace model card (`huggingface.co/deepseek-ai/DeepSeek-V4-Flash-0731`). The model card confirms: same architecture as DeepSeek-V4-Flash-DSpark (284B total / 13B active MoE), MIT license, released July 31, 2026. Benchmark table verifies all numbers cited in the tweet. Multiple X posts corroborate the release with consistent benchmark figures.

**Why this qualified as breaking:** Significant open-source model release. DeepSeek shipped V4-Flash-0731 as an official release with a 7.4x agent benchmark jump (DeepSWE 7.3→54.4) from post-training alone — no architecture change, no new parameters. The budget model (13B active) beating the flagship preview (V4-Pro) on agent benchmarks is a material signal for the open-source LLM landscape.
