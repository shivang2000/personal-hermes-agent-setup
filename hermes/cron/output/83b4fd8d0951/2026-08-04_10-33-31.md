...
MiniMax H3 dropped with open weights: omni-modal video, native stereo audio in the same pass (not post-process), up to 2K, 15s clips. ComfyUI pruned ~40% of modulation weights to a lookup table, 123.6GB to 42.5GB, runs on an RTX 3060. #OpenSource

**URL:** https://x.com/shivangchheda22/status/2084505440306819196

**Source:** Comfy blog (blog.comfy.org/minimax-h3), confirmed via HN front page (276 pts) and live X search showing breaking posts from seconds ago.

**Image attached:** Yes — screenshot of the Comfy blog post showing MiniMax H3 model highlights and local inference optimization details.

**Why this fired:** MiniMax H3 is a significant open-source model launch — first open-weights release from MiniMax, omni-modal video with native stereo audio, and notable engineering work on local inference (66% memory reduction, runs on consumer RTX 3060). Shivang had not yet posted about it. Breaking right now on X.
