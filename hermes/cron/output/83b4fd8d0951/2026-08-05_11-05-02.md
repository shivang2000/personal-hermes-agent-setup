...
- **Tweet text:** "Liquid AI shipped LFM2.5-2.6B: 2.6B on-device model beating Qwen3.5-9B and Gemma-4-8B on tool use. 220 tok/s on M5 Max, 30 tok/s on a phone, under 2.5GB. Per-token cost goes to zero. Agent economics shift when inference is free. #AIAgents"
- **X URL:** https://x.com/shivangchheda22/status/2084875757923504287
- **Source:** https://www.liquid.ai/blog/lfm2-5-2-6b (primary, dated Aug 4 2026)
- **Image attached:** Yes — screenshot of the Liquid AI blog page (1440x1000, 185KB)
- **Length:** 238 characters
- **Event:** Liquid AI released LFM2.5-2.6B, a 2.6B parameter agentic model running on-device (phone/laptop/CPU), trained on ~34T tokens, beats Qwen3.5-9B (9.7B) and Gemma-4-8B on instruction following and tool use benchmarks. 220 tok/s on M5 Max, 30 tok/s on phone, under 2.5GB. Open weights on Hugging Face. Per-token inference cost=REDACTED_SET_LOCALLY
- **No duplicate:** Shivang's 4 recent tweets covered Qwen3.8-Max, Mistral Shieldstral, UK AISI incident, and Cloudflare Agents tracing — none covered Liquid AI LFM2.5.
