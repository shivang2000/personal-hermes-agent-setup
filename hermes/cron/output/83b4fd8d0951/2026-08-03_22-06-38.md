...
**Tweet posted:**
MiniMax H3: first open-weights omni-modal video model from a major lab. Native stereo audio in the same pass, not bolted on after. Runs on a 3060.

But 2K output and Context-IR preprocessing are hosted API only. Open weights get you 768p base gen. #OpenSource

**X URL:** https://x.com/shivangchheda22/status/2084317469788033264
**Image attached:** Yes — screenshot of the HuggingFace model card (MiniMaxAI/MiniMax-H3)
**Source:** https://huggingface.co/MiniMaxAI/MiniMax-H3 (primary), https://blog.comfy.org/p/minimax-h3-day-0-support-in-comfyui (ComfyUI day-0 support), X search corroboration from multiple accounts

**Why this qualified as breaking:** MiniMax H3 is the first open-weights video model from a major lab (MiniMax), with native stereo audio generated in the same pass as video, day-0 ComfyUI integration, and confirmed on HuggingFace with 1.23k likes. Shivang had not yet posted about it.

**Key tradeoff surfaced:** The open weights cover H3-Base (768p generation). The 2K upscaling step (H3-Regenerate-2K) and the Context-IR multimodal preprocessing layer are hosted API only, not included in the open release. This is the kind of "what you actually get vs the headline" detail that matters to builders evaluating whether to self-host.
