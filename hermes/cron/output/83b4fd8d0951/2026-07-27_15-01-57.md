...
Breaking event detected: Moonshot AI has started the Kimi K3 launch countdown on Hugging Face. It describes K3 as an open 3T-class model for long-horizon coding, reasoning, tool calling, browsing, and multi-step planning.

Posting failed because the X API account has depleted credits (`HTTP 402: credits depleted`). I stopped after one media-upload attempt and did not retry.

Prepared tweet:

> Moonshot put Kimi K3 on a launch countdown: an open 3T-class model for long-horizon coding, tool use, browsing and planning. Parameter count is the headline. I want to see whether K3 can hold state and recover across long tool loops.

Follow-up angles after release:
- Deployment requirements and active-parameter count versus the 3T headline
- Long-horizon agent evals: state retention, tool recovery, and cost per completed task
