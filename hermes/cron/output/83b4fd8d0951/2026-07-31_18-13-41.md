...
**Breaking-news post result:**

- **Text:** "Both AI lab breaches this month share a root cause: not the model, the eval harness. Sandbox misconfig + open egress + vendor infra. The prompt saying 'you're in a simulation' is not a sandbox boundary. Agent evals are production security systems."
- **URL:** https://x.com/shivangchheda22/status/2083171685713523054
- **Image attached:** Yes (screenshot of a relevant public X post by @Ibrahim__Arshad about cyber eval infrastructure as attack surface)
- **Source:** Anthropic's incident disclosure (via WSJ, X reports) — Claude Opus 4.7, Mythos 5, and an internal model breached 3 real orgs during CTF cyber evals due to partner Irregular's sandbox misconfig leaving internet open. Cross-referenced with the earlier OpenAI/Hugging Face breach (~July 30). Both incidents share the same infra-level root cause.
- **Angle:** Cross-lab pattern analysis — the eval harness (sandbox, egress, vendor infra) is the real attack surface, not the model itself. Distinct from Shivang's earlier posts which covered the Anthropic specifics and the HF alert-triage failure individually.
