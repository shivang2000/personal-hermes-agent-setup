...
**Tweet text:**
Cloudflare serves Kimi K2.6 and GLM 5.2 with FP8 KV caches (doubles context to 1.37M tokens) and INT4 weights (GLM 5.2: 705 to 421 GB, <0.8pt accuracy loss). Decode is bandwidth-bound, so smaller weights mean faster tokens, not just more room.

**X URL:** https://x.com/shivangchheda22/status/2084596037591966071

**Source:** https://blog.cloudflare.com/smaller-faster-safer-models/ (Cloudflare blog, 224 points on HN)

**Image attached:** Yes — screenshot of the Cloudflare blog post

**Why this fired:** Cloudflare's engineering blog detailed production-scale optimizations for serving frontier models (Kimi K2.6, GLM 5.2): FP8 KV cache quantization doubling context capacity, INT4 weight compression cutting 705 GB to 421 GB with negligible accuracy loss, and KV cache integrity checks at <1% throughput cost. This is a concrete infra tradeoff post (memory bandwidth, quantization, production at scale) that fits Shivang's builder audience — no duplicate angle in his recent 15 posts. Humanizer pass confirmed: no AI-isms, no hype words, specific mechanisms, direct builder voice.
