...
⚠️ sync issue · sync script timed out at 120s
- no git push completed this run
- target repo/branch not confirmed this tick (push didn't finish)
- next sync in ~55m — if it times out again, worth checking `sync_personal_hermes_repo.py` for a slow step (large untracked tree, network stall, or a 120s-budget that's too tight for the payload)
