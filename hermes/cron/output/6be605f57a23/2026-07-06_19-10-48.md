...
⚠️ sync issue · git push failed (exit 128)

- run 2026-07-06 17:56:19 — `git push -u origin main` rejected
- exit 128 → typically auth/credential, network, or remote-side rejection
- stderr carries no git-side error message, only the exit code (script's `subprocess.run` captured `stderr=STDOUT` but git apparently produced nothing visible)
- worth running `~/.hermes/scripts/sync_personal_hermes_repo.py` manually to see git's full error (auth prompt? token expired? remote moved?)
- next sync in ~55m
