...
Strong opportunity found: Anthropic says it removed over 80% of Claude Code's system prompt for newer models with no measurable loss on coding evals.

Posting failed because the X API returned `402 CreditsDepleted`. No retry was attempted.

Draft, 257 characters:

> Anthropic cut Claude Code's system prompt by over 80% with no measurable eval loss. Agent lesson: put instructions where they apply. Tool behavior in schemas, verification in on-demand skills, memory outside the global prompt. Less context, fewer conflicts.
