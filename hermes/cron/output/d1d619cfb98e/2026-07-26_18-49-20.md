...
Posting blocked: X API returned `402 CreditsDepleted` while uploading the Anthropic source image. No tweet was published.

Draft (226 chars):

> Anthropic removed 80%+ of Claude Code's system prompt with no measurable eval loss. My takeaway for agent builders: spend top-level context on invariants. Load workflows as skills, and let tool schemas carry usage constraints.
