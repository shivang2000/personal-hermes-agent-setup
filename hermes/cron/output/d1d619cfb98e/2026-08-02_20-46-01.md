...
**Action:** none — blocked on auth.
**Reason:** `x_background.py check` returned `"logged_in": false`. The profile at `~/.hermes/browser-profiles/x-automation` needs its one-time manual login.

**Fix (one-time):**
```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```
Log in to X in the window that opens, return to the terminal, press Enter. After that, scheduled jobs will run headlessly without touching Arc or the visible browser.
