...
- **Reply to:** https://x.com/yacinelearning/status/2082536499355033996 (165 likes, 15 reposts — eval landscape 2026 interview)
- **Text:** "The eval conversation usually focuses on output correctness. The gap I keep hitting: two agents both pass, but one takes 3 tool calls and the other 30. Trajectory length is where latency and cost live, and most harnesses don't grade it."
- **Source:** @yacinelearning's post on the 2026 eval landscape + a Florian benchmark interview
- **Image:** none (no public primary source artifact to capture; text-only reply is appropriate)
- **Status:** REPLIED — verified

Rationale: highest-engagement relevant tweet in the scan (165 likes, a builder audience discussing evals), original misses the trajectory-efficiency angle, and the reply adds a concrete production mechanism (tool-call count → latency/cost) rather than restating the post.
