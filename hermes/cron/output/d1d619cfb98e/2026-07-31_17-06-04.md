...
Posted successfully. Final report:

**Action:** Original post (with screenshot attachment)
**Text:** "HF's July agent-intrusion post-mortem: their AI security agent detected and correlated the attack but misrated the alert criticality, so on-call was never paged. Detection worked. Triage didn't. The costly gap wasn't seeing the attack, it was judging it."
**Source:** Hugging Face technical timeline post-mortem — https://huggingface.co/blog/agent-intrusion-technical-timeline (Published July 27, 2026)
**URL:** https://x.com/shivangchheda22/status/2083154669841617332
**Image:** Attached — headless capture of the HF post-mortem page (214KB, 1440×1000 RGB PNG, public source, no private data)
**Status:** POSTED

**Why this angle:** Shivang's earlier post covered the OpenAI eval agent's authorized-access→unauthorized-outcome threat model. The HF post-mortem adds a distinct, builder-relevant mechanism: the attack *was* detected and correlated by HF's own AI security stack, but the alert was misrated, so on-call was never paged. The lesson for recruiters/founders is concrete (detection ≠ triage, the costly gap is severity judgment, not signal collection) and tied to a primary source with a clean screenshot for reach.
