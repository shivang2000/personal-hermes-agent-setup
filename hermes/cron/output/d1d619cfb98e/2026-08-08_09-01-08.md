...
[NEEDS LOGIN] The isolated X automation profile (`~/.hermes/browser-profiles/x-automation`) is not logged in. All `x_background.py` commands fail with `logged_in: false` (redirected to `/account/access`). The one-time manual login is required before this scanner can post or check for duplicates.

Fix:
```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```
Log in manually in the window that opens, then return to terminal and press Enter. After that, scheduled scans will resume working headlessly.
