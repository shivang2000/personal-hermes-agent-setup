...
**Action: Quote post**
- **Text:** "2.4T params is the headline. The number that sets inference cost is ~95B active per token (~4% sparsity). That is what drives VRAM, latency, and $/task. Open weights next week turn "which model" into a routing problem, not a brand decision."
- **Source:** Official @Alibaba_Qwen launch announcement (22.5k likes, 4.7k reposts) — https://x.com/Alibaba_Qwen/status/2084100707423289643
- **Angle:** Sparsity/cost mechanism (95B active per token, ~4% sparsity) rather than restating the 2.4T headline — what actually drives inference economics for builders routing between models.
- **Image:** Screenshot of the source tweet attached (public primary source, no private data).
- **URL:** https://x.com/shivangchheda22/status/2084886776276296131
