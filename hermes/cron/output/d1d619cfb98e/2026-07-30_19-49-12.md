...
**Text:** "Perplexity open-sourced Numbat: endpoint agent security hooking into Claude Code, Codex, Cursor. Watches agent actions locally, blocks dangerous ops before they run. Most agent security assumes the threat is remote injection. This assumes the agent itself goes rogue. #OpenSource"

**Source:** https://github.com/perplexityai/numbat — Perplexity Numbat open-source agent security suite (released ~July 30, 2026)

**X URL:** https://x.com/shivangchheda22/status/2082833357281628211

**Image:** Attached — screenshot of the GitHub repo page (189 KB PNG). Captured headlessly from the public repo URL.

**Angle:** Most agent security work focuses on indirect prompt injection (remote threats). Numbat sits on the endpoint and hooks into the coding agents themselves, which is a different threat model: the agent going rogue locally. Concrete mechanism (endpoint hooks + pre-execution blocking), concrete tradeoff (remote-injection vs local-agent-rogue), and tagged to the active agent-safety conversation. No duplicate of recent posts (last 5 tweets were about Opus 5 benchmarks, GPT-5.6 Sol infra, and the ExploitGym sandbox escape).
