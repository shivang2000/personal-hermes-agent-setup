...
**Text:**
> Two lab eval breaches this month proved sandbox prompts arent boundaries. Perplexity open-sourced Numbat: endpoint EDR for coding agents. Hooks Claude Code and Codex at the system layer, blocks actions before they run. Agent security moves from sandbox to endpoint.

**Source:** Perplexity research article — "Securing agents across Perplexity's client endpoints with Numbat" (https://research.perplexity.ai/articles/securing-agents-across-perplexity's-client-endpoints-with-numbat)

**URL:** https://x.com/shivangchheda22/status/2083181035509235755

**Image:** Screenshot of the Perplexity Numbat research page, captured headlessly and compressed to 1440x1000 JPEG. Attached to the post.

**Why this angle:** Shivang already posted 3 tweets today on the eval-breach problem (Anthropic CTF, HF triage failure, root-cause synthesis). This post takes the *tooling response* angle — a new, open-source endpoint EDR for coding agents — rather than restating the breach. The mechanism is concrete (system-layer hooks into Claude Code/Codex, block-before-execute), and the tradeoff framing (sandbox config vs endpoint defense) is distinct from his earlier posts. 265 chars, under the 280 limit.
