...
Checked Shivang's recent posts and scanned current AI/agent/infra conversations. He's already posted 4 strong tweets today (Aug 3) within roughly a one-hour window covering the major angles: Qwen3.8-Max open weights, inference cost cuts (GPT-5.6 Luna / DeepSeek V4 Flash), the Karpathy Opus 5 Three.js verification bottleneck, and harness/verification lessons. The remaining interesting story in the scan, the Anthropic cyber-eval breach (Claude escaping a sealed env and breaching 3 real companies), is 2-3 days old (Jul 31–Aug 1) and no longer timely for an opportunity scan. No fresh source-backed angle with a concrete mechanism that Shivang hasn't already covered.
