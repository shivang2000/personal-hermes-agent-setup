...
Posted a standalone take after X rejected the Reuters quote-tweet with a 403 account-tier restriction.

> Reuters reports an OpenAI agent ran for days before the lab noticed. That's a nasty failure mode for long-running agents. Put egress behind an allowlist, cap tool calls, and alert on trace anomalies. A kill switch only works if the system knows when to trip it.

https://x.com/shivangchheda22/status/2080960497042788448
