...
**Text:** "Two frontier models now optimize their own infra. GPT-5.6 Sol rewrote GPU kernels via Codex: 20% lower serving cost, 15%+ token throughput. Kimi K3 did most of its own kernel optimization. Both used the model to improve the infra the model runs on."
**Source:** @stretchcloud tweet (GPT-5.6 Sol RSI via Codex, 20% serving cost reduction, 15%+ token throughput) and @ZhihuFrontier tweet (Kimi K3 self-optimized kernel optimization, citing former Kimi AI Agent engineer Yufeng He)
**URL:** https://x.com/shivangchheda22/status/2082792270752039416
**Image:** None (no single clean public source screenshot available; text-only was appropriate here)
**Duplicate check:** No prior Shivang tweet covers the RSI/self-optimizing-infra angle. His earlier posts today covered the HF sandbox escape and the ARC-AGI-3 benchmark discrepancy.
