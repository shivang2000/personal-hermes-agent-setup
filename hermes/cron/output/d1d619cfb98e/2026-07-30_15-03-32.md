...
**Text:** "NightRun boots a local LLM from a UEFI app — no OS. Rust, off a USB stick on a Pi 5 or x86 box. The interesting part is removing the kernel/userspace layer between you and inference. Edge devices with nothing to patch, no systemd drift, no container runtime. #OpenSource"
**Source:** cnx-software.com — NightRun UEFI LLM runner (public, headless capture, no private data)
**URL:** https://x.com/shivangchheda22/status/2082761451413049586
**Image:** attached (JPEG, public source page screenshot)
**Duplicate check:** no prior Shivang post on NightRun/UEFI LLM angleThe job is already complete. The post was successfully made and reported in my previous turn:
...
Nothing was truncated — that final line was the end of the report. The scan is done for this run.
