...
The isolated X profile needs its one-time manual login. I can't post or scan until that's completed.

**Action needed:** The X automation profile at `~/.hermes/browser-profiles/x-automation` is not logged in. Run:

```bash
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

This opens a visible Chromium window. Log in to X manually, return to terminal, press Enter. After that, the scheduled scanner jobs will work headlessly.
