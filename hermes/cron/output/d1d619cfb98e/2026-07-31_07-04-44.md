...
**Action:** Original post (text-only — image attachment failed due to composer-text detection issue on X's compose page, not retried to avoid double-posting).

**Text:**
> OpenAI cut GPT-5.6 Luna 80% and Terra 20%, added a 2.5x Fast mode on Sol. Pricing is where the real competition is now, not benchmarks. For agent fleets, per-task model selection beats picking one frontier model. #AIInfra

**Source:** OpenAI blog — "Advancing the price-performance frontier with GPT-5.6" (489 pts on HN, announced minutes before this scan)

**URL:** https://x.com/shivangchheda22/status/2083003350845669834

**Image status:** Screenshot captured from OpenAI blog to `/tmp/x-opportunity-image.jpg`, but the post with `--image` hit a composer-text-detection error. Retried as text-only post, which succeeded. No retry attempted with image to avoid duplicate posting.

**Humanizer check:** Stripped significance language, avoided "game-changer"/"10x" framing, kept concrete pricing numbers (80%, 20%, 2.5x), used builder-oriented framing ("per-task model selection beats picking one frontier model"). 243 chars, well under 280.
