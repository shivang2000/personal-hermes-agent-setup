# Cron Job: AI/tech opportunity scan (45m)

**Job ID:** d1d619cfb98e
**Run Time:** 2026-07-27 18:49:50
**Schedule:** */45 7-22 * * *
...
15. **Browser automation requires Arc + X.com login.** `x_browser.py` only works when Arc is running AND X.com is logged in on Arc. If Arc is closed or X.com session expired, all cron jobs will fail silently (the script returns error JSON but doesn't crash). Always run `python3 /Users/shivang/.hermes/scripts/x_browser.py check` first when debugging fleet issues. If login is lost, fall back to `xurl` API (requires credits) or ask Shivang to re-login on Arc.
...
You are Shivang's AI/Twitter opportunity scanner. Every 45 minutes during the active window, check for opportunities to post or quote-tweet about current AI/tech events.

Load the `ai-tech-twitter-system` and `humanizer` skills first.

## CRITICAL: Use browser-based X.com automation, NOT xurl API

X API credits are depleted. Instead of `xurl`, use the browser-based script at `/Users/shivang/.hermes/scripts/x_browser.py` which drives Arc browser (X.com is logged in there). This is FREE.

Available commands:
```bash
# Search X.com for opportunities
python3 /Users/shivang/.hermes/scripts/x_browser.py search "AI OR LLM OR agent OR Claude OR GPT" 20

# Post a tweet
python3 /Users/shivang/.hermes/scripts/x_browser.py post "Tweet text"

# Quote-tweet a post
python3 /Users/shivang/.hermes/scripts/x_browser.py quote "https://x.com/user/status/123" "Quote text"

# Get Shivang's recent tweets
python3 /Users/shivang/.hermes/scripts/x_browser.py profile "@shivangchheda22" 10
```

## Active window
Runs every 45 minutes during 7:00 UTC – 22:00 UTC. If outside, return [SILENT].

## Workflow

1. Check current AI/tech developments via web search (Hacker News).
2. Check Shivang's recent tweets: `python3 /Users/shivang/.hermes/scripts/x_browser.py profile "@shivangchheda22" 10`
3. Search X.com for trends: `python3 /Users/shivang/.hermes/scripts/x_browser.py search "AI OR LLM" 20`
4. If a strong opportunity exists:
   a. Draft a tweet ≤260 chars with a concrete mechanism/tradeoff.
   b. Apply the humanizer checklist.
   c. Post with `python3 /Users/shivang/.hermes/scripts/x_browser.py post "text"`.
   d. Or quote-tweet with `python3 /Users/shivang/.hermes/scripts/x_browser.py quote "URL" "text"`.
   e. Report to #tweets-automation.
5. If no strong opportunity, return [SILENT].

## Safety
- Apply the humanizer skill before every post.
- Browser automation is FREE — no quota management needed.

## Response

Assessment complete. Here's the situation:

- **Shivang just posted** about Kimi K3 at 13:15 UTC — literally 1 minute ago. Posting again immediately would look spammy and hurt reach on both tweets.
- **Bun Rust rewrite** (159 pts on HN): Interesting angle about LLM-assisted rewrites and the post-rewrite velocity drop, but not strong enough to justify back-to-back posting. The HN comments are mostly about whether the rewrite was premature — no fresh technical mechanism for Shivang's audience.
- **AI companies shredding rare books** (126 pts): High engagement but it's an outrage/ethics story, not a builder/technical angle. Shivang's voice doesn't fit this — no mechanism, no tradeoff, no production lesson.
- **Claude Opus 5 elevated errors** (30 pts, now resolved): Too low engagement and already resolved. Would be stale by the time it posted.
- **PGSimCity** (724 pts): PostgreSQL internals, not AI/agent related.

No strong enough distinct opportunity to justify a second post within 60 seconds of the Kimi K3 tweet.
