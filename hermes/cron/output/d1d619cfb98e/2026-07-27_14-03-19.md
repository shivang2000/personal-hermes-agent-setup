...
Posting blocked by X API billing (`402 CreditsDepleted`). Authentication is configured correctly.

Draft (232 characters):

> Kimi K3 is a good reminder: open weights and self-hostable are different promises. A 3T-parameter MXFP4 checkpoint is ~1.5 TB before KV cache and batching. For agent workloads, watch cost per completed task, not the download button.
