...
The isolated X automation profile is not logged in. This is a blocking issue — I cannot scan, search, or post without it.

**Report:**
- `x_background.py check` → `logged_in: false`
- The profile at `~/.hermes/browser-profiles/x-automation` needs its one-time manual login.
- No scan, search, or post was attempted.

**Action needed:** Run `python3 /Users/shivang/.hermes/scripts/x_background.py login` in a terminal with display access, log in to X manually in the separate browser window, then return to the terminal and press Enter. After that, the headless cron jobs will work normally.
