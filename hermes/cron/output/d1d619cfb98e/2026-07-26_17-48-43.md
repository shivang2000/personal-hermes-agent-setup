...
Not posted: X API credits are depleted. Recent-tweet search and media upload both returned HTTP 402 `CreditsDepleted`.

Prepared draft (253 chars):

> Anthropic cut 80% of Claude Code's system prompt with no loss on coding evals. The useful pattern: keep invariants in the base prompt, load review/verification skills on demand, and enforce safety through tool permissions. Less context, fewer conflicts.

Source: Anthropic’s “The new rules of context engineering for Claude 5 generation models.”
