...
No post made. X API returned `402 credits depleted`, so recent-tweet checks and posting are unavailable. Auth itself is valid (`xurl whoami` succeeded). Restore X API credits before the next scan.
