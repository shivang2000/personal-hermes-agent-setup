...
Strong opportunity found: Anthropic reports removing 80%+ of Claude Code’s system prompt with no measurable loss on coding evals. The post could not be published because X returned `402 CreditsDepleted`. No retries attempted.

Draft:

> Anthropic cut over 80% of Claude Code's system prompt with no measurable loss on coding evals. The useful shift: keep repo gotchas upfront, load verification skills only when needed. Smaller context means fewer conflicting rules, not fewer guardrails.
>
