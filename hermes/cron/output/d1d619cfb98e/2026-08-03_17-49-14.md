...
**Text:** 52k ternary parameters packed 4 per byte. Mamba, not attention, because there's no room for a KV cache on a 6502. 9KB inference code + 13KB weights inside 25KB. No multiply instruction, so ternary dot products cost 30 cycles vs 150 for int8 multiply-accumulate.

**Source:** Matt Beton, "Autoregressive Language Model on the 6502 Processor" (mattbeton.com/blog/bitnet-6502.html) — HN front page, 117 points

**URL:** https://x.com/shivangchheda22/status/2084252676259520694

**Image:** Attached screenshot of the blog post (public source, no private info)

**Why this angle:** Concrete mechanism (ternary BitNet quantization eliminates the need for hardware multiply, Mamba's fixed-size state avoids KV cache growth that would blow the 32KB RAM budget), specific numbers (52k params, 30 vs 150 cycles, 9KB+13KB split), and a real tradeoff (choosing recurrent over attention not for quality but because attention's growing KV cache is physically impossible in 25KB). Ties to Shivang's infra/efficiency interests and shows systems-level judgment. Not a duplicate of his earlier tweets today (which covered Qwen 3.8-Max, omni-modal serving, meat-proxy pattern, and SQLite hallucinated CVEs).
