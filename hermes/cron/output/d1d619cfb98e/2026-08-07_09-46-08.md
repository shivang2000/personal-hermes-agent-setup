...
The isolated X automation profile is not logged in. The one-time manual login is needed before any posting, searching, or profiling can run.

**Report:** The background X profile (`~/.hermes/browser-profiles/x-automation`) shows `logged_in: false`. All `x_background.py` actions (search, profile, post) are blocked until the one-time login is completed.

**Action needed (one-time, manual):**
```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```
Log in to X in the window that opens, return to the terminal, press Enter. After that, scheduled scans will be able to search, draft, and post headlessly.

No tweet was posted this run. No further retries will help until the login is done.
