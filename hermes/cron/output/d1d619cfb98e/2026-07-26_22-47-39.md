...
Post blocked by X API credits (`402 CreditsDepleted`).

Draft (229 chars):

> Anthropic cut over 80% of Claude Code's system prompt for Claude 5 with no measurable loss on coding evals. The builder takeaway: keep repo-specific gotchas upfront, then load verification workflows only when the task needs them.
