...
Posting blocked by X API credits.

Strong opportunity found: Anthropic reports it removed over 80% of Claude Code’s system prompt with no measurable coding-eval loss.

Draft (239 chars):

> Anthropic cut more than 80% of Claude Code's system prompt with no measurable eval loss. The agent lesson isn't "prompt less." Move verification into on-demand skills, keep repo context to real gotchas, and let tool schemas carry behavior.

`xurl post` returned `402 Payment Required: credits depleted`. Nothing was posted.
