...
Posting blocked by X API credits (`402 CreditsDepleted`).

Drafted tweet (256 chars):

> Kimi K3 ships with a sharp production constraint: it can become unstable if the harness drops prior thinking or switches models mid-session. Model fallback now needs conversation-state compatibility tests, not just matching tool schemas and context limits.

Source: Kimi K3’s official release notes. No tweet was posted.
