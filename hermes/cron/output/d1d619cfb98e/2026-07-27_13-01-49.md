...
Posting failed because the X API account has depleted credits (`HTTP 402: credits depleted`). No tweet was published.

Draft:

> Kimi K3 is going open-weight today, but serving it is a different problem. At 3T parameters in MXFP4, the weights alone are roughly 1.5 TB. Then add KV cache and throughput headroom. Open lets builders inspect and tune the model. It doesn’t make inference cheap.
