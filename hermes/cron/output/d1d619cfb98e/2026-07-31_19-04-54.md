...
The post went through successfully despite the terminal timeout. The profile check confirms the tweet is live.

**Action: Original post (with screenshot)**

**Text:** "Inference APIs are quietly killing session portability. Reasoning tokens billed to you return as encrypted blobs. Web searches the model sees but your client can't. Compacted context only the provider can decrypt. Your transcript is now a partial view of a session you don't own."

**Source:** Earendil Engineering's "The Session You Cannot Take With You" (https://earendil.com/posts/session-portability/) — a technical post on how inference APIs increasingly return provider-bound encrypted state (reasoning tokens, compacted context, web search results, file/vector/cache references) instead of portable transcripts, shifting session ownership from user to provider.

**X URL:** https://x.com/shivangchheda22/status/2083184493448544713

**Image:** Attached optimized JPEG screenshot of the source article (captured headlessly via `x_background.py capture`, converted to RGB JPEG under 1600px).
