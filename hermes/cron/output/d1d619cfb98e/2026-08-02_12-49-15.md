...
**Text:** Building an MCP server taught me why the July 28 spec matters. Stateless requests mean no sticky load balancers, no Redis session store. You scale like normal HTTP. But it's a breaking change, so every existing remote server needs migration work. #DevTools
**Source:** MCP 2026-07-28 spec change (multiple X posts from @stretchcloud, @JDSalbego, @wizrdoraven, @robertmcpcio; HN discussion)
**URL:** https://x.com/shivangchheda22/status/2083814834458800408
**Image:** None (no public primary source screenshot needed; the spec change is a protocol-level fact, not a benchmark/chart)
