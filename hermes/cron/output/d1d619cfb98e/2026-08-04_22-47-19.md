...
**Text:** "Not Diamond Code: cache-aware model router for coding agents. The part most miss: switching models mid-session flushes the KV cache. Cache-aware routing drops to cheaper models on easy turns without reprocessing context. One extra call per turn, savings compound."
**Length:** 263 chars (within 280 limit)
**Source:** Multiple live tweets about Not Diamond Code launch (x_background.py search), ~10 min old at post time
**URL:** https://x.com/shivangchheda22/status/2084690105042874770
**Image:** None (no public benchmark chart or source page worth capturing for this launch; text-only was the right call)
**Duplicate check:** Scanned Shivang's last 20 posts — 5 today covered Qwen3.8-Max, LFM2.5-2.6B, DeepSeek V4 Flash, OpenAI Astra, and AIDE². None touched model routing or KV cache economics.
