...
**Action:** Reply to @hamostaf04's evals post (139 likes, 27 replies — high-visibility thread)
**Text:** "The gap is answer-level vs action-level evals. An agent can write a clean summary and still call the wrong tool, hit the wrong endpoint, or leave a payment in a non-terminal state. Test the state transitions, not the prose."
**Source tweet:** https://x.com/hamostaf04/status/2083665857209917481
**Status:** REPLIED
**Image:** none

The reply adds a concrete mechanism Shivang has real production experience with (payment terminal states from AIConcierge321 booking flows) rather than restating the original point. It's a distinct angle from his earlier system-prompt/evals tweet, which focused on prompt scaffolding depreciating as models improve — this one focuses on what an eval actually needs to test.
