...
- **Action:** Original post (with screenshot of HuggingFace model card)
- **Text:** "DeepSeek V4 Flash dropped with MIT weights. 284B MoE, 13B active per token. At 1M context: ~10% of V3.2 inference FLOPs, ~7% KV cache. That is what matters for production agents running long tool-call traces. Context cost matters more than benchmark scores. #AIAgents"
- **Source:** DeepSeek-V4-Flash-0731 official release on HuggingFace (MIT weights, 284B MoE / 13B active, 1M context). FLOPs/KV-cache reduction figures from the V4 paper and X analysis (e.g. @adidshaft, @yasutaketin).
- **URL:** https://x.com/shivangchheda22/status/2083441544586207712
- **Image:** Captured headlessly from huggingface.co/deepseek-ai/DeepSeek-V4-Flash-0731, converted to JPEG (1440×1000 → optimized), attached to post.
- **Length:** 267 chars (within 280 limit, under 260 before hashtag).

This is a breaking event (new major open-source model release with full weights). The angle focuses on the inference-cost/KV-cache mechanism rather than benchmark scores, which is the production-agent-relevant tradeoff and aligns with Shivang's builder audience. Humanizer pass: stripped hype, no AI-isms, concrete technical nouns (MoE, active parameters, inference FLOPs, KV cache, tool-call traces), direct builder voice.
