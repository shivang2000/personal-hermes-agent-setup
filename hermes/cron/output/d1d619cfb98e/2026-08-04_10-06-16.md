...
- **Text:** "Qwen3.8-Max: 2.4T MoE, 95B active, 1M context, tool calls + vision. 37 pts behind Claude Opus 5 on Frontend Code Arena. $2/$6 per 1M tokens. Open weights next week.\n\nMoE trick: 95B active keeps inference cheap, 2.4T total holds the knowledge. #OpenSource"
- **Source:** https://huggingface.co/Qwen/Qwen3.8-Max (Hugging Face model card)
- **URL:** https://x.com/shivangchheda22/status/2084498553708466260
- **Image:** Attached (Hugging Face model card screenshot, 65KB, public source)
- **Length:** 253 chars (under 260 target)

The Qwen3.8-Max announcement by Alibaba is the strongest breaking AI event this scan: a 2.4T-parameter MoE flagship going open-weight next week, closing to within 37 points of Claude Opus 5 on Frontend Code Arena at $2/$6 per 1M tokens. The MoE architecture (95B active of 2.4T total) is the concrete mechanism worth surfacing: it holds near-flagship knowledge while keeping per-token inference cost low. Not duplicating Shivang's earlier DeepSeek V4 Flash post (different model, different angle — that one was about agentic gap closure; this one is about MoE cost architecture and the open-weight frontier closing on closed models).
