...
> Manifold found a nasty coding-agent edge case: some agents ran `git status` before the trust prompt. In repos shared as files, `.git/config` could use `core.fsmonitor` to execute a host command. Inspect that config before opening the repo in an agent.

https://x.com/shivangchheda22/status/2095496153106383358
