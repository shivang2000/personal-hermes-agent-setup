...
**Opportunity scan — 2026-08-07 14:XX IST**

**Status: BLOCKED — isolated X profile not logged in.**

`x_background.py check` returned `logged_in: false` (URL: `https://x.com/account/access`, title: "Just a moment..."). The profile and post commands refuse to run for the same reason.

The isolated Playwright Chromium profile at `~/.hermes/browser-profiles/x-automation` needs its one-time manual login:
...
No scan, duplicate check, or post was attempted. This job cannot proceed headlessly until the login is restored. All scheduled X jobs sharing this profile (autopost, reply scanner, opportunity scans) will fail the same way until then.

No retry was performed, so no duplicate posts risk. Reporting once as instructed.
