...
**Action: Quote-tweet**
- **Text:** "Omni-modal serving with disaggregated stages is the real shift. Text gen, video diffusion, and audio synthesis each get their own scheduler instead of blocking on one monolithic forward pass. Per-stage backpressure beats whole-model stalls. #AIInfra"
- **Source:** vLLM tweet on MiniMax H3 open weights with day-0 vLLM-Omni support (424 likes, 28 reposts)
- **URL:** https://x.com/shivangchheda22/status/2084237787528867865
- **Quote of:** https://x.com/vllm_project/status/2084109135688401316
- **Image:** None (quote-tweet renders the original, screenshot would be redundant)

**Why this angle:** MiniMax H3 open weights with day-0 vLLM-Omni support is fresh (Aug 3). The original tweet focuses on the model capabilities. The new angle is the serving architecture — disaggregated stages mean per-stage scheduling and backpressure rather than one monolithic forward pass, which is the actual infra tradeoff for production teams. Quote-tweeting the high-engagement vLLM account (424 likes) puts the post in front of their audience. No duplicate with Shivang's earlier Qwen/HF/Astra posts today.
