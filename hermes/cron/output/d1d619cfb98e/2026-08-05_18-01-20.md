...
The isolated X automation profile (`~/.hermes/browser-profiles/x-automation`) is not logged in — `x_background.py check` returns `"logged_in": false` and the profile/last-tweets commands both error out with "not logged in."

This means the one-time manual login has expired or was never completed for this profile. No posting, searching, or duplicate-checking can proceed.

**Action needed:** Run once on the machine where the browser profile lives:
```bash
python3 /Users/shivang/.hermes/scripts/x_background.py login
```
Log in to X in the window that opens, return to the terminal, press Enter. After that, `x_background.py check` should return `"logged_in": true` and the opportunity scanner can resume normal operation.

No tweet was posted. No retry attempted (would just hit the same auth wall).
