...
**Text:** "Kimi K2.6 won on price, speed, and a training lens most US models lack. K3 dropped all three — users say it 'sounds like Sonnet.' Meanwhile the House probe into DoorDash's K2.6 use pushes enterprises back to US frontier regardless of capability."
**Source signals:** House Select Committee on China/Homeland Security investigation into DoorDash's Kimi K2.6 use (~1h old); user reports that K3 "sounds like Sonnet" and lost the non-Western training lens; DoorDash internal comparison showing K2.6 and Fable 5 beating Sonnet 4.6/Opus 4.8 at lower cost.
**URL:** https://x.com/shivangchheda22/status/2083758655238029508
**Image:** None — the proof is scattered across multiple tweets and a committee action, not a single clean source page, so text-only was the right call over filler.
**Length:** 247 chars.
