...
The isolated X automation profile at `~/.hermes/browser-profiles/x-automation` is not logged in (`logged_in: false`, redirected to `x.com/account/access`). This run cannot post, search, or read the profile.

**Action needed (one-time, manual):**
```
python3 ~/.hermes/scripts/x_background.py login
```
Log in manually in the separate browser window that opens, return to the terminal, and press Enter. After that, headless cron runs will work again.
