...
The isolated X automation profile is not logged in. No post was made this run.

**Blocker:** `x_background.py check` reports `"logged_in": false` — the persistent Chromium profile at `~/.hermes/browser-profiles/x-automation` needs its one-time manual login.

To fix, run once:
```bash
python3 /Users/shivang/.hermes/scripts/x_background.py login
```
Log into X in the separate window that opens, return to the terminal, and press Enter. After that, all autopost/opportunity cron jobs will resume working headlessly without touching Arc or the visible desktop.

This was reported once. I will not repeat it on future runs until the login is completed.
