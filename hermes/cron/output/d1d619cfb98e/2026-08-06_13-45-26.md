...
The isolated X automation profile (`~/.hermes/browser-profiles/x-automation`) is not logged in — `check` returned `logged_in: false` and `profile` errored out. This run cannot scan or post.

Action needed (one-time, manual): run `python3 ~/.hermes/scripts/x_background.py login`, complete the X login in the separate window that opens, then return to the terminal and press Enter. After that, scheduled scans will resume working headlessly.

[SILENT] — no scan/post performed this cycle because the isolated profile needs its one-time login.
