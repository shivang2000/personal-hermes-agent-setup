...
- Action: quote post
- Text: "The trick: Kimi K3 is MoE, so only a slice of the 2.8T params activates per token. Stream active experts from NVMe instead of holding all weights in VRAM. You trade GPU memory for disk I/O and latency. Fine for a local demo, painful for production serving."
- Length: 256 characters
- Source: Brian Roemmele's tweet about running full Kimi K3 on a consumer laptop (1,129 likes, 193 reposts) — https://x.com/BrianRoemmele/status/2082909527515779164
- Posted: https://x.com/shivangchheda22/status/2083240830354301009
- Image: none (the original tweet already carries the visual proof; no separate public primary source screenshot would add value here)
- Angle: mechanism explanation (MoE expert activation) + production tradeoff (NVMe streaming latency vs VRAM footprint), not a restatement. Rides the reach of a 1.1k-like original in a timely Kimi K3 moment Shivang hadn't covered yet.
