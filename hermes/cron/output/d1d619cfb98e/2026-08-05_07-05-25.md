...
- **Text:** "Manifest deprecated their 4-tier LLM router after 4 months / 7000 users. Prompt text alone can't reveal task complexity: it emerges through tool calls and repo context. 'evaluate tests for $GIT_REPO' is trivial for a static site, brutal for the Linux kernel."
- **Length:** 259 characters
- **Source:** Manifest blog "Everyone is building LLM routers, we deprecated ours" (https://manifest.build/blog/why-we-deprecated-our-llm-router/) — 132 points / 86 comments on HN, posted Jul 31, 2026
- **URL:** https://x.com/shivangchheda22/status/2084815442452029460
- **Image:** Screenshot of the Manifest blog post attached (934 KB, captured headlessly via `x_background.py capture`)
- **Status:** POSTED

**Angle rationale:** This contrasts with Shivang's Not Diamond cache-aware router post from 8h ago. Not Diamond argued cache-aware routing works; Manifest's deprecation provides the counter-data — a 4-tier prompt classifier gave up after 4 months because task complexity isn't visible in the prompt alone. The concrete mechanism (complexity emerges through tool calls + repo context, not prompt text) is the kind of infra tradeoff Shivang's audience values, and the Linux kernel vs static site example makes it instantly legible. No duplicate angle in his recent timeline.
