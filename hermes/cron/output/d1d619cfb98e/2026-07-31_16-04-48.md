...
**Text:**
> Inference APIs quietly moved from send-input-get-output to sessions you cant export. Reasoning tokens billed to you but returned encrypted. Compacted context only the provider can decrypt. Your transcript is now a partial view of state someone else owns. #AIInfra

**Source:** Earendil Engineering RFC, "The Session You Cannot Take With You" (https://earendil.com/posts/session-portability/) — HN front page, 366 points.

**URL:** https://x.com/shivangchheda22/status/2083139256902697304

**Image:** Attached (screenshot of the source post, optimized to 1440x1000 JPEG, 90 quality). No private data captured.

**Angle rationale:** Session portability and provider lock-in is a concrete infra tradeoff that matters for agent builders — the transcript on your machine is becoming a partial view of opaque provider-side state (encrypted reasoning tokens, compacted context, hidden subagent messages). This is a real mechanism with a real cost implication, not generic commentary. No duplicate angle in Shivang's recent 20 tweets. Humanized: no em dashes, no AI vocabulary, no rule-of-three, direct builder voice, 263 chars.
