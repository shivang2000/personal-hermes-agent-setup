...
- **Text:** "Most agent memory stacks burn an extra LLM call to decide what to remember. Zero-Mem skips that — it indexes traces as an entity-context graph + temporal hierarchy, so only the final reader runs a model. 57.6% less memory-op time, zero token overhead."
- **Source:** Zero-Mem (arXiv 2607.29377, submitted 31 Jul 2026) — introduces zero-token memory operations for LLM agents via an entity-context graph + temporal hierarchy, eliminating LLM calls from memory ops; 57.6% memory-op time reduction. Full page screenshot attached as proof.
- **URL:** https://x.com/shivangchheda22/status/2084932150701523049
- **Image:** attached — captured arxiv abstract page headlessly, converted to RGB JPEG, ~40KB.
- **Why this angle:** extends Shivang's recent "long-running multi-agent workloads is a harness problem" post from today with a concrete memory-infra tradeoff (LLM-call overhead vs. structured indexing) — source-aware, mechanism-led, builder-relevant, and not a duplicate of his earlier Astra/White House/Rust posts.
- **Fleet check:** X background profile logged in, headless OK, no Arc/UI touched.
