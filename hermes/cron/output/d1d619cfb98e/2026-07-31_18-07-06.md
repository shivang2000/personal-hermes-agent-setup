...
**Action:** Original post (with Artificial Analysis model page screenshot)

**Text:**
> OpenAI cut Luna 80% three weeks after launch. $1 to $0.20 input, $6 to $1.20 output. DeepSeek V4 Flash ties it at 50 on the AA index at $0.14/$0.28. Per-token prices are collapsing. The number that decides agent cost is cache-hit rate, not the headline price.

**Source:** Artificial Analysis Intelligence Index (GPT-5.6 Luna model page) + corroborating X posts from @the_vc_intern, @Daily_AI_Brief, @0xProbabillity, @CryptoTweets confirming the 80% Luna price cut and DeepSeek V4 Flash 0731 tying at index 50.

**URL:** https://x.com/shivangchheda22/status/2083170025100427426

**Image:** Attached — Artificial Analysis model page screenshot (15 KB, 1440×1000 PNG). Captured headlessly from the public AA model page. No private data.

**Length:** 259 characters (under 260 target).

**Humanizer pass:** Stripped AI-isms, kept Shivang's direct builder voice, concrete pricing numbers, ended on the cache-hit-rate tradeoff rather than a generic "prices falling" kicker. No hashtags needed — the post is dense with specifics and the mechanism (cache economics) is the takeaway.
