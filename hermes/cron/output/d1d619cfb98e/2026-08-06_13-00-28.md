...
The isolated X automation profile is not logged in. The profile at `~/.hermes/browser-profiles/x-automation` shows `logged_in: false` and redirects to an access/login page.

Action needed: one-time manual login via `python3 ~/.hermes/scripts/x_background.py login` (log in manually in the separate window, return to terminal, press Enter). No post, search, or profile scan can run headlessly until that is done.
