...
The X API credits are depleted (`402 Payment Required`), which means I cannot search for recent tweets or post anything this cycle. `xurl whoami` still works because it's a cheap endpoint, but all search and write endpoints will fail.

**Fleet status note:** Two X-facing cron jobs (breaking-news trigger and reply-engagement scanner) are showing timeout errors from their last run, but they're still `active` (not paused). The opportunity scan job (this one) ran successfully last cycle. None are paused due to 429, so this isn't the fleet-wide quota-bomb pattern — it's a credit balance issue at the X Developer Portal level.
