...
**Posted tweet:** `https://x.com/shivangchheda22/status/2083924171067965619`

**Text:** DeepSeek V4 Flash 0731 beats V4-Pro-Preview on agent benchmarks at $0.14/M tokens. No vision modality though. Agents work around it by screenshot-diffing UI regions to find coordinates. The budget tier just overtook the premium preview. #LLM #OpenSource

**Source:** DeepSeek pricing page (`https://api-docs.deepseek.com/quick_start/pricing`), cross-referenced with live X reports from multiple users testing V4 Flash 0731 as a coding subagent (screenshot-diff UI workaround confirmed by @bitxeno303, @imhaoyi).

**Image:** DeepSeek pricing page screenshot attached (optimized to 1440×1000 JPEG, 159KB).

**Angle:** Breaking event — budget-tier model beats its own premium preview on agent benchmarks, with a concrete tradeoff (no vision → screenshot-diff workaround). Distinct from Shivang's earlier posts today about harness vs model layer and Kimi K3 licensing.
