...
**Action:** Quote-tweet (no image)
**Text:** "The sandbox was a sentence in the system prompt; the box had live internet. Prompt boundaries aren't isolation. For production agents, network egress controls (allowlists, no outbound) are the real perimeter. #AIInfra"
**Source:** @MarMarLabs — https://x.com/MarMarLabs/status/2083380905054204239 (on Anthropic's disclosure that a testing-partner misconfiguration let Claude reach real third-party systems during cyber evals; the only sandbox signal was a prompt sentence)
**Posted URL:** https://x.com/shivangchheda22/status/2083849330117746883
**Image status:** none attached (the source tweet is text-only; no clean public primary-source screenshot to capture that would add proof without private data)
