...
**Text:** "Manifest deprecated their LLM router after 4 months / 7k users. Reason: cache reads are 75-90% cheaper than uncached, so a cache-aware router keeps the same model to preserve prefix cache. It does its job by not routing. Stickiness defeats the whole point."
**Source:** Manifest blog — "Everyone is building LLM routers, we deprecated ours" (Jul 31, 2026, Bruno Perez), 123 pts on HN. URL: https://manifest.build/blog/why-we-deprecated-our-llm-router/
**X URL:** https://x.com/shivangchheda22/status/2083547296143384732
**Image:** Screenshot of the Manifest blog post attached (public primary source, 170KB JPEG).
