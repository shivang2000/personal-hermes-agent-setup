...
**Text:** "Most evals grade plausibility, not correctness. The output looks structured so it passes. In production the quiet ones bite you: a refund that reads complete but sits in a non-terminal state, a booking saved with the wrong currency code."

**Replying to:** @hamostaf04 (125 likes, 22 replies) — "Most evals are bullsh*t. A few companies have asked me for help with evals..."

**Source tweet:** https://x.com/hamostaf04/status/2083665857209917481

**Image:** None (text-only reply, appropriate for a reply addition)

**Status:** REPLIED ✓

**Rationale:** The original tweet (125 likes, from an eval consultant) argues teams grade whether agents "sound good, not whether it does the job." The reply adds a concrete production failure mode the original didn't name: silent non-terminal payment states and wrong-currency bookings. This is a useful technical addition that shows Shivang's production engineering judgment, not generic agreement. No image needed for a reply. No duplicate angle in Shivang's recent posts.
