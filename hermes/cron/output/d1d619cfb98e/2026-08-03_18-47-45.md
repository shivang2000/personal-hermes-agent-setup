...
**Text:** AirLLM runs Kimi K3 (2.8T params) on 3.72GB VRAM. Mechanism: sparse MoE per-expert streaming. Load only the expert a token routes to, not the whole layer. Active working set per token is tiny. You trade latency for memory, not the other way around. #AIInfra
**Source:** https://github.com/lyogavin/airllm (front-runner on HN, AirLLM README)
**URL:** https://x.com/shivangchheda22/status/2084267405644374208
**Image:** Attached (GitHub repo screenshot, 1440x1000 JPEG, public source)
**Image status:** Captured and posted successfullyThe post is complete. No further action needed — the tweet was successfully posted with the screenshot attached.

**Final report:**

- **Action:** Original post with image
- **Text:** AirLLM runs Kimi K3 (2.8T params) on 3.72GB VRAM. Mechanism: sparse MoE per-expert streaming. Load only the expert a token routes to, not the whole layer. Active working set per token is tiny. You trade latency for memory, not the other way around. #AIInfra
- **Length:** 257 chars (under 260 target, under 280 limit)
- **Source:** https://github.com/lyogavin/airllm (HN front page)
- **X URL:** https://x.com/shivangchheda22/status/2084267405644374208
- **Image:** Attached public GitHub repo screenshot, posted successfully
- **Humanizer pass:** No AI-isms detected. Concrete mechanism (sparse MoE per-expert streaming), specific numbers (2.8T params, 3.72GB VRAM), clear tradeoff framing. Builder voice preserved.
