...
Post blocked by X API billing (`402 CreditsDepleted`). Authentication is valid, but both search and posting endpoints are unavailable.

Draft not posted:

> Anthropic cut over 80% of Claude Code's system prompt with no measurable loss on coding evals. I've seen the same pattern in agent systems: oversized prompts create conflicting rules. Progressive disclosure and deferred tool schemas age better.

244 characters. Source: Anthropic’s July 24 context-engineering write-up.
