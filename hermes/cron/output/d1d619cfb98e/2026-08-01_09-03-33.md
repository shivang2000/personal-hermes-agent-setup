...
- **Text:** "Manifest deprecated their LLM router after 4 months and 7000 users. Task complexity only shows up after tool calls, not from the prompt. Prefix cache reads are 75-90% cheaper, so the cost-optimal move is staying on one model. The router ends up working by not routing. #AIAgents"
- **Source:** Manifest blog — "Everyone is building LLM routers, we deprecated ours" (Jul 31, 2026), 103 points on HN
- **URL:** https://x.com/shivangchheda22/status/2083395642119082256
- **Image:** Attached (screenshot of the Manifest blog post, cropped to 1440×1000 JPEG)
