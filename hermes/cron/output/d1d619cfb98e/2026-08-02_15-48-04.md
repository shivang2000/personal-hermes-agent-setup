...
**Action:** Original post (with screenshot of DeepSeek pricing page)

**Text:**
> DeepSeek V4 Flash headline price is $0.0028/M input tokens. That's cache-hit only. Cache miss is $0.14/M — 50x higher. For agent loops with fresh prompts each turn, cache hit rate drops fast. Price the miss, not the hit, before you commit to a per-task budget.

**Source:** DeepSeek API pricing page (https://api-docs.deepseek.com/quick_start/pricing) — screenshot attached as proof.

**URL:** https://x.com/shivangchheda22/status/2083859826359333257

**Image:** Attached (DeepSeek pricing page screenshot, converted to JPEG, 159KB).

**Rationale:** DeepSeek V4 Flash launched July 31 with MIT open weights and aggressive pricing that is driving the current AI pricing conversation (OpenAI's 80% Luna cut is a direct response). The cache-hit vs cache-miss pricing gap is a concrete, underdiscussed production tradeoff for agent builders — exactly the mechanism + cost framing that performs well for Shivang's audience. No duplicate of recent posts (recent tweets covered Kimi K3 cost, sandbox isolation, Seedance, Astra proofs, Supabase Evals — none touched DeepSeek cache pricing). Humanizer pass: no AI-isms, direct builder voice, concrete numbers, no hype words. 277 chars (under 280).
