...
**Action:** Quote-tweet
**Text:** "This matches what I see shipping multi-agent booking systems. The model swap is the easy part. Tool loop design, retry/idempotency, context retrieval, and error sanitization decide whether a booking completes or silently double-charges someone."
**Source:** Quote of @talwar_divyam's tweet on harness-vs-model economics (https://x.com/talwar_divyam/status/2084682721323040785) — "Harness choice changed coding-agent economics by up to 40×... model upgrades moved tokens per solved task only 1.0–1.3×. Context, tools, checks, and recovery decide what ships."
**URL:** https://x.com/shivangchheda22/status/2084846157256331600
**Image:** None (text-only source tweet, no benchmark chart to attach)

The angle: Divyam's harness-vs-model point is adjacent to Shivang's recent post-training beat but adds the harness dimension Shivang hasn't covered. The quote-tweet adds a concrete builder-implication take anchored in Shivang's actual multi-agent booking work (retry/idempotency, error sanitization, double-charge risk), which is a useful technical addition rather than a generic restatement. It also rides Divyam's engagement (5 replies, 3 RT, 5 likes, ~10h old).
