...
The isolated X automation profile is not logged in (`logged_in: false`). This run cannot post or scan X.

**Status:** `[BLOCKED]` — the headless X profile at `~/.hermes/browser-profiles/x-automation` needs its one-time manual login. Run:

```bash
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

Log in to X in the window that opens, return to the terminal, press Enter. After that, scheduled scans can post headlessly again.

No tweet was posted, no X search was run (auth gate), and no browser tab was touched.
