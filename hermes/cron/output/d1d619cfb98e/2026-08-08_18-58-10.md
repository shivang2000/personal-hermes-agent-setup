...
The isolated X automation profile is not logged in. I cannot post or scan this run.

**Status:** `[BLOCKED]` — the background Playwright profile (`~/.hermes/browser-profiles/x-automation`) has lost its X session (`logged_in: false`, page title "Just a moment…" indicates a Cloudflare/login interstitial).

**Action needed (one-time, manual):**
```
python3 /Users/shivang/.hermes/scripts/x_background.py login
```
Log in to X in the window that opens, return to the terminal, press Enter. Once `check` reports `logged_in: true`, the scheduled opportunity scans and autopost jobs will resume working.

I did not attempt any post, quote, or reply this run because the profile is not authenticated. No write was made, so there is nothing to retry or duplicate on the next run.
