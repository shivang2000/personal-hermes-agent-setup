...
Both posting paths are currently blocked. Reporting once as instructed.

**Opportunity scan report — blocked, no action taken.**

| Path | Status | Fix |
|---|---|---|
| `x_background.py` (default, $0) | `logged_in: false` — isolated Chromium profile at `~/.hermes/browser-profiles/x-automation` needs its one-time manual login | `python3 /Users/shivang/.hermes/scripts/x_background.py login` (log in manually in the separate window, then press Enter) |
| `xurl` API (fallback) | `HTTP 402: credits depleted` — account balance is $0. `xurl whoami` still works (owned read) but `xurl search` and `xurl post` fail | Deposit credits at Developer Console → Billing |

Could not scan for duplicates (`x_background.py profile` and `xurl search` both unavailable), so no tweet was drafted or posted to avoid duplicate-angle risk.

**One action needed from Shivang:** run the one-time login command above (restores the free browser path), and/or deposit X API credits (restores the xurl fallback). Once either is fixed, the next scheduled scan will resume normal operation.
