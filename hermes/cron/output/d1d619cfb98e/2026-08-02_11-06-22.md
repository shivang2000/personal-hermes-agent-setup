...
**Text:** When a production agent breaks, it's rarely the model. It's the harness: which tool call gets retried, which error ends the run, which permission was too broad. Same as any distributed system. Boundary logic is where things break. #AIAgents
**Source:** Derived from the active Anthropic/OpenAI cyber-eval breach discussion (eval sandbox isolation failures, 141,006 runs, 3 real breaches) — reframed as a general production-agent reliability principle rather than restating the eval-specific news Shivang already posted 13m ago.
**URL:** https://x.com/shivangchheda22/status/2083788919146266741
**Image:** None (insight post, no benchmark/proof artifact to attach)
**Chars:** 240/280
**Duplicate check:** Shivang's recent posts covered eval isolation specifically, Opus 5 long-horizon codegen, effort control, and Kimi K3. This post generalizes the harness-as-failure-surface angle into distributed-systems territory, distinct from the eval-specific post.
