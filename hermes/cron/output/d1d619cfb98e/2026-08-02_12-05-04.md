...
Reply posted to @0xlelouch_'s distributed-systems-for-AI-agents tweet (36 likes, 3 reposts).

**Action:** Reply
**Text:** "Idempotency keys get the attention. Terminal states do the real work. A retried refund against 'pending' can double it. Payment and refund both need explicit terminal states, or the retry loop and the state machine disagree about what happened."
**Source:** x.com/0xlelouch_/status/2083715603395715172 (Abhishek Singh, "Most distributed systems work in 2026 is still 10 concepts..." — timeouts, retries, idempotency)
**URL:** status REPLIED to https://x.com/0xlelouch_/status/2083715603395715172
**Image:** none

**Rationale:** Shivang already posted a 5-tweet eval/infra/Kimi cluster in the last hour — too saturated to add a 6th original. This reply adds a concrete production mechanism (terminal states for refund flows) the original tweet's idempotency bullet understates, drawing on AIConcierge321 payment/refund reliability work. Distinct domain (payment reliability) from the earlier eval/infra cluster. 36-likes tweet from a 5k+ follower account puts Shivang's handle in front of the distributed-systems/AI-agent audience.
