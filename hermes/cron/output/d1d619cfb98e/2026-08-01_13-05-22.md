...
**Text:** Perplexity open-sourced Numbat: endpoint security that blocks risky agent actions before they execute. The right layer. Prompt guardrails don't stop an agent mid-tool-call writing to prod or calling a payment API twice. Execution-layer controls do. #AIAgents
**Source:** Perplexity Numbat open-source release (confirmed across multiple independent accounts, July 31)
**URL:** https://x.com/shivangchheda22/status/2083456498978664598
**Image:** None (text-only — no public benchmark/chart; screenshot would be filler)
**Length:** 258 chars (under 260 preferred budget)

**Why this angle:** Shivang's 4 earlier posts today covered model pricing/benchmarks (DeepSeek V4 Flash, GPT-5.6 Luna, DeepSWE). Numbat is a fresh, non-duplicate topic — agent security at the execution layer. It connects to his production reliability work (payment APIs, idempotency, vendor integrations) and targets the recruiter/founder audience by showing judgment about *where* to put safety controls: execution layer, not prompt layer.
