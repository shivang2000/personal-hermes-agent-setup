...
> OpenAI cut GPT-5.6 Luna 80% to $0.20/M input tokens. DeepSeek V4 Flash dropped the same week: MIT-licensed open weights, 1/90 the cost of Claude Opus 4.8, matching Luna on agent benchmarks. For production agent budgets, the open/closed cost gap just collapsed.

**URL:** https://x.com/shivangchheda22/status/2083381164849684485

**Source:** Artificial Analysis model pricing page (https://artificialanalysis.ai/models/pricing) — screenshot attached showing the price/intelligence comparison that backs the cost-gap claim.

**Image:** Captured headlessly from the public pricing page, 1440×1000 RGB PNG, no private data.

**Why this angle:** OpenAI's 80% Luna price cut and DeepSeek V4 Flash's release (MIT-licensed, matching Luna on agent benchmarks at ~1/90 of Opus 4.8 cost) landed within the same week. The concrete mechanism is the cost/intelligence collapse for production agent budgets — a real tradeoff for builders running thousands of task calls/day. No duplicate in Shivang's recent timeline (recent posts covered the sandbox-escape eval failure, DeepSeek V4 Flash on the Aikido CVE benchmark, and Kimi K3 MoE VRAM streaming).
