...
**Action:** Quote-tweet (amplification of high-engagement Astra leak)
**Text:** 'Long-running multi-agent workloads' sounds like a model upgrade. In practice it is a harness problem. A model built for 1-hour agent loops exposes every retry bug, leaky context window, and missing checkpoint you ignored on 2-minute chat calls.
**Source:** Quote of @vepsi__ (163 likes, 8 reposts) on the OpenAI Astra/GPT-6 long-running multi-agent leak
**URL:** https://x.com/shivangchheda22/status/2084920955458204157
**Image:** Screenshot of source tweet attached (236 KB)
**Length:** 245 chars
**Angle:** Builder implication — reframes "long-running multi-agent" from a model capability into a harness/reliability problem (retry bugs, context window leaks, missing checkpoints), which connects directly to Shivang's recent posts on the 40x harness gap and MCP failure modes.
