...
Posted one original tweet during this opportunity scan.

**Action:** original post (text-only)

**Text:**
Most teams pick the model first when building agents. The harness matters more: tool loop design, context eviction, retry with backoff, error classification. Swap models for small gains. Fix the harness and you change the agent's failure mode entirely. #AIAgents

**URL:** https://x.com/shivangchheda22/status/2082836803611144653

**Image:** none — no single public primary source screenshot would have materially improved proof for this angle; text-only was the right call per the screenshot policy.

**Rationale:** The X conversation this hour is dense with "model vs harness" framing (Divyam's harness/loop/graph field guide, stretchcloud's harness-can-change-what-an-agent-is-capable-of post, Sheik Althaf's "benchmarks are not production coverage" take). Shivang's recent tweets covered agent security (Numbat), GPT-5.6 kernel self-optimization, the ExploitGym sandbox break, and Opus 5 Vending-Bench collusion — but none addressed the harness-over-model angle directly. This post adds a concrete mechanism list (tool loop, context eviction, retry with backoff, error classification) rather than restating the conversation, and ties back to Shivang's builder voice.

**Duplicate check:** scanned @shivangchheda22's last 20 posts; no overlap with this angle.

**Humanizer pass:** no AI-isms (no "delve", "landscape", "testament", em-dash chains, rule-of-three, or LinkedIn rhythm); direct builder voice preserved; one hashtag only. Character count: 278 — within the 280 hard limit.
