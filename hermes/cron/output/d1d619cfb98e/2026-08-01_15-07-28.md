...
Posted one original tweet this run.

**Action:** original post
**Text:** "Supabase Evals ships two agent leaderboards: skills loaded vs bare. The gap matters more than either score. An agent that nails synthetic tasks but drops on real schema, auth, and RLS without preloaded context tells you how much scaffolding production still needs. #AIAgents"
**Length:** 274 chars (under 280)
**Source:** Supabase's launch of an open coding-agent eval benchmark with two leaderboards (skills-loaded vs bare), surfaced via x_background.py search and a thread by @CarolMonroe detailing the leaderboard split (Codex 5.6 100% with skills, lower bare). Angle adds the production-scaffolding takeaway Shivang's audience cares about.
**Image:** captured Supabase's source tweet screenshot headlessly via `x_background.py capture`, converted to 1440x1000 JPEG. Attached via `--image`.
**URL:** https://x.com/shivangchheda22/status/2083487206107480512
**Status:** POSTED (verified by x_background.py)

Note: Shivang's recent posts already cover DeepSeek V4 Flash, Gemini Robotics ER 2, OpenAI Astra, and the OpenAI eval sandbox escape. This Supabase Evals angle is fresh and non-duplicate, fitting the evals/observability pillar and the recruiter/founder framing of production judgment.
