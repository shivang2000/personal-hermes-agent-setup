...
Shivang has posted 4 tweets in the last ~90 minutes covering the major AI agent/eval/security themes (Qwen3.8-Max GA, multi-agent booking systems, post-training > scaling, UK AISI filter-off evals). The remaining search angles (Hugging Face credential exposure, MirrorCode benchmark, Supabase Evals) overlap the same security/evals cluster he already covered. Adding a 5th post in the same window would be noise rather than signal — better to let the existing cluster breathe.
