...
**Text:** Supabase Evals scores coding agents on real backend work (RLS, Auth, Edge Functions), not staged demos. Claude Code's doc reference rate was under 40%. The harness matters more than the model. Real user issues become regression tests. #AIAgents

**Source:** https://supabase.com/blog/introducing-supabase-evals (captured as /tmp/x-opportunity-source.png, attached)

**URL:** https://x.com/shivangchheda22/status/2083829438358466768

**Image:** attached (Supabase Evals blog page screenshot)

**Why this angle:** Supabase open-sourced Evals ~6h ago — a benchmark that tests coding agents (Claude Code, Codex, OpenCode) on real backend tasks (RLS, Auth, Edge Functions) instead of curated demos. The standout finding: Claude Code's doc reference rate was under 40%, reinforcing that the harness/orchestration layer matters more than raw model intelligence. Real user issues get converted into regression tests. This hits Shivang's audience (recruiters/founders see production-hardening judgment; builders see a concrete eval mechanism) and avoids duplicating his recent posts on GPT-5.6 pricing, DeepSeek V4, Kimi K3, MCP, and multi-currency booking.
