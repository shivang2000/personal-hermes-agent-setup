...
**Text:** "Supabase open-sourced a coding-agent eval harness. Agents run against a real MCP server + CLI, scored with deterministic checks + LLM judge. 'Can this agent fix a broken RLS policy' is now a reproducible benchmark instead of a vibe check. #DevTools"

**Source:** Supabase's own announcement threads — `https://x.com/supabase/status/2083282155170340898` ("Introducing Supabase Evals") and the follow-up describing real MCP server + CLI + deterministic checks + LLM-as-judge scoring. Public benchmark at https://supabase.com/evals.

**X URL:** https://x.com/shivangchheda22/status/2083578254242586867

**Image status:** Attached — screenshot of https://supabase.com/evals (public primary source, 119KB, no private data).

**Angle chosen:** Supabase open-sourcing a real coding-agent eval harness is a concrete DevTools/evals mechanism (real MCP server, deterministic + LLM-judge scoring, tasks like fixing RLS policies), not generic benchmark commentary. Distinct from Shivang's earlier posts today (DeepSeek V4 API surface, GPT-5.6 price cuts, Claude sandbox escapes, Astra Lean proofs). Recruiters/founders see builder-relevant proof-of-work: the industry is moving from "vibe check" agent demos to reproducible eval harnesses, which aligns with Shivang's own eval/reliability thinking.
