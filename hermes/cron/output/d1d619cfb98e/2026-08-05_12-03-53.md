...
- **Text:** The 40x harness gap is why I spend more time on retry policy, tool schemas, and partial-data handling than model selection. A model upgrade moves 1.3x per task. The right harness moves 40x.
- **Source:** Quote of @talwar_divyam's tweet on harness vs model economics (controlled study: model upgrades moved tokens/solved-task 1.0–1.3×, harness choice changed economics by up to 40×)
- **URL:** https://x.com/shivangchheda22/status/2084890580556108025
- **Image:** none (no public primary source artifact to attach for this angle)
- **Status:** POSTED

New angle added to the original: concrete mechanisms (retry policy, tool schemas, partial-data handling) from production multi-agent work, not a restatement of the original study.
