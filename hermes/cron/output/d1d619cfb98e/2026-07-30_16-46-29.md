...
- **Type:** Reply to @nykdotdev
- **Target:** https://x.com/nykdotdev/status/2082786147369918682
- **Text:** "96.8% cache reads. Anthropic cache reads cost 10x less than fresh input tokens, so real spend is ~1/10th of what 266M implies. The tradeoff: 97% cache hits means your agent is mostly re-reading, not processing new info."
- **Image:** None (no clean public proof image for this angle)
- **Status:** REPLIED ✓

**Why this run:** The original tweet reported 266M tokens across 15 coding-agent sessions but didn't separate cache reads from fresh input in cost terms — 257M/266M=REDACTED_SET_LOCALLY
