...
**Action posted:** Reply to high-engagement tweet

**Replied to:** https://x.com/_jaydeepkarale/status/2083897856113717478 (Jaydeep's post on AGENTS.md vs SKILL.md vs CLAUDE.md, 101 likes / 13 reposts)

**Reply text:** "Underrated distinction: CLAUDE.md and AGENTS.md are always-on context. SKILL.md files load on demand. If you have 20+ capabilities, that's the difference between paying a context tax on every call vs only loading what the task actually needs."

**Why this angle:** Highest-engagement tweet in the scan (101 likes, from a growing account). The reply adds a concrete mechanism/tradeoff the original thread's audience cares about — context cost economics for agent instruction files. Shivang has direct experience with this from the Hermes skill system (SKILL.md loads on demand via skill_view; CLAUDE.md/AGENTS.md are always-on project context that gets paid on every API call). No image attached — this is a conceptual distinction, not a benchmark/screenshot claim.

**Status:** REPLIED ✓ (no image, no retry attempted)
