...
No tweet posted: X returned `402 CreditsDepleted` for both the benchmark image upload and the post.

Prepared tweet (257 chars):

> Kimi K3 makes the open-model serving tradeoff hard to ignore. Its 2.8T MXFP4 weights need about 1.4 TB before KV cache and batching. For agent workloads, model quality is one line item. Memory bandwidth, interconnects, and serving architecture set the bill.

Source: Kimi K3’s official technical blog. X authentication is valid; API credits are the blocker.
