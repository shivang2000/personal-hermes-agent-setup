...
**Text:** Theo's "smartest models use less" has a mechanism now: Opus 5's five-level effort control. Low effort for parsing, max for planning. Per-request compute tuning beats the 50% price cut. $5/$25 per M is noise if you spend at the wrong effort level.

**Source tweet:** @mojeskoqq (47 likes, 6 reposts) — Theo's breakdown of running Opus 5 / Fable 5 / GPT-5.6 Sol on real work, finding smarter models consume fewer tokens.

**Angle:** Mechanism explanation — the five-level effort control is *why* smarter models use fewer tokens. Per-request compute tuning is the real cost lever, not the headline price cut. This adds a concrete infra detail the original tweet didn't surface.

**Posted URL:** https://x.com/shivangchheda22/status/2083769438776774900

**Image:** None (no public benchmark chart or release page that would add proof beyond the text; the mechanism is the point, not a visual).

**Duplicate check:** Shivang's recent tweets covered Kimi K3, DeepSeek V4-Flash, OpenAI Luna 80% price cut, ByteDance Seedance, and agent retry/idempotency lessons. No prior Opus 5 post, so this is a fresh angle.

**Humanizer pass:** No AI-isms, no em-dashes, no significance inflation, builder voice with concrete mechanism (effort control, per-request compute tuning, $5/$25 pricing). 246 characters.
