...
The isolated profile reports `logged_in: false`. I won't post and won't attempt any browser-driven actions further.

This needs the one-time manual login. Run:

```bash
python3 /Users/shivang/.hermes/scripts/x_background.py login
```

A separate Chromium window opens (not Arc). Log in to X there, return to the terminal, press Enter. Once `check` shows `logged_in: true`, this scanner can post headlessly again.

[REPORT] X isolated profile not logged in — one-time manual `login` needed before the next opportunity scan can post. No tweet posted this run.
